...
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\InvokeMSBuild.ps1" -MSBuildPath "%MSBUILD_EXE%" -SolutionPath "%~dp0AshEngine.sln" -Target Tests -Configuration "%CONFIG%" -Platform "%PLATFORM%" -MaxCpuCount -Verbosity minimal
if errorlevel 1 (
    echo [Error] Build failed.
    exit /b 1
)

echo [Build] Tests build succeeded.
exit /b 0
...
:help
echo Usage: build_tests.bat [Configuration] [Platform]
echo.
echo Defaults:
...
echo.
echo Example:
echo   build_tests.bat Debug x64
exit /b 0
