...
**状态：** 提议  
**日期：** 2026-04-27  
**范围：** `project/src/engine/Function/Render`、`project/src/engine/Graphics`、shader 编译/反射/缓存路径、`SceneRenderer`、未来 `Decal` 渲染子系统  
**当前不纳入范围：** `UI` domain 的正式接入、`PostProcess` 接入材质系统、Dear ImGui backend 重写
...
- `Transparent`、`Decal` 只能继续在 renderer 内部扩散 ad-hoc 逻辑
- 未来材质编辑器难以围绕稳定 ABI 和编译框架展开
- shader 资源声明、资产绑定、program 缓存的职责边界不清晰
- `PostProcess` 与 material/instance 驱动模型共性较弱，若强行纳入同一框架会引入额外抽象负担
...
3. `StaticMesh`、`SkeletalMesh`、`Decal` 是不同的 `EngineShaderFamily`，不是不同的材质资产类型。
4. `PostProcess` 保持在材质系统之外，作为独立的 screen-pass shader/effect 管线处理。
5. `Transparent` 不是新的 `MaterialDomain`，而是 `Surface` domain 下的 `BlendMode + PassFamily` 组合。
6. 同一 domain 内，材质可写的 `MainNode` 输出字段必须固定，不允许随 `EngineShaderFamily` 漂移。
7. family 差异通过 adapter 与 capability 吃掉，而不是暴露给材质源码。
8. `.AshMat` 是材质资产描述文件，不是某个具体宿主 shader 的配置文件。
9. `MaterialShader` 只负责材质逻辑，不直接声明引擎管理的资源寄存器、entry point 或 pass 逻辑。
10. 编译期 permutation 缓存与运行时 binding snapshot 必须分层。
11. `MaterialResource` 承担单 permutation 的只读编译结果；`MaterialRenderProxy` 只承担某个材质实例的 render-thread 绑定快照。
...
`PostProcess` 不纳入本次统一材质框架。建议保持独立的 `PostProcessSubsystem` 或 `ScreenPassRenderer`，由其自行负责：

- fullscreen triangle / quad
- effect 专属 shader 绑定
- effect 专属参数 UBO / push constants / descriptor 组织
- screen-space pass orchestration

这条边界的正式含义是：

- `PostProcess` 不定义 `MaterialDomain`
- 不走 `.AshMat`
- 不走 `MaterialInstance`
- 不走 `MaterialRenderProxy`

如果未来确实需要“可资产化的后处理效果”，建议另起独立的 `ScreenPassShader` / `PostProcessEffect` 体系，而不是把它强行并回 `MaterialSystem`。
