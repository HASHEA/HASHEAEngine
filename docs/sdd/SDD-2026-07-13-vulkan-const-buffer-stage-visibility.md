...
只读追踪发现 `DX12Buffer::init()` 对 UPLOAD 与 READBACK 都在资源创建时持久 `Map`，且两者实际都传入空的 `D3D12_RANGE{0, 0}`。该范围按 [Microsoft `ID3D12Resource::Map` 契约](https://learn.microsoft.com/en-us/windows/win32/api/d3d12/nf-d3d12-id3d12resource-map) 明确表示 CPU 不会读取；官方 [readback heap 流程](https://learn.microsoft.com/en-us/windows/win32/direct3d12/readback-data-using-heaps) 则要求 fence 完成后以真实读取范围调用 `Map`，使 GPU 写入对 CPU 可见。当前 API 随后直接解引用该持久 pointer，且 RenderDevice 的 texel/capture readback 还会复用同一 buffer，属于独立 P1 readback 可见性契约错误。修订设计因此新增 `DX12Buffer.h/.cpp`：UPLOAD 保持持久 mapping，READBACK 改为每次 `get_mapped_data()` 以完整 buffer range 重新 Map，且 readback Unmap 声明空 written range。该新增 Graphics 文件面须重新批准后才能修改。
...
- 让 DX12 READBACK buffer 在 fence 已完成后按真实 CPU read range 建立可见 mapping；双 diagnostic 同进程与复用 readback buffer 不再观察 stale data。
...
- 不逐个改造 DX12 resource/view 析构器，不在 shutdown 录制 pending uploads，不把本修复扩大为 descriptor heap 或 D3D12MA 生命周期重构。
- 不改变 `Buffer::get_mapped_data()` 公共签名，不给它增加隐式 GPU wait，不把 DX12 readback 修复扩大为 upload heap、copy queue 或 fence API 重构。
...
- Known constraints:
  ...
  - `Buffer::get_mapped_data()` 不提供同步；调用者必须先等待对应 GPU fence。DX12 READBACK mapping 还必须在每次 CPU 读取前用真实 read range 重新建立，不能沿用创建时声明“CPU 不读”的 `{0,0}` persistent map。
...
### DX12 readback mapping contract

- `ASH_RESOURCE_ACCESS_WRITE` 继续在资源创建时持久 Map，并传入空 read range，明确 CPU 只写、GPU 读取。
- `ASH_RESOURCE_ACCESS_READ` 在创建时保持 unmapped。`DX12Buffer::get_mapped_data()` 每次调用都先结束上一次 readback mapping（若存在，Unmap 的 written range 为 `{0,0}`），并立即把 `m_mappedData` 置空，再以 `{0, buffer_size}` 调用 Map；只有 Map 成功且返回非空 pointer 时才重新赋值。失败路径记录 HRESULT、返回 null，并保持可安全重试/析构的 unmapped 状态。这样 Map 发生在调用者已经等待 GPU fence 之后，并以真实 CPU read range 完成必要 cache invalidation。
- `get_mapped_data()` 本身不等待 GPU、不改变公共 RHI 接口，也不把 readback mapping 放入普通渲染热路径。现有 texel readback、frame capture 与两项 RHI self-test 都通过同一接口受益。
- 若最小 A/B 后双 flag DX12 仍失败，立即恢复该实验并返回根因分析；不得在同一批准中顺带修改 fence、command allocator 或 descriptor heap。
...
| Graphics/DX12 readback | READBACK 在每次 CPU 访问时以完整 read range 重新 Map；Unmap 声明 CPU 未写 | `DirectX12/DX12Buffer.h/.cpp` |
...
| Tests | 新增确定性 Vulkan barrier policy doctest、CLI parsing coverage，并加入 Vulkan SDK header include | `project/src/tests/Graphics/vulkan_barrier_policy_tests.cpp`、`project/src/tests/Function/application_automation_tests.cpp`、`project/src/tests/premake5.lua` |
...
- `Buffer::get_mapped_data()` 公共接口不变；DX12 READBACK 从创建时错误 persistent map 改为调用时 full-range remap，UPLOAD mapping 行为不变。调用者仍负责在读取前完成 GPU fence wait。

### Backend impact

- Vulkan：stage scope 从 vertex-only 修正为 vertex + fragment + compute；layout/access 不变。
- DX12：ConstBuffer native state 映射不变；补齐首帧前 pending-upload teardown 与 readback CPU 可见 mapping，运行同一 GPU self-test，防止验证逻辑写成 Vulkan 特化。
...
- DX12 readback 每次 `get_mapped_data()` 增加一次必要的 Unmap/Map，只作用于验证、texel read 与 frame capture 等低频 CPU readback 路径；UPLOAD 与普通帧资源不变。
...
| Combined diagnostics | 同一 Application 生命周期内 command/readback 复用 | Vulkan/DX12 均同时传 `--rhi-selftest-indirect --rhi-selftest-constant-buffer`；两项都须 PASS。DX12 RED 已稳定复现 indirect PASS 后 constant tile 全零；修复后必须 GREEN |
| DX12 readback mapping | fence 后 full-range Map 与重复读取 | combined diagnostics 是真实 GPU RED/GREEN；DX12 validation fresh delta 还须无 Map/resource-state error，frame/texel capture 由 RenderGate 与 Application lifecycle 覆盖 |
...
6. 以 DX12 双 diagnostic 同进程 RED 为回归，修正 READBACK full-range Map/empty written-range Unmap；若 A/B 不转 GREEN 则恢复实验并重新定位，不顺带改 fence 或 allocator。
...
| DX12 READBACK 创建时以空 range persistent Map，复用时 CPU 观察 stale data | 创建时不 Map READBACK；每次 `get_mapped_data()` 在调用者 fence 后以完整 range remap，Unmap 声明空 written range；combined self-test 提供稳定 RED/GREEN，RenderGate 覆盖 capture |
| 把 readback 修复误扩为隐式同步或热路径开销 | `get_mapped_data()` 不等待 GPU，只改变 READBACK mapping；UPLOAD/DEFAULT buffer 与普通帧不变，调用点仍显式拥有 completion wait |
...
- DX12 readback mapping 修复保持独立提交。若 full-range remap A/B 不能让双 diagnostic GREEN，或引入现有 texel/frame capture regression，立即 revert 该提交并阻断 constant-buffer CLI/CI 合并；不得用额外 sleep/device idle、关闭 validation 或 bless golden 掩盖 stale readback。
...
- 双 flag DX12 GREEN 新增了 `DirectX12/DX12Buffer.h/.cpp` 文件面与 readback mapping 行为。该修订须完成独立设计审查并取得用户再次批准后，才能修改这两个新增生产文件；当前 self-test 修正保持未提交，GPU/构建窗口已释放。
