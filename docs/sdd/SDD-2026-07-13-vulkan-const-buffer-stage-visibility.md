...
- DX12：ConstBuffer native state 映射不变；补齐首帧前 pending-upload teardown，运行同一 GPU self-test，防止验证逻辑写成 Vulkan 特化。
...
5. 新增双后端 constant-buffer GPU self-test 与 CLI/CI 接线；显式处理 self-test RTV 状态并隔离 CB/UAV barrier batch；用双后端故障注入复现首帧前 pending ownership 导致的 Vulkan/VMA 与 DX12/D3D12MA teardown failure；两后端都在首次 drain 前释放未提交 upload ownership，Vulkan 另由唯一 accessor 把 `UINT32_MAX` 映射到 queue 0；确认两后端均由正常 teardown 非零退出且不崩溃/挂起，再恢复并跑双后端 GREEN。
