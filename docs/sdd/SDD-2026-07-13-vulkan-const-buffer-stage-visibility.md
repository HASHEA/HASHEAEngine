...
| Docs | 回写 ConstBuffer stage 契约、验证入口与命令行契约 | `docs/specs/modules/graphics.md`、`docs/specs/modules/application.md` |
...
| Failure-path RED/GREEN | 首帧前失败由正常 teardown 在有界时间内返回非零，不能由 watchdog 强杀掩盖 hang/leak | 临时把 compute expected red 从 17 改为 20：修复前稳定得到 mismatch 后 `0xC0000005`，accessor fallback 后稳定复现 VMA live allocation/hang；修复后须保留精确 mismatch/FAIL 与 wrapper exit 1，且在 120 秒 watchdog 前由 Application 正常完成 teardown。日志必须包含 `VMA leak tracking: no live VMA allocations detected before allocator shutdown.`，不得包含 `Fatal Error: readiness process deadline expired`、`Detected ... live VMA allocations`、`VMA leak:`、`Assertion failed` 或 `0xC0000005`；不得 hang、crash 或 forced exit。验收后再精确恢复 expected red |
...
5. 新增双后端 constant-buffer GPU self-test 与 CLI/CI 接线；用故障注入复现首帧前 deletion queue 越界与 pending upload 持有导致的 VMA teardown hang；在唯一 accessor 把 `UINT32_MAX` 映射到 queue 0，并在 shutdown 首次 drain 前释放未提交 upload ownership；确认注入后仅非零退出、不崩溃/挂起，再恢复并跑双后端 GREEN。
...
| 首帧前 fail-fast 时多个资源析构器索引无效 frame，pending upload 又持有 VMA 资源 | accessor 对 `UINT32_MAX` 返回 queue 0；shutdown 在首次 drain 前 swap/drop pending ownership，析构器入 queue 0 后由既有 flush 回收；正常 frame 不变；故障注入验证 mismatch 非零退出且无崩溃/挂起/live allocation |
