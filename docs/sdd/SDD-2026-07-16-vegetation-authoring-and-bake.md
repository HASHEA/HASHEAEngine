...
Done（2026-07-28；原 contract-first 方案于 2026-07-16 获用户批准，并授权后续阶段采用明确标注的推荐方案时无需重复等待；Task 2 codec 执行澄清于同日按该持续授权并入；2026-07-26 用户批准 Task 9 bake surface binding 修订为按次显式传入；Task 12 自动化矩阵与 Vulkan/DX12 人工 disabled-path 门禁均已完成）

## Context

总体设计见 `SDD-2026-07-13-world-scale-gpu-vegetation.md`。Phase 0 的 GPU 性能观测和 Phase 1 的 RenderGraph buffer、indexed indirect 与通用 GPUDriven page/prototype 底座已经完成并通过 PR #9 合入 `main`。本 SDD 批准时仓库尚无生产植被资产、Scene 组件、笔刷、烘焙产物或运行时渲染；截至 2026-07-28，前四项对应的 Phase 2 CPU implementation 与完整 exit matrix 已完成，运行时 vegetation rendering 仍未接入。
...
`VegetationSurfaceBinding` 保存 Scene 中的 `surface_entity_id`，全零 binding 非法并在 provider capture 前 fail closed。provider 在逻辑线程捕获一个 `shared_ptr<const IVegetationSurfaceSnapshot>`；snapshot 必须拥有采样所需的全部不可变数据和值拷贝 transform，不得持有 `Scene&`、Editor service、Terrain working set 或 catalog 内部指针。
...
Editor Panel 只绘制 palette、brush/bake/status 并提交 intent；`VegetationEditorService` 持有 working set、immutable publication、active stroke、bake future 与 last-known-good 状态。`GetStatusSnapshot()` 在 Editor 逻辑线程先同步一次 working-set publication，再从同一个 service state 按值返回 session、operation、source path、content/persisted generation、observed revision、active/last-known-good manifest digest、immutable palette view、capabilities 与 detail；capabilities 不得触发第二次同步或独立状态读取。palette pointer 只在完整成功的 load/edit/revert publication 后替换；failed/cancelled/stale/dirty-conflict/malformed 路径保留原 coherent publication 与 last-known-good identity。New Layer、palette Add/Replace/Remove、load/save/reload不依赖surface provider；palette edit使用与stroke相同的原子before/after patch和already-executed document command，Remove含非零weight时必须显式确认清除。provider 通过构造/装配注入，不用全局 singleton。没有 provider 时 palette/load/save可用，Paint/Erase/Bake显示精确 unavailable 原因并禁用。

Bake intent 必须按次调用 `RequestBake(const VegetationSurfaceBinding& binding, steady_clock::time_point now)`，调用者从该次明确的 Scene/Editor intent 提供 binding。service 不保存 mutable setter、session 默认 binding、最近一次 stroke binding、selection 或其他可变 UI fallback；zero binding 是入口第一项检查，在 service/provider/capability lookup、operation创建或worker排队前纯拒绝，即使当前已有active operation或没有provider，也不得改 working set、operation/detail、history、retry 状态和 active pointer。service 在接受请求时按值捕获该 exact binding；初始 capture 及 `Pending` 后每个到期重试都只复用该 operation 捕获值，禁止从 stroke、selection、Scene 或 UI 状态重新读取。Task 10 的集成 helper 和 Task 11 的 Bake caller 同样必须显式传入非零 binding；Phase 2 产品 bootstrap 因没有 provider 而保持 Bake disabled，不制造占位 binding。
...
Save 只写当前 source path，并携带打开时的 expected revision。worker只在目标同目录创建唯一stage writer，使用一个稳定native handle按 `1..1 MiB` 且offset严格连续的块写入；一次成功flush/close后token永久关闭，禁止二次写/flush，零字节artifact非法。随后有界回读同一stage byte snapshot，strict parse并核对canonical bytes/SHA，不持有目标replace能力。所有读取都接受显式byte ceiling；超限必须返回独立的 `LimitExceeded` 且bytes为空，任何非Succeeded结果都不得泄露部分snapshot。`VegetationPreparedLayerWrite` 是storage-only构造、外部只读的prepared capability：公开默认构造只能得到empty Failed，只有storage内部friend access可填充private payload；调用方只能读取，不能重绑kind、root/path/identity、stage、revision或operation serial。Editor主线程收到completion后才取得按 lowercase canonical path 的SHA-256派生的Windows named-mutex commit lease；每次prepare/commit都必须携带完整operation control（nonnull shared cancel flag + non-default absolute deadline），不完整control在lease或target mutation前fail closed。lease在等待中轮询cancel flag且不越过绝对deadline，把Cancelled/TimedOut/Failed作为null lease受控结果，禁止无限阻塞Editor。commit在lease前、acquired后、目标有界重读/revision校验后、紧邻publish前各检查一次control；任一检查失败只定向清理自己的stage。commit还必须在首次target mutation前证明prepared stage属于传入registry；伪造prepared或错误registry在publish前fail closed。lease内从目标同一byte snapshot计算revision；只有与expected相等、operation仍为当前serial且目标仍是existing regular file才允许replace-only `AtomicReplace`，目标不存在时禁止退化成create。shutdown先取消/回收worker且不消费completion，因此不存在service销毁后的迟到replace。service 的公开合同同时只允许一个 active operation；serial 只在成功 schedule 时递增，active 期间所有新 schedule 都在零副作用下拒绝，而 `ClearAsync` 只会在 exact task completion 已被观察并 `Join` 后执行。因此遵守公开 API 与 executor 合同的调用方无法构造“旧 serial completion 在新 operation 后迟到交付”的状态，serial 比较仍作为 defensive checked-commit 分支保留。Task 9 不得用 test-only getter/friend 或破坏 executor 合同伪造该状态；验证采用 active-conflict suite、production/manual executor exact-join/lifetime tests，以及对 serial schedule/check/clear 顺序的结构审查。所有目标必须经canonical/reparse-component检查证明仍在asset root内。FileOps结果有唯一legal shape：inspection/bytes/stage file/stage tree/lease的status与payload必须匹配；stage成功时path非空且writer非空、失败时二者都空，stage tree成功时root非空、失败时为空，lease Acquired时lease非空、其他状态必须为空。每个storage consumer都必须在读取或move payload前独立验证legal shape，不能信任production或scripted provider。成功replace/create-new已经消费自己的stage；`ForgetConsumedStageFile/Tree` 只是幂等registry bookkeeping，有效exact path即使entry已被并发定向清理也视为成功，不执行文件删除，且不得把已经成功的publish降级为失败。publish前失败只定向清理自己的stage；删除失败必须保留exact path供shutdown重试且操作最终为Failed，禁止RetryAll误碰同registry其他operation。成功后persisted generation和observed revision同时推进；任一步失败保留原文件、working set、history和dirty。Task 9 先以公开行为间接锁定该推进时机；Task 10 再通过 `VegetationEditorStatusSnapshot::persisted_generation` 与同一快照内的 `content_generation`、session、observed revision 共同验证 checked main-thread commit 后的 coherent publication。worker 已完成但 `Tick` 尚未消费时目标文件保持不变，checked main-thread commit 成功后的目标字节发生变化，后续 capabilities、save/reload 与 clean/dirty 行为继续绑定该已提交 session。New Layer只创建内存Dirty session并以“expected revision absent”表示目标必须不存在；首次Save在lease内用create-new/non-replacing publish，若目标在期间出现则保留对方文件并返回AlreadyExists。Save Copy As同样只接受asset root内不存在的`.AshVegetationLayer`并用create-new提交，但不重绑当前session。Chunk stage tree本身是cleanup registry的ownership unit；每个child必须是root内canonical relative path、由FileOps atomic create-new并返回同类稳定writer，禁止absolute/dot/reparse/existing child与路径隐式创建。publish消费child后无需逐文件forget，最终只定向清理剩余owned tree。

Task 7 的精确事务补充如下：prepared capability 为 move-only，copy 与 copy/move assignment 均删除，并私有绑定接受该 stage 的 cleanup registry；move构造必须在转移后显式把source重置为empty `Failed`，不能留下第二个可清理同一stage的capability。storage 在首次 `WriteBlock` 前独立验证 stage 是 normalized absolute、与 target 不同且为同目录 sibling，并比较从真实稳定handle取得的volume serial+file index；大小写别名与hard-link同一identity都在写入前fail closed，不能仅靠lowercase path。成功stage与存在的inspection必须携带identity，其他result shape必须清空。`AtomicReplace` 返回 `Replaced / TargetPreserved / RecoveryRequired`，并在同目录预留 registry-owned backup 传给 `ReplaceFileW`。紧邻调用前，registry必须把已存在且未在cleanup的exact owned stage原子pin为`Publishing`；调用后再原子解析为`TargetPreserved / Consumed / RecoveryRequired`，禁止用瞬时`OwnsStageFile`检查留下cleanup竞态。backup reservation必须绑定publication前由稳定target handle捕获的native identity，并保持该handle可共享delete地存活到`ReplaceFileW`返回，使随后backup清理只可能消费被移动的旧target对象。未解析的`Publishing` pin也属于fail-closed受保护恢复态：若终态转换不能确认，`AtomicReplace`可以上报该exact stage，`IsRecoveryStageFile`必须承认它，caller只能在同步调用返回后通过`ReleaseRecoveryStageFile`显式释放；普通cleanup与`RetryAll`始终排除它，且`RetryAll`必须把它列入`retained_recovery_stage_files`。成功publication若已经消费stage但留下未解析pin，Storage只在同一FileOps对exact stage返回合法`NotFound`后调用`ReconcileConsumedStageFileAfterPublish`擦除`Owned/Publishing` entry；仍存在的stage与显式`Recovery`永不由该路径删除，reconciliation失败也不得降级已完成的target mutation。若 Windows error 1177 已把原文件移到 backup，必须先恢复 target 才可返回普通失败；只有backup明确`Missing`才允许改保replacement stage，probe `Failed / Invalid`与present但恢复失败都必须继续保护backup。非1177且target不可确认时不得报告未证实存在的backup，而应保护经确认仍存在的replacement stage。post-call路径使用明确结果状态，只能报告仍受registry保护的恢复artifact，不能由异常兜底重新报告已消费或未生成的backup。consumer收到非法shape或无效recovery ownership时，也只有在exact registry仍持有prepared stage、同一有界回读成功且revision等于staged revision后才可把它转为fallback recovery；已消费/缺失stage返回受控失败，不重新制造entry。恢复失败时 exact backup（或最后可用的 replacement stage）进入 recovery-protected 集合，`RetryAll` 不得删除，并由 storage 以 `RecoveryRequired + recovery_path` 上报。上段“任一步失败保留原文件”的普通失败含义仅指 `TargetPreserved`；若 OS 已发生部分变更且自动恢复也失败，则不谎报普通失败，而是保留唯一恢复 artifact 并要求显式恢复。stage-file registry登记时必须同时保存exact normalized path与创建handle的native identity；`RemoveOwnedStageFile`仍只接受operation-owned naming shape，但还必须在`DELETE|FILE_READ_ATTRIBUTES`稳定handle上核对path、regular/non-reparse属性和expected identity，只有相同identity才用`FileDispositionInfo`删除。缺失或不同identity视为旧ownership已消费并让registry收敛，不删除同名replacement；ownership registration失败时不得猜测删除未认领path。stage-tree继续使用其独立的root identity递归删除合同。

Task 8 将同一身份合同扩展到 active chunk-set 和 create-new：prepared capability私有保存stage创建handle的native identity；Existing source-active还保存stage sibling创建后稳定复核所得native identity，NoActive保存unavailable/全零identity。active stage成功`FlushAndClose`后及最终有界回读返回后必须各自重新检查同一个operation control；晚到cancel/deadline只定向清理exact stage、清空ownership并返回`Cancelled/TimedOut`，不得产出prepared capability。publish前重新inspection必须仍是同一对象，Existing在lease内先做native identity CAS再回读48-byte ASVA，因此同字节delete/recreate ABA也返回`SourceChanged`且不调用replace。`NoActive` active pointer、Layer first Save和Copy As只走create-new，`Existing` active pointer和已有Layer只走replace。若任一publication调用在结果返回前抛异常，consumer不得根据“同字节”猜测成功；只有stage已消失、target native identity等于captured stage identity，且target的canonical bytes、revision/SHA与manifest identity都等于staged payload，才调和为终态成功。recovery result除registry记录的source-stage/target provenance外还必须有物理证据：exact stage保持原identity和新payload；distinct backup位于同一目录、为合法owned命名、identity可用，并strict decode为精确旧Layer revision或source-active ASVA。distinct backup的registry查询必须接收该次稳定inspection得到的当前identity，并同时精确匹配entry中publication前捕获的expected identity及source-stage/target provenance；path、provenance或同字节旧payload均不能替代当前对象identity。若同路径backup已换成新identity，consumer必须放弃旧target identity对应的registry ownership、保留replacement且不得上报其恢复能力，只允许回退到自己仍完整的prepared stage。invalid artifact只允许回退到自己仍完整的prepared stage，否则返回普通受控失败；若fallback取证证明原stage已经缺失或同名对象native identity已漂移，consumer必须原子放弃该旧identity的`Owned/Recovery` entry并使registry收敛，identity漂移replacement保持原状且不得收到remove调用。异常后的物理取证及fallback证明自身也必须no-throw，证据读取失败时保留未解析Publishing pin并返回普通`Failed`，不得暴露未经验证的recovery path。active reader封装普通FileOps/resolver异常，任何失败都不发布partial snapshot；入口先拒绝非canonical rootless Layer path并解析normalized absolute asset root，随后active/manifest/object的每个成功inspection都必须把canonical relative与resolved absolute精确绑定到该次请求且resolved path为root严格后代，path substitution/root escape在对应byte read前fail closed。

Task 9 的 Editor 编排把上述 checked capability 收紧为 request-entry 与 commit-time 两道身份栅栏。`RequestBake(binding, now)` 在入口一次性冻结 binding、immutable Layer、generation、dirty evidence、source revision、canonical/resolved/native source identity、resolver snapshot 与 canonical Species 集合；`BakeRequestContext::layer` 是 worker 与 completion-time palette identity 校验共同使用的唯一 frozen palette owner，service 不保留第二份 `captured_palette`。`Pending` 重试只允许重新 capture surface，禁止重新读取 working set、palette、source 或 resolver。一个集中 `ClearBakeRequest()` 负责释放上述 frozen Layer/Species/resolver/palette、binding、deadline 和 retry 字段；capture失败/throw/malformed、serial或surface identity失败、`Submit==0`、Pending重试耗尽和`Tick`绝对截止这些全部无worker终态都必须调用它，已经schedule的worker则保留到exact join后的`ClearAsync`。wrapper 返回 `Ready+snapshot` 后取得的 `surface_id` 仍必须非零；全零 identity 在 active-read、stage 创建或 worker `Submit` 前受控失败并走同一 `ClearBakeRequest()`，后续请求不得继承旧 binding/context；大 palette 生命周期测试锁定前一终态会在下一次请求前释放 frozen record。worker 完成后，Editor 逻辑线程重新读取并 strict decode 当前 source Layer，逐项比较 canonical path/identity、revision、resolved absolute path 与 native file identity；外部 in-place 修改、同字节 delete/recreate 或删除都返回 `SourceChanged`，不得切换 active pointer。commit 的 `current_identity` 必须从当前 working Layer/generation、当前 wrapper-validated surface identity、当前 palette canonical Species digest，以及本 operation 的 chunk-store/source-active/target coords 独立重建，禁止把 `prepared.expected_identity()` 当作 current identity 回传。snapshot 的虚 `identity()`、service直接及间接触发的 `InspectPath`/Layer reread统一经过 no-throw 边界；throw 或非法 identity 映射为受控失败/`SourceChanged`，仍尝试exact-owned cleanup，失败则保留精确retained ownership，不得从Create/Open/Request/`Tick`越出异常。

每个公开 intent/getter 在作出覆盖、capability 或 view 决策前先同步当前 working-set publication；当前 generation 与 `persisted_generation` 不同且 session 原为 Clean 时立即变为 Dirty，不能依赖下一次 `Tick`。同步首先用 O(1) working-set generation cursor 判定稳定帧；generation 未变时必须在 `publish_snapshot()` 与 palette path scan 前返回。generation 变化时才按 palette 的有序 `{path,id,digest}` identity 与已发布 immutable view 比较；identity 相同的 stroke/Undo/Redo generation 只推进 publication cursor并复用现有 view，不重新 typed-load、canonical encode 或 hash Species，identity 不同才重建 palette view。palette/stroke 已执行 mutation 的 history 记录必须完整处理 `Recorded / RolledBack / RollbackFailed`：Recorded 标 Dirty 且成功；RolledBack 同步当前 view、保持 generation 单调并恢复 mutation 前 Clean/Dirty（Clean 时把恢复后 generation 记作同一 persisted content）；RollbackFailed 保守标 Dirty 且失败，Reload/Open/Create 不得覆盖。

Reload 在 schedule 时冻结 working generation，worker 在同步文件读之前和之后都检查同一个 cancel/deadline control；Editor `Tick` 在发布 candidate 前再次要求 generation 未变。clean reload 与 confirmed `ReloadDiscard` 在 worker 完成后发生 Undo/Redo 或其他 generation 漂移时均返回 `SourceChanged` 并保留当前 working set；同步读阶段的 cancel/deadline 精确映射为 `Cancelled/TimedOut`。bake active-read 与 sampling 的 cancel/deadline同样保留精确状态，不得折叠为普通 `Failed`。所有异步 completion（成功、失败、异常、取消、超时或 stale）都必须先 observe、exact join，再调用 `RetryAll`；retained cleanup 使 service 保持 `Failed`、所有capability为false并阻断全部真实intent，只有后续 Editor-thread idle retry 收敛 registry 后才恢复可调度状态。timeout测试通过显式传入已过期的request time确定性命中，不使用wall-clock sleep，也不为测试扩张production clock/Deps seam。

Task 9 的 executor 不增加 dispatcher、FIFO 或 worker queue。production executor 每个提交创建一个受控 thread；调用方必须在 `Join(task_id)` 前观察 completion 与 exception，`Join` 等待 worker 后立即从任务表移除该 Task state，重复 Join/查询已消费 ID 为安全 no-op/absent，`CancelAndJoinAll` 也逐 ID 走同一回收路径，禁止保留无界完成历史。service `Shutdown` 同样必须 request cancel、等待 completion acknowledgement、观察 exception 后才 exact Join；teardown worker throw 映射为 `Failed`，不能被 Join 静默消费。测试用 caller barrier 只证明该 thread 已进入后、任何 provider/file/business side effect 前观察 cancel，不能称作 queued cancellation。test-only manual executor 则确定性覆盖 genuine cancel-before-run：取消请求立即把该 record 完成为 cancelled 且完全不执行 body；manual 与 production throwing adapter 分别锁定 shutdown observe-before-Join。first Save race 在 worker `Prepare` 完成后、main-thread `Tick` 前插入外部 target，必须保留外部字节并返回 `AlreadyExists`；Copy As 成功后继续 edit+Save 只能更新原 source，已创建 destination 保持 copy 时字节，证明 Copy As 不重绑 session。

Task 9 的 `DefaultLoadBudget()` 固定为 file/payload 256 MiB、decoded 1 GiB、palette 65534、tile 262144、instance 8388608；`DefaultChunkSetLoadBudget()`复用该per-file budget并固定manifest 262144、total inspected 2 GiB、summary 64 MiB。focused test逐字段锁定这些值，并分别取得两个返回值、污染其中一个的每个字段，再证明另一个实例与一次新调用仍逐字段等于默认值；`EditorApplicationImpl`的实际bootstrap构造/注入仍属于Task 11。production New Layer ID每个candidate只调用两次 `std::independent_bits_engine<std::random_device,64,uint64_t>`取得两个u64，并按明确little-endian顺序展开为16 bytes；全零/当前ID碰撞最多重试32次。测试继续只用既有 `create_layer_id` 注入覆盖zero/collision，不新增entropy test seam。Add/Replace的Undo/Redo必须恢复exact canonical authoring payload bytes且generation单调；4097个stroke request必须按每个字段保持原顺序拆成4096+1两批，不能只断言batch size。`EndStroke` 只创建一份 `shared_ptr<const StrokeContext>` immutable worker snapshot，closure 仅复制该指针；completion 通过 const reference 读取 service-owned optional，不再复制完整 path，使 active stroke 只保留一份 mutable context 与一份 worker snapshot。
...
- provider Pending：Editor按 `50,100,200,400,800,1000,1000,1000 ms` 最多8次重试，并受30秒operation deadline约束；初次请求按值捕获一个非零 `VegetationSurfaceBinding`，全部重试精确复用该值，不从任何可变状态重新求值；超限为TimedOut，不创建history、不发布chunk。
...
- cancellation在每个batch、chunk和每个最多1 MiB的stage写块前后检查。bake task由生命周期长于`VegetationEditorService`的受控Editor executor拥有；正常 cooperative service shutdown先置cancel、等待completion acknowledgement、观察exception并exact join，禁止detach或丢弃仍执行Engine/Editor代码的thread。join后通过`RetryAll`尝试清理exact-owned stage；cleanup失败必须保留registry精确归属的retained stage file/tree并报告Failed，不宣称无条件清理。EditorApplication shutdown在释放service、Engine DLL或worker infrastructure前join该executor全部task。provider实现必须只读resident immutable data且单batch不做IO，并按control deadline/cancel合同在50 ms内返回；throw/partial/malformed result由wrapper转Failed。fault test用cooperative slow provider在采样循环和stage写入中触发shutdown，必须证明cancel被观察、executor完成join、尝试exact-owned cleanup且无pointer切换；cleanup fault必须证明精确retained ownership。provider是受信任的in-process Engine扩展，C++17没有安全强杀任意卡死调用的机制；若实现违反50 ms/cancel合同，普通teardown可能无界等待，没有provider-specific hard kill或guaranteed diagnostic。仓库通用readiness process watchdog可终止进程并使门禁失败，但Phase 2不宣称对buggy/malicious provider有bounded in-process shutdown。需要强隔离的第三方provider必须在后续设计中移到进程外。
...
- Editor bake API 固定为按次显式 `RequestBake(const VegetationSurfaceBinding&, steady_clock::time_point)`；拒绝 zero binding，Pending retry 复用该 operation 的值拷贝，禁止 service-level implicit binding。
...
- Tasks 1–11 已实现：Asset contracts 与 typed loaders、immutable surface snapshot、Scene v7 reference/provider state、deterministic brush math 与 `RecordExecutedCommand`、ASVM/ASVA storage、bake worker/service、Editor history、panel integration、lifecycle wiring 和 last-known-good retention。
- 长期行为合同已汇总到 [`../specs/features/vegetation.md`](../specs/features/vegetation.md)，Asset、Scene、Editor 模块 spec 均链接到该 feature 真源。
- Task 12 自动化 exit matrix 已完成：source HEAD `10dd9d1433dac84d38b4aa6bacf25a50956d36a1` 上 Debug/Release direct focused 各通过 `338/338` cases、`39276/39276` assertions（`177` skipped），full suite 各通过 `515/515` cases、`41756/41756` assertions；fresh generation、Editor/Sandbox Debug/Release builds、ArchGate（`35` 条未新增的 legacy warning）、AIDevDoctor、四组合 readiness 均通过。最终审查发现并关闭两项 P2：Create Layer 扩展名误用区分大小写比较，以及 Scene Hierarchy clipboard 在场景替换后保留、可能让未 remap 的 surface 数值误绑定。替换审查先要求 clipboard 回归走 `ResetTransientState()`，其 unresolved `EditorTreeWidgetState::ResetDragState` 产生预期 link RED；三字段纯状态 reset 原样 inline 后取得 GREEN。随后契约审查要求验证真实事件链路，生产 panel 与测试共用 `BindSceneHierarchySceneLifetimeEvents`：测试绑定真实 `EditorEventBus`，发布 active-scene、reload、replace 三种事件并逐次检查两份 clipboard 数组。最终 Debug/Release exact 各通过 `1/1` case、`6/6` assertions，focused 各通过 `339/339` cases、`39279/39279` assertions（`177` skipped），full 各通过 `516/516` cases、`41759/41759` assertions。修复新增 mixed-case 4 条与 lifecycle 6 条断言；当前 Windows token 创建可选目录 symlink 返回 error `1314`，对应 storage test 比有权限路径少 7 条条件断言，因此相对原证据净增 `+10-7=+3`，计数一致。
- non-bless RenderGate 报告为 `Intermediate/test-reports/render-gate/20260726-225756-841-51704-a9d2c145/`；Standard PerfGate 报告为 `Intermediate/test-reports/perf-gate/20260726-225855-409-62620-349c3c4a/`，按仓库设计保持无固定 baseline 的 `MISSING`；VegetationFullPipeline Release 报告为 `Intermediate/test-reports/perf-gate/20260726-230214-725-37128-07947be8/`，Vulkan/DX12 均为 `PASS/COMPARED`。未 bless 或修改 golden/perf baseline。
- post-remediation Standard 首次四组合报告 `Intermediate/test-reports/perf-gate/20260728-111016-083-78468-8835755f/` 为 PASS。随后 frozen rerun `Intermediate/test-reports/perf-gate/20260728-111537-301-83988-122cc676/` 因 Sandbox/DX12 单次 `1.785159 s` sampling gap 被保留为 FAIL；未改代码/配置/baseline 的完整重试 `Intermediate/test-reports/perf-gate/20260728-112001-249-78452-35b6d97b/` 四项 PASS、最大 gap `0.089162 s`、零 warning/failure。`Intermediate/test-reports/ai-dev/20260728-032406/` 对该 FAIL/PASS 链给出 `4/4` fresh 与零 inferred gap；后续 review-driven test strengthening 仍需在最终候选上刷新 affected gates 与 AIDevDoctor。
- event-bus lifecycle-test strengthening 后，Debug/Release exact 各 `1/1` case、`6/6` assertions，focused 各 `339/339` cases、`39279/39279` assertions，full 各 `516/516` cases、`41759/41759` assertions。Editor Debug/Release 与 ArchGate 再次 PASS；fresh Editor Vulkan/DX12 readiness（PID `84004` / `34516`）均在 frame `3` 就绪，四份新日志无拒绝诊断并完整 shutdown/all-memory-free，两根进程退出、四份配置逐字节恢复。此前 freeze Standard `Intermediate/test-reports/perf-gate/20260728-113221-924-78488-26510d98/` 四项 PASS、零 warning/failure、最大 gap `0.046864 s`。最终文档锁之后的 fresh run 由固定包 `Intermediate/test-reports/ai-dev/phase2-final-freeze-20260728-v2/` 索引；该包必须显示 `4/4` fresh、logs/perf fresh、零 inferred gap 后才能送最终哈希审查。
- 2026-07-28 人工门禁使用独占 session `manual-20260728-100807-064`。用户分别确认 Vulkan 与 DX12 的 New Layer、Add、Save、Reload、same-ID Replace、Remove、Undo/Redo、panel responsiveness，以及 Paint/Erase/Bake 精确禁用原因 `No vegetation surface provider is registered.`；产品中未出现 test provider 或 fake Terrain。首轮 DX12 的 UI 结论因缺少独立 `manual-dx12.AshVegetationLayer` 落盘证据未被接受，补验在 AI 只读确认该文件、原 Species ID/path/digest 后完成并由用户报告 PASS。三次 Editor 会话均正常退出，每次新增的 Engine/Application 日志拒绝诊断为 `0`。
- 人工门禁后四份配置按 SHA-256 精确恢复，session-owned 目录在校验父路径、文件白名单与无 reparse 后删除；effective Editor/Sandbox/AshImageDiff/gate roots 为 `0`，GPU window 已释放。Phase 2 因而关闭为 **Done**。
- Phase 2 仍只包含 CPU authoring/bake/storage；不新增 vegetation GPU rendering，也不修改 Graphics/RenderGraph/RHI。GPU integration 仍是独立的 Phase 3 handoff。

## Verification plan

| 验证 | 覆盖 | 命令 |
| --- | --- | --- |
| Focused unit | wire golden、SHA/CRC/hash vectors、strict shape、坐标、brush/patch/provider/bake | `product\bin64\Debug-windows-x86_64\Tests.exe --test-case="*Vegetation*"`；`product\bin64\Release-windows-x86_64\Tests.exe --test-case="*Vegetation*"`（先构建对应 Tests target） |
...
| 异步 stale worker覆盖新 layer/surface | single-active schedule/exact-join结构不变量 + defensive operation serial + layer generation +完整 surface identity checked commit；不通过测试后门伪造不合法completion |
...
| provider永久Pending或违反cancel合同 | Pending由8次/30s约束；trusted provider接收deadline/cancel且正常50 ms内返回，worker由受控executor join且禁止detach；违反合同可能使普通in-process teardown无界等待，不保证provider-specific诊断，仅通用readiness process watchdog可终止进程并使门禁失败 |
