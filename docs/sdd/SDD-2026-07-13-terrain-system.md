...
六个设计部分已于 2026-07-13 在任务讨论中逐段批准；标准 SDD 落盘版本也于同日完成用户复核。实现必须遵守本文边界；任何新增范围或公共 RHI 扩展都需要先修订 SDD 并重新批准。

风险级别：**S2**。原因是该功能同时改变 Scene 数据模型、Asset 格式、Render 数据流、shader 绑定约定、Editor 工作流和自动化门禁。方案不新增 RHI 公共接口，也不引入新的后端能力要求。
...
首期工具为 Raise/Lower、Smooth、Flatten、Noise、Paint、Erase。公共 brush 参数为世界空间半径、强度、falloff、stroke spacing、当前层和确定性 seed。

笔刷按世界移动距离重采样，不按 frame 调用次数采样。同一输入路径、参数与 seed 必须在不同输入帧率下生成相同结果。

- Raise/Lower 写 Additive delta。
- Smooth 读取包含邻接 halo 的当前合成高度，再把结果写入当前层。
- Flatten 在首次落笔捕获目标高度，并写 Alpha target。
- Noise 用 world sample coordinate 与 stroke seed 生成确定性值。
- Paint 增加选中层权重并重分配其余权重。
- Erase 降低选中层权重，余量按其他非零权重比例分配；全部为零时回退 Layer 0。

一次 mouse-down 到 mouse-up 形成一个 `TerrainStrokeCommand`。命令在首次修改 tile 时懒捕获 before 数据，只保存改变矩形的 before/after patch；patch 使用内建确定性 RLE 或 raw 编码，不引入额外压缩依赖。空修改不进入历史。命令按 Terrain asset ID、Layer UUID 和 tile coordinate 定位，连续 stroke 不得越序发布。
...
后端只使用现有 buffer staging upload、texture array、UAV texture、instanced draw 和 RenderGraph barrier。若实现中发现必须增加公共 texture-region upload 或其他 RHI API，当前 SDD 即失效，必须先修订 SDD 并重新批准，不得顺手扩 Graphics。
...
| 验证 | 覆盖 | 命令 |
| --- | --- | --- |
| Unit tests | tile ownership、层合成、权重归一化、笔刷确定性、undo、LOD、query、容器恢复 | `RunTests.bat Debug`、`RunTests.bat Release` |
...
| RHI validation | StorageBuffer upload、atlas UAV/SRV barrier、texture array | Vulkan/DX12 validation 各跑 Terrain readiness capture |
...
1. **Phase 0：可行性与空 Editor 基线**
   - 建立 1440p TerrainPerf harness 和 GPU pass 计时。
...
2. **Phase 1：资产与纯逻辑**
   ...
   - 实现 layer compose、笔刷核、query、min/max/LOD error 与故障注入测试。
...
4. **Phase 3：Editor authoring**
   ...
   - 完成六类工具、layer 管理、undo/redo、save/reload/conflict UI。
...
无。任何新增范围（物理碰撞、spline、植被、World Partition、RVT、Nanite、RHI texture-region API）都需要新的或修订后的 SDD，并重新取得批准。
