...
- Entry points:
  ...
  - PerfGate：Standard 为 Debug、Sandbox/Editor × Vulkan/DX12；Phase 0 已新增固定 Release 2560×1440、Sandbox × Vulkan/DX12 的 `VegetationFullPipeline`，输出 CPU/GPU frame 与 11 个 pass-group metric。
...
- Known constraints:
  ...
  - `IGpuProfilerContext` 仍是 Tracy facade；独立的 `IGpuTimingTelemetry` 已提供固定 ring、exact submit acknowledgement 与 non-blocking resolved timestamp samples，仅由 PerfGate opt-in。
  - Phase 0 的 RTX 5060 Release 2K 无植被 candidate 已测得 `GPU.Frame` avg 14.39 ms（Vulkan）/14.55 ms（DX12），coverage 1.0；当前完整管线未达到 3.33 ms 整帧目标，AO 约 11.1 ms 是主要现状成本。candidate 尚未 bless。
...
Phase 0 于 2026-07-14 完成可归因候选报告：RTX 5060、2560×1440、Release、完整无植被场景下 Vulkan/DX12 `GPU.Frame` avg 为 14.3884/14.5529 ms，全部 required metric coverage 为 1.0。该结果说明 300 FPS 不是在当前完整 pipeline 上直接叠加植被即可达到的水位；后续阶段必须同时区分“现有 renderer 基础成本”“植被增量成本”和“整体优化收益”。本次未 bless baseline，任何毫秒硬门槛仍须由用户确认候选机器与水位后发布。
...
1. Phase 0 已完成可信的 Release CPU/GPU candidate；baseline bless 仍等待用户确认水位。
