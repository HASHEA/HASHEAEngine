...
这两个脚本会通过 `scripts/InvokeMSBuild.ps1` 启动 MSBuild。该 PowerShell 启动器会先修复当前进程里大小写冲突的 `PATH` / `Path`，再为 MSBuild 子进程重建大小写不敏感的环境变量表，只保留规范的 `Path`。这样可以避免 MSBuild/.NET 在某些 shell 环境下因为同时看到 `PATH` 与 `Path` 而触发 `MSB6001`。
...
- swapchain buffer 数
- 标题
- vsync 默认值（运行时通常由 `Engine.ini` 的 `Rendering.VSync` 覆盖）
- 期望 backend
- backend 配置文件路径
...
3. 读取 `Engine.ini`，解析并发布已注册的全局渲染开关
4. 解析最终 backend
5. 创建窗口
6. 创建 `GraphicsContext`
7. 创建 `Swapchain`
8. 创建 `RenderDevice`
9. 创建 `Renderer`
10. 初始化 `SceneRenderer`
11. 初始化 `ScenePresentationSubsystem`
12. 创建并初始化 `UIContext`
...
- `EnvironmentLighting.RuntimeBakeCache`
- `EnvironmentLighting.DefaultRadianceSize`
- `EnvironmentLighting.DefaultIrradianceSize`
- `EnvironmentLighting.DefaultPrefilterSize`
- `EnvironmentLighting.DefaultPrefilterMipCount`
- `EnvironmentLighting.DefaultBRDFLUTSize`
- `EnvironmentLighting.DefaultSampleCount`
...
- `Base/IniConfig.*` 是 Engine 共享 INI 解析入口，DynamicRHI 和渲染开关配置都应复用它；解析已兼容 UTF-8 BOM，不要假设配置文件一定是无 BOM 文本。
...
运行时全局渲染开关由 `project/src/engine/Function/Render/RenderFeatureConfig.*` 管理，配置源同样是 `product/config/Engine.ini`。新增全局开关时应扩展 `RenderSwitch`、`k_render_switch_descriptors` 和必要的访问器，避免在各个 pass 中手写 INI 读取逻辑。

当前注册的全局渲染开关：

- `Rendering.VSync`：默认 `false`。关闭时 swapchain present mode 优先级为 `MAILBOX`、`IMMEDIATE`、`FIFO`；开启时只请求 `FIFO`，让 Vulkan / DX12 都走垂直同步 present。
...
`directional_shadows.enabled=false` 时跳过 sunlight 与普通 directional light 的 shadow depth / mask pass，directional light 走 unshadowed 路径。方向光数量在 scene 数据层不设硬上限，但 `LightComponent.sunlight=true` 的 directional light 全场景最多只能有一个。Sunlight 使用 `SunLightShadowPass` 的大场景 CSM、static cache、dynamic overlay、screen-space shadow mask 和 cascade debug view；普通 `LightType::Directional && !sunlight` 使用 `DirectionalLightShadowPass`，在每个普通方向光自己的 deferred lighting pass 之前临时 clear atlas、绘制所有 shadow caster、生成该光的 screen-space mask，并在下一个普通方向光处理时复用同类 transient graph 资源生命周期。普通方向光不使用 static cache，不区分 static/dynamic caster，也不因为共享 directional shadow atlas budget 跳过光照。Sunlight 与普通方向光共享 `DirectionalShadowCascadeMath`：cascade frustum 仍按 view-space Z split 构建，稳定路径使用 view-space cascade sphere 固定投影尺寸，并按实际 cascade tile resolution 对 light-space sphere center 做 shadow texel snapping，减少相机移动和旋转时阴影边缘的亚像素游动。shadow mask 在非末级 cascade 的 split 末端按固定过渡比例采样下一层并平滑混合，降低不同 cascade projection / bias / resolution 造成的切层硬边。两类方向光也共享 `DirectionalShadowCascadeShaderData` 布局：`texel_size_flags.xy` 必须都写入 atlas-normalized texel size 供 shadow mask PCF 二维采样，`z` 为 cascade index，`w` 为 cache mode；sunlight dynamic cascade pass 从 static cache 拷贝时必须在 RenderGraph builder 中声明 `static_cache_atlas` 的 GraphicsSRV read。
...
`volumetric_lighting.enabled=false`、`scattering_intensity <= 0` 或 `density <= 0` 时跳过 Volumetric Lighting 并把 sky/background 后的 HDR 直接交给 Bloom；缺失 `scene_config.volumetric_lighting` 的旧场景走默认关闭。Volumetric Lighting 在 `SceneSkyBackgroundPass` 后、`BloomPass` 前运行；主路径使用 frustum-aligned froxel atlas，通过 compute pass 写入 `SceneVolumetricDensity`、按 view-space depth 重建 froxel 世界坐标并注入 directional / point / spot 的 `SceneVolumetricScattering`、temporal scattering history，以及读取 `SceneDeferredDepth` 的全屏 `SceneVolumetricIntegratedLighting`。体积覆盖距离由 sunlight CSM 最远 split / light shadow distance 或 point/spot range 推导，避免 reverse-Z device depth 线性切片把 froxel 全部压在相机近处。当当前帧存在 shadowed `sunlight=true` directional light 时，froxel light injection 会读取 `SunLightShadowPass` 生成的 dynamic atlas 与 `SceneDirectionalShadowCascades`，按 froxel view depth 选择 cascade，并在 split 末端平滑过渡到下一层，再用 PCF shadow factor 调制 sunlight in-scattering；缺少 sunlight shadow 资源时会绑定 dummy cascade buffer 并关闭该分支，仍保持 Vulkan/DX12 反射资源完整。Temporal scattering history 不再按相同 atlas UV 直接采样上一帧，而是保存每个 view 的上一帧 view-projection、camera position、view forward、froxel atlas/slice 参数、view distance、reverse-Z 与 scene static/light revision；temporal shader 会把当前 froxel 还原为世界坐标，投影到上一帧屏幕，再用上一帧 camera forward 计算上一帧 depth slice 并采样 history atlas。若 atlas/slice/view distance/reverse-Z 或 static/light revision 不兼容，或像素级重投影越界，则本帧自动把 blend 置零并写入当前帧结果，避免相机旋转时 frustum-aligned froxel history 错按旧 UV 混合造成 lightshaft 漂移；history 采样后还会按当前散射值做幅度 clamp，降低遮挡和光照突变带来的 ghosting。Light injection 使用密度归一化可见度标定：`density` 主要控制 extinction/雾浓度，`scattering_intensity` 主要控制光束亮度，避免为了看见 lightshaft 被迫把整屏雾拉高。Integrate pass 会按每个像素的 scene depth 截断可见 froxel segment，用该 segment 对应的 view-space 世界长度累计 in-scattering，并对 in-scattering 与 extinction 都应用世界长度尺度标定后写入 `SceneVolumetricIntegratedLighting.a`；fullscreen composite 使用 `hdr * transmittance + in_scattering` 写回线性 HDR，避免 froxel slice 数、sunlight shadow distance 或全背景深度把整屏 additive 雾量放大。`volumetric_lighting.debug_view` 可把 Density / Scattering / IntegratedLighting / HistoryValidity / CompositeHDR / ScreenSpaceMask / ScreenSpaceFinal 中间结果直接送入后续 tone-map/Bloom 链；`screen_space_fallback=true` 时改走 depth mask + radial blur 的 `SceneLightShaftScreenSpacePass`，输出 screen-space lightshaft composite；该路径只作为低成本兼容 fallback 或调试对比，不作为 sunlight 主效果路径。标准 `Sandbox.scene.json` 显式开启高可见度配置（`froxel_resolution_scale=0.25`、`scattering_intensity=2.0`），用于覆盖 directional / point / spot 光源的 froxel 体积光、shadowed sunlight injection、temporal reprojection 和 HDR composite 主链路。

`[RenderDebugView].Enabled=false`、`[RenderDebugView].Selected=Off` 或 `SceneOutput` 时不追加 pass，保持正常主画面。选择其他当前帧可用 RT 时，`SceneRenderer` 会在 `SceneDeferredToneMapPass` 后、`SceneDebugDrawOverlayPass` 前追加 `SceneRenderDebugViewPass`，读取被选 RT 为 SRV，并把可视化结果直接写回 `SceneRenderViewContext.output_target`。当前注册项包括 `SceneOutput`、`SceneGBufferA/B/C/D/E`、`SceneDeferredDepth`、`SceneDeferredLightingDiffuse`、`SceneDeferredLightingSpecular`、`SceneDeferredSceneHDRLinear`、`SceneAmbientOcclusionRaw`、`SceneAmbientOcclusion`、`SceneAmbientOcclusionTemporal`、`SceneBloomSetup`、`SceneBloomMip1..6`、`SceneBloomFinal`、`SceneBloomCompositeHDR`、`SceneVolumetricDensity`、`SceneVolumetricScattering`、`SceneVolumetricIntegratedLighting`、`SceneVolumetricCompositeHDR`、`SceneVolumetricHistoryValidity`、`SceneLightShaftOcclusionMask`、`SceneLightShaftScreenSpaceFinal`、`SunLightShadowDynamicAtlas`、`SunLightShadowStaticCache`、`SceneSunLightShadowMask`、`SceneSunLightShadowCascadeIndex`、`DirectionalLightShadowTransientAtlas` 与 `DirectionalLightShadowTransientMask`；depth、normal、motion vector、AO 与 linear HDR 会使用对应 visualization shader 路径，其中 `LinearHDR` 是 raw pre-tonemap 线性值的 clamped preview，不应用 exposure 或 ACES。体积光的 density / scattering / history validity 是 froxel atlas 资源，debug item 使用 atlas 实际尺寸；`SceneVolumetricIntegratedLighting` 与 `SceneVolumetricCompositeHDR` 才是全屏 view 尺寸。

Reverse-Z 行为约定：

- Reverse-Z 是 `CameraComponent.reverse_z` 的逐相机属性，不是 `Engine.ini` 全局开关
- `SceneView` 和 `VisibleRenderFrame` 会携带当前相机的 `reverse_z`，后续 pass 和 draw 提交必须使用 view/frame 上的值
- 开启时投影矩阵把 near 映射到 NDC depth `1.0`，far 映射到 `0.0`
...
```text
SceneGBufferPass -> SceneAmbientOcclusionPass -> optional SunLightShadowPass depth/cache/debug -> SceneDeferredLightingBasePass -> per light: optional SunLightShadowMask or DirectionalLightShadowPass transient depth/mask -> SceneDeferredDirectionalLightingPass / PointLightingPass / SpotLightingPass -> SceneDeferredEnvironmentLightingPass -> SceneDeferredCompositePass -> SceneSkyBackgroundPass -> SceneBloomSetupPass -> SceneBloomDownsamplePasses -> SceneBloomUpsamplePasses -> SceneBloomCompositePass -> SceneDeferredToneMapPass
```

### Skybox 与 IBL Environment

Scene authoring 使用单个 active `EnvironmentComponent`。Runtime 渲染消费 `VisibleEnvironmentData`（经 `RenderScene` 快照进入 `VisibleRenderFrame`）；render code 不得直接读取 `Scene`。环境资产使用 `.ashibl` v1：uncompressed radiance cubemap、irradiance cubemap、prefiltered specular cubemap 与 BRDF LUT。irradiance cubemap 存储半球积分后的 irradiance，常量 radiance 会烘焙为 `pi * radiance`；`SceneDeferredEnvironmentLightingPass` 的 diffuse IBL 负责乘 Lambert BRDF（`base_color / pi`），不能把 irradiance 当 radiance 直接乘 base color。BRDF LUT 使用 split-sum IBL 对应的 Smith geometry term（`k = roughness^2 / 2`），避免 specular IBL scale/bias 在掠射角大于 1 后额外放大环境反射。`EnvironmentComponent.intensity` 是总倍率；`lighting_intensity` 只进入 `SceneDeferredEnvironmentLightingPass`，`background_intensity` 只进入 `SceneSkyBackgroundPass`，用于避免可见 skybox 需要正常亮度时把无阴影 IBL 填光同步推高。`.ashibl` metadata 可携带 `dominant_light`：`direction` 是 skybox-local 的单位向量，指向 radiance cubemap 高亮峰值簇中心；`azimuth_degrees` / `elevation_degrees` 是同一方向的角度表达；`luminance` 记录峰值亮度；`source` 当前为 `radiance_peak_cluster_v1`。`Scene::load_from_file()` 会在 active environment 的 `.ashibl` metadata 存在 `dominant_light` 时创建或更新 `EnvironmentSunLight`：先应用 `EnvironmentComponent.rotation_degrees`，再取反作为 DirectionalLight 的传播方向。`read_ashibl_metadata_file()` 可只读取 metadata，不加载 radiance / irradiance / specular / BRDF payload。离线 baking 与 cache baking 均通过 `EnvironmentMapBaker` 产出同一 `EnvironmentMapCookedData` contract。

`Application::initialize()` 会读取 `[EnvironmentLighting]` 并设置 `.ashibl` bake 默认值。`RenderAssetManager::request_environment_map_asset()` 不在 render 请求路径同步烘焙 HDR；它优先加载 `EnvironmentComponent.ibl_asset_path`，失败后在 `RuntimeBakeCache=true` 时按 `source_texture_path` 内容 hash 查找 `product/caches/EnvironmentCaches/<hash>.ashibl`，仍缺失则立即使用 fallback environment。`RuntimeBakeCache=false` 时跳过 source cache 查找。

`EntryPoint.h` 提供 headless `.ashibl` 生成入口：`Sandbox.exe --bake-ashibl <source.hdr> <output.ashibl>`。该入口不创建窗口或 RHI device，默认使用 `[EnvironmentLighting]` bake 参数，也可通过 `--radiance-size=N`、`--irradiance-size=N`、`--prefilter-size=N`、`--prefilter-mips=N`、`--brdf-lut-size=N` 和 `--sample-count=N` 覆盖单次离线烘焙参数，通过 `EnvironmentMapBaker` 产出 uncompressed `.ashibl` v1 聚合资产。
...
具体 API 语义、pass 编写约束和常见错误记录在 `docs/specs/modules/render-graph.md`；新增 graph pass 时应先按该 spec 补齐资源声明、Tracy 打点、错误处理和 Vulkan / DX12 验证。
...
- Framebuffer cache 会在连续数帧未使用后清理；`clear_transient_render_targets()` 也会清掉 framebuffer cache，避免 transient / resize 路径长期压住旧 attachment。主窗口 resize 后，`Application` 会调用该清理并通知 `SceneRenderer::handle_output_resized()` 丢弃尺寸敏感 history；`RenderDevice::begin_frame()` 还会检测 swapchain extent 变化并做一次兜底清理，覆盖 Vulkan out-of-date/suboptimal 等后端触发的 resize 路径。
...
- Vulkan swapchain acquire semaphore wait 和 swapchain image barrier source scope 必须覆盖 acquire 后首个 layout / copy / clear barrier。当前 `VulkanContext` 对 `vulkanRenderBeginSemaphore` 使用 all-commands wait stage，`VulkanCommandBuffer` 对 swapchain image 写入前 barrier 并入 `COLOR_ATTACHMENT_OUTPUT` source stage，Dear ImGui Vulkan secondary viewport 的 acquire 后首个 dynamic-rendering layout barrier 也必须使用 `COLOR_ATTACHMENT_OUTPUT` source stage，避免 Editor 首帧、secondary viewport 或 offscreen-present 路径在 barrier 先于 color attachment draw 时触发 `SYNC-HAZARD-WRITE-AFTER-READ`。
...
- `SceneRenderer` 默认 deferred path 通过 `RenderGraphBuilder` 创建 graph transient GBuffer targets、可采样 D32 depth、统一 `SceneAmbientOcclusion` AO texture、两张 RGBA16F 的 `SceneDeferredLightingDiffuse` / `SceneDeferredLightingSpecular`（光照 pass MRT 输出）、一张 RGBA16F 的 `SceneDeferredSceneHDRLinear`（composite 输出的线性 HDR 中转），体积光使用的 `SceneVolumetricDensity` / `SceneVolumetricScattering` / `SceneVolumetricScatteringTemporal` / `SceneVolumetricHistoryValidity` / `SceneVolumetricIntegratedLighting` transient `RGBA32_SFLOAT` storage-image RT、`SceneVolumetricCompositeHDR` 或 `SceneLightShaftOcclusionMask` / `SceneLightShaftScreenSpaceCompositeHDR` transient `RGBA16_SFLOAT` RT，以及 Bloom 使用的 `SceneBloomSetup` / `SceneBloomMip1..6` / `SceneBloomFinal` / `SceneBloomCompositeHDR` transient `RGBA16_SFLOAT` RT。froxel atlas 会按 `VolumetricLightingQuality` 限制运行时 froxel budget：Low 512K、Medium 1M、High 2M、Epic 4M；`froxel_resolution_scale` 只决定期望上限，超过 budget 时会按比例缩小 tile 尺寸。
...
- `VolumetricLightingPass` 采样 sky/background 后的 HDR、GBuffer depth 和当前 view/light frame data；默认 froxel 主路径按 density -> light injection -> temporal resolve -> depth-aware full-screen integrate -> HDR composite 的 compute/graphics pass 顺序执行，并通过 per-view persistent history ping-pong 维护 `SceneVolumetricScatteringHistory`。light injection 从 atlas pixel 解码 froxel slice，按 view-space depth 重建世界坐标后分别计算 directional / point / spot 的相位函数、距离衰减和聚光锥衰减；体积最大 view distance 从 sunlight shadow plan 的最远 split、sunlight shadow distance 或局部光 range 推导。对 shadowed sunlight directional，pass 会额外读取 `deferred_resources.sunlight_shadow_dynamic_atlas` 和 `SunLightShadowPassOutputs::cascade_buffer`，绑定到 `DirectionalShadowDynamicAtlas` / `SceneDirectionalShadowCascades`，并通过 `AshVolumetricSunShadowParams` 指定 first cascade、cascade count、启用位和 PCF radius。shader 先按 froxel view depth 命中 cascade split，再用 froxel 世界坐标投影并采样 dynamic atlas 得到 `sunlight_shadow`，split 末端会混合下一层 cascade，再乘到 directional attenuation。散射强度使用 density normalization 做艺术标定，低 `density` 仍能产生可见 shaft，雾浓度继续由 extinction 控制；integrate pass 再按 `SceneDeferredDepth` 决定每个屏幕像素可见 segment，用 segment 的 view-space 世界长度积分散射并输出 transmittance，composite 以 `hdr * transmittance + in_scattering` 合成，避免只写 atlas 左上角、穿过前景表面、reverse-Z 近处分布塌缩或随 slice 数变化整屏发灰/无效果。atlas 日志会输出 configured/effective scale，resize 时 `clear_history()` 会丢弃旧尺寸 history，避免最大化后的大体积光 history 在小窗口继续存活。配置 `debug_view` 时会像 Bloom debug view 一样把选中的体积光中间 RT 直接作为后续后处理输入；配置 `screen_space_fallback=true` 时改用 `SceneLightShaftScreenSpacePass` 生成 depth occlusion mask 和 radial lightshaft composite，便于低成本兼容路径验证。
...
- 静态网格 instance vertex buffer 使用逻辑 slot + 3 帧物理 ring：同一渲染 frame 内的多 view/pass submit 仍从 frame-local 游标申请不同逻辑 slot，跨渲染 frame 时再由 `resolve_frame_lagged_instance_buffer_slot()` 映射到不同物理 buffer slot。该 epoch 来自渲染侧当前 `Application::get_frame_index()`，不能使用逻辑侧 prepared packet 的 `VisibleRenderFrame::frame_index`，否则 render 线程复用 packet 时 motion vector history 会被误判无效。temporal history 只允许 GBuffer pass 使用；shadow / DepthOnly pass 只依赖当前 pass 的 object-to-clip，避免 shadow depth draw 的实例流携带 camera previous matrix。不要把该路径改回单物理 slot 复用，否则 Vulkan Release 下 CPU 写 host-visible buffer 可能覆盖 GPU 仍在读取的上一帧实例矩阵，表现为 GBuffer depth / normal / motion vector 裂缝或闪烁。
...
- Vulkan texture upload 必须同时对齐 staging slice base offset 和每个 subresource 的局部 offset；`vkCmdCopyBufferToImage` 的 `bufferOffset` 要满足目标格式 texel block size 对齐，例如 `RGBA16_SFLOAT` 需要 8 字节对齐
...
材质 V2 生成路径会把 `MaterialResource::combined_source_hash` 传入 `GraphicsProgramDesc::source_hash`，再进入 `ShaderCreation::source_hash`。因此材质编译 hash、generated bindings 文本、host shader path、用户 shader 内容或 `Surface.StaticMesh` 共享 HLSL header 变化时，都不应复用旧 shader 对象。
...
- DX12 root signature 生成会把反射到的 CBV/SRV/UAV 绑定合并到一个 descriptor table，把 sampler 绑定合并到另一个 descriptor table；`DX12ProgramBindingInfo::descriptorOffset` 记录每个资源在 table 内的偏移，避免资源数量较多的 pass 触发 D3D12 64 DWORD root signature 限制
...
- submit 阶段在 proxy 缺失或材质 version / compile hash / shader 文件签名 / texture asset version 变化时，通过 `RenderAssetManager` 的 cache 准备/刷新 `MaterialRenderProxy`；shader 文件签名包含 engine family host、用户 shader、generated bindings 和 `Surface.StaticMesh` 共享 HLSL header，检查按 `MaterialRenderProxy` 节流，不能在每个可见 section、每帧都进行 filesystem probe
...
- `SceneInstantiationDesc` + free function `instantiate_asset(Scene&, AssetDatabase&, AssetId, ...)`：按 `AssetId` 实例化 `Prefab` / `Model` / `Mesh`，可指定 parent、root name override 和 root world transform；`use_world_transform=false` 时保持资源自身 transform。
- `request_scene_entity_pick(SceneViewBindingHandle, x, y)` / `poll_scene_entity_pick_result(...)`：Editor Scene viewport GPU picking facade。Engine 在 GBuffer 后写入 per-pixel entity id，并在帧提交完成后 readback 点击像素；`ScenePickResult` 返回 `EntityId`、depth、world position、normal。
...
- instance buffer 资源池仍由 `SceneRenderer` 复用，但 buffer slot 分配分为两层：渲染侧当前 frame epoch 变化时只重置逻辑 slot 游标；实际物理 slot 由逻辑 slot 和 3 帧 frame-lag ring 共同决定。同一渲染 frame 内顺序提交多个 scene view 时，每个 view/pass 使用不同逻辑 slot range；跨渲染 frame 时连续 3 帧不会复用同一个 host-visible vertex buffer 物理 slot，避免 CPU 更新下一帧实例矩阵时影响上一帧已提交给 Vulkan queue 的 draw。motion vector 的 previous view state 只给 GBuffer pass 使用，取同 view 上一次实际提交成功的状态，不用 `VisibleRenderFrame::frame_index` 拦截同一个 prepared packet 的重复渲染；DepthOnly / shadow caster pass 不查询该 history。
...
- `Scene::load_from_file("product/assets/scenes/Sandbox.scene.json")`
- scene asset 引用的模型、环境和材质资源校验
- 逻辑线程相机更新
- `Sandbox` 在标准场景 ready 后声明一个 `Window` output 和一个 scene binding
...
- DX12 shader-visible descriptor heap 分帧复用：
  - `DX12GPUDescriptorHeap::begin_frame(frameIndex, frameCount)` 会把 heap capacity 按 in-flight frame slot 分区；`DX12Context::begin_frame()` 等待当前 slot fence 后只重置该 slot 的线性分配 offset
  - 这条规则避免 RenderGraph 等高频 SRV 绑定路径在本帧复用上一帧命令列表仍可能引用的 descriptor slot，从而触发 GPU-based validation 的 invalid descriptor / invalid resource 报错
...
日常提交前操作、报告字段和基线更新流程见 `docs/PerfGateUsageGuide.md`。
...
- PerfGate 性能门禁使用说明：`docs/PerfGateUsageGuide.md`
