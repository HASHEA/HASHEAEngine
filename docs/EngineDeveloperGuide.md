...
Temporal=false
TemporalBlend=0.85
TemporalDepthThreshold=0.01
TemporalNormalThreshold=0.75
DebugView=Off
...
- `AmbientOcclusion.Temporal`
- `AmbientOcclusion.TemporalBlend`
- `AmbientOcclusion.TemporalDepthThreshold`
- `AmbientOcclusion.TemporalNormalThreshold`
- `AmbientOcclusion.DebugView`
...
`Mode` 支持 `Off`、`SSAO`、`HBAO`、`GTAO`。默认 `Off` 会让 `AmbientOcclusionPass` 注册一张 neutral 1x1 white AO texture，保证 deferred lighting 始终绑定同一个 `SceneAmbientOcclusion` shader resource。`Quality` 影响样本数 / 方向数 / 步进数，`Radius`、`Intensity`、`Power` 控制遮蔽半径和曲线，`HalfResolution` 与 `Blur` 用于性能和噪声折中。`Temporal=true` 时 AO pass 会在 raw/blur AO 后追加 `SceneAmbientOcclusionTemporalPass`，使用 `GBufferD` 的 screen-space motion vector 回投影上一帧 AO，并通过上一帧 depth + normal history 做 rejection；`TemporalBlend` 是历史权重，`TemporalDepthThreshold` 和 `TemporalNormalThreshold` 控制历史接受条件。`DebugView` 支持 `Off`、`RawAO`、`FinalAO`、`TemporalAO`、`HistoryWeight`、`Depth`、`Normal`、`MotionVector`；非 `Off` 时 `SceneRenderer` 会跳过 deferred lighting / composite，把 `AmbientOcclusionPass` 生成的调试图直接交给 tone-map，用于定位 AO 抖动来源。

通用 Render Debug View 使用独立 typed runtime config：`project/src/engine/Function/Render/RenderDebugView.*`。`Application::initialize()` 会读取 `[RenderDebugView]`，`Enabled=true` 时 `Application` 的 Engine overlay 会显示 `Render Debug View` 窗口，并通过下拉框选择当前帧注册的 active RT：
...
- `MotionVector3D` 表示 screen-space velocity.xy + 上一帧 depth.z，不是 world-space velocity；`TemporalFlags.a` 表示当前像素是否有可用上一帧静态网格历史。`Surface.StaticMesh.GBuffer` 通过 per-view previous view-projection 和 per-entity previous world transform 生成该 velocity。
...
- `SceneRenderer` 注册 `SceneGBufferPass`，随后由 `AmbientOcclusionPass::add_passes()` 注册 AO（`Off` 模式导入 neutral white texture；`SSAO` / `HBAO` / `GTAO` 生成 AO RT，可选 depth-aware blur；`Temporal=true` 时再追加 `SceneAmbientOcclusionTemporalPass`，读取当前 AO、`GBufferD` motion vector、当前 depth/normal、上一帧 AO history 和上一帧 depth/normal meta history，输出 temporal AO 并 ping-pong 写回两组持久 history RT）。当 `AmbientOcclusion.DebugView != Off` 时，`SceneRenderer` 会直接 tone-map AO debug texture；否则继续由 `DeferredLightingPass::add_passes()` 注册 `SceneDeferredLightingAccumPass` 与 `SceneDeferredCompositePass`，随后由 `PostProcessToneMapPass::add_pass()` 注册 `SceneDeferredToneMapPass`。当 `[RenderDebugView] Enabled=true` 且 `Selected` 指向当前帧可用 RT 时，`SceneRenderDebugViewPass` 会追加在 tone-map 后并替换主画面输出。
...
- instance buffer 资源池仍由 `SceneRenderer` 复用，但 buffer slot 分配分为两层：渲染侧当前 frame epoch 变化时只重置逻辑 slot 游标；实际物理 slot 由逻辑 slot 和 3 帧 frame-lag ring 共同决定。同一渲染 frame 内顺序提交多个 scene view 时，每个 view/pass 使用不同逻辑 slot range；跨渲染 frame 时连续 3 帧不会复用同一个 host-visible vertex buffer 物理 slot，避免 CPU 更新下一帧实例矩阵时影响上一帧已提交给 Vulkan queue 的 draw。motion vector 的 previous view state 取同 view 上一次实际提交成功的状态，不用 `VisibleRenderFrame::frame_index` 拦截同一个 prepared packet 的重复渲染。
