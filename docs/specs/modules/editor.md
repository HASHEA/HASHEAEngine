...
- 命令与 undo/redo：`EditorCommand`（`GetLabel/Execute/Undo/TryMerge/GetSelectionAfterExecute/GetSelectionAfterUndo`，配合 `EditorCommandSelection`）、`CompositeCommand`；执行入队走 `CommandService`，历史栈在 `UndoRedoService`。常规命令由 `ExecuteCommand` 先执行再入栈；Engine 已原子完成的编辑（如 Terrain stroke）只能走 `RecordExecutedCommand`，该路径不得再次调用 `Execute`，且历史存储失败时必须用同一命令的 `Undo` 回滚，避免出现未被历史追踪的修改。
