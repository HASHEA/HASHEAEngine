...
- `TerrainEditorService` 的文件操作状态为 `AwaitingPublication -> Running -> Succeeded/Failed`。Save 先等待目标 generation 完整合成/发布，再把按操作所需的 immutable snapshot、自 persisted generation 以来的累积 Component payload 和解析路径交给 worker；worker 不捕获 service 或 mutable working set。成功只标记启动时捕获的 generation，更新编辑仍 dirty；失败保留 working data 和历史。Save 与 Optimize 只使用当前 working-set `source_path` 并拒绝显式目标路径；只有 Save Copy As 接受显式新路径，并通过同目录临时完整写入、回读校验和 durable non-replacing 原子发布创建副本，竞态目标与失败均不得被覆盖或留下临时件；它不重绑当前资产、不清 dirty。Optimize 仅允许 clean saved asset。所有文件操作都要求有效 AssetDatabase root，生产路径必须留在 root 内且扩展名为 `.AshTerrain`；worker 不可用时快速失败，不回退到 UI 线程写盘。Running worker 允许后续编辑，但在完成前无条件禁止换资产、关闭或直接打开另一 snapshot，即使当前 session 已因 history rollback 失败进入 quarantine；`Shutdown()` 等待 Running worker 完成后才释放 service 状态。
...
- Terrain authoring 还需运行 focused stroke/layer/viewport/brush-overlay/save-generation/file-operation/`Ctrl+S` 行为测试、幂等历史门禁、稳定选择、锁定与 rollback/quarantine 契约，以及 Debug/Release 全量 `RunTests.bat`；brush overlay 改变 scene presentation output，必须额外运行双后端 `RunRenderGate.bat`。
