...
- PerfGate 受保护 report-import 只能读取当前仓库 `Intermediate/test-reports/perf-gate/` 下、SHA-256 与显式批准值逐字节匹配、未 bless且整体/逐 run 均为 PASS 的 schema v2 报告。summary 与每个 raw telemetry 都必须一次 `ReadAllBytes`，对同一 byte snapshot 做 strict UTF-8 JSON 解析和 SHA-256；artifact 顶层必须是原生 JSON object，summary 只保存 report-relative raw 路径与 digest。导入复用 live canonical validator 从 raw 重建 CPU/memory/draw、runtime、GPU identity/counts/coverage/全部 required metric，并与 summary 逐项一致后才允许写 baseline；最终 baseline 数值直接取 canonical raw，而不是容差接近的 summary 副本。所有 schema scalar 都必须保持原生 JSON 类型：identity/status/path 是非空 string，计数是 integer，性能/coverage/timeline 是 number，flags 是 boolean；`workload`、`runtime`、CPU/memory/render stats、GPU/metrics/backend info 与逐 metric 等容器必须是原生 object。单元素数组不得经 PowerShell 管道展开后伪装成标量或对象。adapter/driver 必须是原生非空字符串；engine shutdown live bytes 与支持 GPU allocator 时的 allocator shutdown live bytes 必须是原生非负整数且为 0。profile/configuration/baseline path、精确 target×backend 矩阵、当前 source SHA、OS build、workload fingerprint、elapsed coverage/gap/rate、进程树/Job cleanup 也必须全部匹配。任何校验失败都不得改变 baseline；成功 entry 记录 repo-relative `source_report` 与 `source_report_sha256`，且导入路径在任何 build/report/process side effect 前退出。
- report-import 对 summary/raw 的每个现存路径组件做 lexical containment 与 `ReparsePoint` 拒绝。此合同防止静态 symlink/junction 逃逸，但信任执行导入的本机用户在校验期间不会并发替换目录组件；当前实现没有用持久文件句柄锁定最终路径，不能宣称抵御同权限恶意本地写者的竞态攻击。
