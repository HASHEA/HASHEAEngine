...
- `SandboxStandardScene`：
  ...
  - `start(AssetDatabase&)` / `reset()` / `update_logic(InputState)`；状态机 `SandboxStandardSceneLoadState`（Idle/LoadingScene/Ready/Failed）；`snapshot()` 返回 `SandboxStandardSceneSnapshot`（scene、主相机实体 id、推荐移速）。
