...
- 线程：`initialize_threading(EngineThreadingConfig)`；`register_current_thread_role` + `is_in_render/logic/worker_thread` 判定；跨线程投递用 `enqueue_render_command` / `pump_render_commands` / `flush_render_commands`；后台任务用 `dispatch_background_task`。`initialize_threading` 与 `shutdown_threading` 只允许生命周期 owner 线程外部串行调用，不声明 init/init、init/shutdown 或 shutdown/shutdown 并发安全；内部 condition mutex 仅把 worker enqueue admission 与 owner shutdown 的 stop flip 线性化，shutdown 已接受的命令会排空，stop flip 后的新命令返回 exceptional ready future。
...
- 不同 `LogService` 初始化会话不得复用或 append 到同一日志文件；快速重启、同进程重新初始化和多进程并行均必须保留各自完整日志，Engine/Application 文件必须可由相同 session 后缀配对。
...
- [SDD-2026-07-14-log-file-session-identity](../../sdd/SDD-2026-07-14-log-file-session-identity.md)：日志文件以微秒、PID 和进程内序号组成唯一会话身份，Engine/Application 成对且快速重启不覆盖前序证据。
