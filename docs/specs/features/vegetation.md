...
Windows 长路径清理回归把 store root 精确构造成 248 个 native path
characters，再验证唯一 owned stage tree 超过 260。fixture 的长度不得依赖
checkout root、PID、时间戳或固定目录段造成的步长余数。
...
The completed feature was subsequently rebased onto Terrain-enabled `main` at `d8b4db8137483037eb123dd89589d819c3b4e11c`. Integration preserves the combined Scene v7 schema, Terrain plus Vegetation asset identities, and both Editor services. Fresh Debug/Release full suites each passed `965/965` cases with no failures (`2` privilege-dependent skips), executing `70115/70115` and `70116/70116` assertions respectively. Four builds, ArchGate, AIDevDoctor, four-combination readiness, and non-bless RenderGate passed. Standard PerfGate passed all four runs at `Intermediate/test-reports/perf-gate/20260728-154310-682-40936-353aa4d6/`. The final uninterrupted VegetationFullPipeline run at `Intermediate/test-reports/perf-gate/20260728-161100-669-20676-c387bc32/` completed both backends as `COMPARED` with zero failures, complete GPU coverage, unchanged draw count, and whole-frame GPU avg/p95 deltas within `4.4%`; its retained WARN is limited to CPU P99 tail variance and `0.1191/0.0675 ms` DeferredLighting average deltas. Because Phase 2 has no Graphics/RenderGraph/RHI or runtime vegetation-rendering change, the WARN is accepted as environment/sub-millisecond variance without blessing or modifying the baseline. Two earlier interrupted reports remain retained as failures because their logs show the window was minimized for about 20 seconds during sampling.
