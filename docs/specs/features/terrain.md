...
- surface vertex shader 从全 Terrain packed R16 buffer 解码高度，用下一粗级的同三角形平面做 geomorph；较粗邻边无条件使用 coarse height，确保细边顶点与粗网格共线。GBuffer position varying 传递该 geomorph position 经 object-to-world 线性部分变换后的 translation-free world-space offset，其导数与绝对 world position 相同但不携带远原点平移。pixel shader 先安全单位化 `ddx` / `ddy` direction，再以二者叉积和无量纲相对角度阈值计算实际光栅三角形的 world-space 几何法线；变换后的 Terrain local +Y 同样先检查有限正 length squared 后单位化，非法时回退 world +Y，最终用于统一朝向和退化 fallback，禁止 `normalize(0)` / NaN。GBuffer 与 depth-only permutation 复用完全相同的高度/morph 路径，depth-only 不声明材质资源。
