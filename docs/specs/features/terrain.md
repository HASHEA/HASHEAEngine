...
# Feature Spec: Terrain Phase 1 Asset Core

## 当前范围

Phase 1 提供 Terrain 的纯 CPU 资产核心：网格与分块数据、不可变快照、非破坏编辑层、笔刷与 patch 回放、空间查询、`.AshTerrain` 容器、RAW/PNG/EXR 高度图导入导出，以及 `AssetDatabase` 发布与失效接口。该阶段没有把 Terrain 接入 Scene 实体、渲染器或 Editor。
...
- `TerrainComponent`、Scene JSON schema v6、Scene 世界变换适配、Scene 级拾取或角色贴地 adapter。
- Component 流送/淘汰、RenderAssetManager activity 集成、Terrain render proxy、LOD 绘制、shadow/GBuffer shader、GPU atlas 或双后端渲染资源。
...
Phase 1 不改变 shader、RHI 或 rendered frame data，因此不要求 RenderGate 或 PerfGate；进入 Terrain 渲染或性能阶段后必须按 `docs/VERIFY.md` 的对应行执行。
