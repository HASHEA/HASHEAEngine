...
Terrain authoring 的默认目标范围为 2048 × 2048 m，对应 2049 × 2049 个高度采样和 8 × 8 个 Component。Target Size X/Z 可独立输入 `[256, 8192]` m 内的整数，提交编辑或执行 Create/Import 前分别吸附到最近的 2 的幂，精确中点向上；每个 Component 固定 256 × 256 quad、257 × 257 个快照采样，采样间距固定 1 m。`TerrainGridLayout{}` 与 `make_default_terrain_grid_layout()` 为兼容既有资产和最大压力测试继续表示 8193 × 8193 / 32 × 32 的历史 full-pressure 布局，不再表示 Editor authoring 默认。
...
- GPU ownership 包含按实际 Component 数分配的全 Terrain packed-height `StorageBuffer`、dirty-weight staging `StorageBuffer`、两张固定 4144 × 4144 RGBA8 UAV/SRV atlas、按 `(component_count_x × 32 + 1, component_count_z × 32 + 1)` 分配的 coarse target、三张 1024 × 1024 × 8 fallback material arrays 及固定 256-slot 帧边界 metadata。高分辨率 atlas 不随 Terrain 总尺寸伸缩；真实材质层 texture cook/填充留给后续渲染 slice。
- 首次 load、同布局更新和布局替换都必须发布一个 snapshot/resources/bounds 一致的 view。布局变化或资产替换先验证完整、非空、row-major Component table，再在 candidate bundle 中分配并上传全部布局相关资源；只有完整成功才在帧边界原子替换 published view。任一 validation、allocation、height、coarse 或 initial-atlas 阶段失败都保留旧 published view 和可见画面，并返回包含资产路径、阶段、candidate layout、原因与 retained/cleared 结果的诊断。
- `RenderAssetManager::request_terrain_asset/finalize_pending_terrain_asset` 按规范化资产 key 只登记一次 pending owner；Ready/Failed 只结算一次，并把 Terrain failed key 与 activity epoch 合入通用 readiness。首次 load 在 published view 建立前保持 Pending 且不可见；同路径异步 load 或 candidate 失败时，只要旧 published view 仍有效就继续返回该资产供绘制，实际资产路径变化才清除旧 proxy。GPU finalize 必须运行在 render thread。
- `RenderTerrainProxy` 固定引用一个不可变 Terrain published snapshot generation 和对应 `TerrainRenderAsset` published view，从同一 view 的本地范围与 world transform 计算保守 AABB。transform-only 更新复制 proxy 后原子替换，不修改已发布帧；内容 revision 则重新解析 snapshot，并复用同一路径的 render asset cache 对象。
- `RenderScene::rebuild_terrains_from_scene` 消费 Scene extraction 并原子替换 terrain proxy 集合；`build_visible_render_frame` 对 published bounds 做 frustum AABB 裁剪，输出 `VisibleTerrainFrame`。`ScenePresentationSubsystem` 在构建 visible frame 前捕获 render asset epoch，并用该 epoch 判断整帧 current；构建期间发生 publication 变化时该帧不得伪装为 current。每帧 scene submission 还单独发布 Terrain content-ready 计数：plain smoke 必须等 resolve、height upload 与当前可见 atlas residency 收敛，不能因通用 capture warmup 仅限 dump 而在首次异步 load 的 0-draw 帧提前成功。topology rebuild 必须先于同帧 transform update，避免已删除实体的旧 proxy 造成伪失败。
...
- atlas compute 使用 8 × 8 thread group 覆盖 259 × 259 slot：内部 257 × 257 texel 和一圈 gutter 一次写入；高分辨率 slot 优先复用同坐标，其次使用空 slot，256 slot 已满时仍更新 coarse map。动态 coarse map 以 1/8 采样写入前四层权重，其范围为 `(component_count_x × 32 + 1) × (component_count_z × 32 + 1)`；非末端 Component 不拥有 +X/+Z 边界，末端 Component 写最终边界。只有 dispatch 成功后才提交 slot metadata 并消费该 pending upload；失败或被新 immutable generation 取代时保留/重建下一帧工作。Vulkan shader 编译器在 DXC rewriter 完成后，为行首 `RWTexture2D<unorm float4>` 补入 `vk::image_format("rgba8")`，使 SPIR-V storage image 格式与固定 RGBA8_UNORM atlas 一致；DX12 shader 文本和公共 RHI 契约不变，rewrite 版本进入 shader cache key。
...
- surface vertex shader 从全 Terrain packed R16 buffer 解码高度，用下一粗级的同三角形平面做 geomorph；较粗邻边无条件使用 coarse height，确保细边顶点与粗网格共线。shading normal 与 morph 几何有意解耦：每个顶点读取全分辨率 global height 的规范梯度；细 LOD 边遇粗邻居时，按粗边两个实际顶点的规范梯度插值，同 LOD seam、LOD 差 1 和双粗邻角点因此共享相同插值。VS 经 object-to-world 线性部分变换 tangent 后输出未归一化 world normal，PS 插值后安全单位化并统一朝向；`ddx/ddy` 只在法线退化时回退。全局外边界使用单边差分，morph 0/0.5/1 不改变 shading field，有限正非均匀缩放保持连续。GBuffer position varying 仍传递 translation-free geomorph world offset 供 fallback，避免远原点平移损失精度。GBuffer 与 depth-only permutation 复用完全相同的高度/morph 路径，depth-only 不声明材质资源。
...
- `SceneRenderer` 在既有 `SceneGBufferPass` 前准备 atlas compute，在同一 clear pass 内先画 static mesh、再画 Terrain，禁止增加第二次 GBuffer clear。方向光 sunlight CSM 与普通 directional shadow 共用组合 caster callback，先提交 static mesh、再提交所有允许的 Terrain shadow caster；`DynamicOnly` shadow cache 更新不重复画静态 Terrain。sunlight outer cache identity 绑定 scene runtime/content epoch，只枚举实际 Static/Stationary mesh caster 与 `casts_shadow=true` Terrain：mesh 覆盖 transform、section、GPU asset publication 及已准备 DepthOnly material publication；Terrain 以一次锁内的 accepted snapshot、generation/residency、pending/published/readiness 快照配合 visible snapshot 与 world transform。全体 primitive revision、Movable mesh 和不投影 Terrain 不进入 identity。static tile 只在 clear 与 `StaticOnly` Terrain/mesh draw 都成功录入 CPU command recording 后标记可复用；现有 RenderGraph 不提供 GPU completion 回执。
- `DirectionalShadowMask` 先以 clip-space quotient rule（含有限 `w²` guard）求 shadow NDC 的屏幕导数，再经 atlas Y flip/tile scale 求 receiver-plane depth gradient；每个 3×3 PCF tap 按自身 texel offset修正 comparison depth。非有限导数、相对退化 Jacobian 或过大 offset 都回退到原 constant-bias path，不增加 tap 数，也不靠放大全局 bias 掩盖条纹。
- Terrain surface 每个 LOD batch 的 `first_instance` 保持 0，真实实例基址由 root constants 传给 shader；240-byte `TerrainSurfaceConstants` 的末尾 `uint4` 携带实际 sample/component 布局。实例 StorageBuffer 使用 GPU-only 资源与既有 staging upload，并保留 3 帧物理 ring，避免 CPU 覆盖 GPU 仍在读取的数据。禁止把 storage/UAV usage 与 CPU-write upload heap 组合，因为该资源描述在 DX12 上非法。首期同一 scene/view 只消费第一个合法 1–32 × 1–32 矩形主 Terrain；多 Terrain 同帧独立材质/atlas 绑定尚未实现。
...
- `TerrainGate.AshTerrain` 由正式 Phase 1 writer 生成，显式使用 8193 × 8193 / 32 × 32 full-pressure layout，不依赖 authoring 默认；九个环形坡面写入 canonical Base，唯一固定 ID 的 weight edit layer 提供八个 one-hot 材质区域与一个四层混合区域，cooked height/weight/min-max 必须匹配独立全域 oracle 与正式 recomposition。固定 `lod_errors` 是经批准的唯一 gate-only metadata 例外，用于强制九个 LOD batch，并对全部 1024 个 Component 独立精确验证；普通 authoring 不采用该例外。schema 6 `Terrain.scene.json` 固定一个 Terrain、一个主相机和一个方向光，并关闭粒子、体积光、AO、Bloom 与 TAA。`.AshTerrain` writer version 2 / reader v1-v2 与 Scene schema v6 均未因可配置尺寸而改变。TerrainGate 只用于自动化，人工编辑验收必须从 Create Flat 新资产开始。
...
Terrain Mode 的 Create Flat 与 Import 共用 `Target Size X (m)` / `Target Size Z (m)` 文本草稿，初值均为 2048。编辑过程中保留原文本；严格十进制整数在 Enter、失焦和动作提交前分别规范化到 `[256, 8192]` 内最近的 2 的幂，精确中点向上，X/Z 独立。空值、符号、空白、尾随字符或溢出保持原文本并 fail closed，不得提交旧值。UI 同步显示推导出的 `Samples X × Z | Components X × Z | 1 m/sample`；Create 与 Import 必须从同一规范化 target 构建布局，Import 的 source width/height 仍是独立源尺寸。UI 另提供 height minimum、height maximum 与 flat height；有限且 `minimum < maximum` 时提交 `height_offset = minimum`、`height_range = maximum - minimum`。Import 继续提交源宽高、PNG / RAW R16 / RAW R32F / EXR 格式、RAW 大小端、独立 X/Z flip、精确 EXR channel，以及 Reject / Crop / Catmull-Rom。Export 使用另一组独立草稿提交 PNG / RAW R16 / RAW R32F / EXR、RAW byte order、精确 EXR channel 与 half/float，并支持最终合成高度、Base 高度、指定高度层和指定材质权重层；Import 与 Export 的路径、格式和 codec 字段不得共享 UI state。
...
- Component 实际流送策略/LRU、真实材质层 texture-array cook/填充，以及多 Terrain 同帧独立材质/atlas 绑定。当前首期渲染路径面向一个合法 1–32 × 1–32 矩形 Scene 主 Terrain。
