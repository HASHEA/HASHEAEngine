...
Phase 1 提供 Terrain 的纯 CPU 资产核心：网格与分块数据、不可变快照、非破坏编辑层、笔刷与 patch 回放、空间查询、`.AshTerrain` 容器、RAW/PNG/EXR 高度图导入导出，以及 `AssetDatabase` 发布与失效接口。Phase 2 已完成七个基础 slice：Scene v6 `TerrainComponent`、独立 revision/extraction 和 world-space CPU query adapter；Function-only 原生 2D texture-array wrapper；`TerrainRenderAsset` 的 content-generation 状态、不可变 Component pointer diff、GPU 资源 ownership 和 `RenderAssetManager` readiness/activity 接入；`RenderTerrainProxy`、`VisibleRenderFrame::terrains` 与 ScenePresentation 的独立 terrain revision 同步；纯 CPU Component quadtree culling、投影误差 LOD、邻接修复与稳定 draw batch 生成；dirty weight-atlas 的 raw upload、RenderGraph UAV→SRV 契约与 compute shader；以及 9 级共享 index grid、packed-height/morph surface shader 和固定 top-four 材质混合。Terrain 尚未接入 `SceneRenderer` 实际 draw，也未进入 Editor 工作流。
...
| `project/src/engine/Function/Render/TerrainRenderPass.h/.cpp`、`Shaders/Terrain/*` | persistent weight resources 的 graph 注册、单 Component raw staging、gutter/coarse compute 更新、9 级共享 grid 与 GBuffer/depth surface shader ownership |
...
- Component 实际流送策略/LRU、LOD batch 的实际 GPU 提交、真实材质层 texture-array cook/填充或双后端可见渲染。当前 atlas/surface resources 尚未由 `SceneRenderer` 调用，等 integration slice 接入既有 GBuffer/shadow pass 后才形成可见路径。
...
atlas/shared-surface slice 已加入 compute/graphics shader 与 RenderGraph 资源契约，但尚未由 `SceneRenderer` 调用；本 slice 以 headless graph compile、DXIL/SPIR-V 全 permutation 离线编译与 binding reflection、全量单测/构建和 ArchGate 为门禁。`SceneRenderer` 可见路径的 integration 提交必须执行完整双后端 readiness、RenderGate、validation 与 PerfGate。
