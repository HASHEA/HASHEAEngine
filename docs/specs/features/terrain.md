...
Phase 1 提供 Terrain 的纯 CPU 资产核心：网格与分块数据、不可变快照、非破坏编辑层、笔刷与 patch 回放、空间查询、`.AshTerrain` 容器、RAW/PNG/EXR 高度图导入导出，以及 `AssetDatabase` 发布与失效接口。Phase 2 的首个已完成 slice 进一步加入 Scene v6 `TerrainComponent`、独立 revision/extraction 和 world-space CPU query adapter；Terrain 仍未进入渲染器或 Editor 工作流。
...
当前已完成范围不改变 shader、RHI 或 rendered frame data，因此不要求 RenderGate 或 PerfGate；进入 Terrain 渲染或性能阶段后必须按 `docs/VERIFY.md` 的对应行执行。
