...
Phase 1 提供 Terrain 的纯 CPU 资产核心：网格与分块数据、不可变快照、非破坏编辑层、笔刷与 patch 回放、空间查询、`.AshTerrain` 容器、RAW/PNG/EXR 高度图导入导出，以及 `AssetDatabase` 发布与失效接口。Phase 2 已完成前三个基础 slice：Scene v6 `TerrainComponent`、独立 revision/extraction 和 world-space CPU query adapter；Function-only 原生 2D texture-array wrapper；以及 `TerrainRenderAsset` 的 content-generation 状态、不可变 Component pointer diff、GPU 资源 ownership 和 `RenderAssetManager` readiness/activity 接入。Terrain 尚未生成可见 proxy、LOD batch 或实际 draw，也未进入 Editor 工作流。
...
- Component 实际流送策略/LRU、Terrain render proxy、LOD 绘制、shadow/GBuffer shader、weight-atlas compute 写入、真实材质层 texture-array 填充或双后端可见渲染。
...
当前已完成范围只增加未被 draw path 消费的 Function render resources/状态，不改变 shader、RHI 公共接口或 rendered frame data，因此不要求 RenderGate 或 PerfGate；进入 atlas compute、Terrain shader/pass 或可见绘制后必须按 `docs/VERIFY.md` 的对应行执行。
