...
- Terrain Mode 的创建/导入/导出文件 job、viewport 笔刷路由与 overlay、保存/重载和外部冲突工作流。现有 Manage 页会显示状态与这些操作的后续边界，但在对应 service 契约完成前保持禁用。
