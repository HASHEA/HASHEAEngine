...
- `TerrainGate.AshTerrain` 由正式 Phase 1 writer 生成，使用 8193 × 8193 / 32 × 32 production layout；九个环形坡面写入 canonical Base，唯一固定 ID 的 weight edit layer提供八个 one-hot 材质区域与一个四层混合区域，cooked height/weight/min-max 必须匹配独立全域 oracle 与正式 recomposition。固定 `lod_errors` 是经批准的唯一 gate-only metadata 例外，用于强制九个 LOD batch，并对全部 1024 个 Component 独立精确验证；普通 authoring 不采用该例外。schema 6 `Terrain.scene.json` 固定一个 Terrain、一个主相机和一个方向光，并关闭粒子、体积光、AO、Bloom 与 TAA。TerrainGate 只用于自动化，人工编辑验收必须从 Create Flat 新资产开始。
