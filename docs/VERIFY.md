...
- 确定性保证：抓帧模式固定初始相机 (0, 5, 0)、隐藏引擎 overlay、禁用 TAA 亚像素抖动；抓帧需 5000 帧等资产流送完成（2000 帧实测 Vulkan 会抓到未加载完的空场景）
...
| RenderGate 流送等待 | 固定 `--smoke-test=5000` 帧按时间等资产流送 | 资产流送完成信号驱动抓帧，缩短耗时且消除时序脆弱性 |
