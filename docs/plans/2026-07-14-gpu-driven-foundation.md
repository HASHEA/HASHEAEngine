...
- [x] Step 1: Write pure descriptor/policy RED cases
...
- [x] Step 2: Implement shared validation and Renderer routing
...
- [x] Step 3: GREEN, full tests and backend builds
...
- [x] Step 1: Add the smallest RED contract check
...
- [x] Step 2: Set explicit NonIndexed
...
- [x] Step 1: Add RED lifecycle and graph-chain tests
...
- [x] Step 2: Implement without buffer copy
...
- [x] Step 4: Dual-backend self-test matrix
...
执行结论：Debug Vulkan 与 DX12 均确认 validation 已启用，raw RHI 与 Function RenderGraph 两段自测 PASS；Release Vulkan 与 DX12 两段自测也 PASS，但现有构建策略分别通过仅 Debug 定义的 `VULKAN_DEBUG_REPORT` 与非 `ASH_DEBUG` 强制关闭 DX12 validation，使 Release validation 不可用。该限制按既有构建能力缺口记录，未扩张本任务的 Graphics/构建策略范围，也未虚报 Release validation 已启用。
