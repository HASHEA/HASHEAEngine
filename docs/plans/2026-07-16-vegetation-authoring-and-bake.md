...
- Modify: `project/src/engine/Function/Asset/VegetationFileOps.h`
- Modify: `project/src/engine/Function/Asset/VegetationFileOps.cpp`
- Modify: `project/src/engine/Function/Asset/VegetationLayer.h`
- Modify: `project/src/engine/Function/Asset/VegetationLayer.cpp`
...
- Modify: `project/src/tests/Vegetation/vegetation_brush_tests.cpp`
- Modify: `project/src/tests/Vegetation/vegetation_storage_tests.cpp`
- Modify: `project/src/tests/Vegetation/VegetationTestSupport.h`
- Modify: `README.md`
...
- [x] **Step 1: Write deterministic bake and transaction RED cases**
...
- [x] **Step 3: Implement pure bake and checked pointer publication**
...
`VegetationPreparedChunkSet` captures the validated layer/store target, operation identity and exact source-active identity from the read snapshot. For Existing it also binds the native file identity from the stable post-stage-sibling reinspection; NoActive binds an unavailable all-zero file identity. Main-thread commit accepts only `IVegetationCommitFileOps`, revalidates the bound target, compares `current_operation_identity`, then uses the supplied operation control to acquire the chunk-store lease with the same cancel polling and absolute deadline contract as Layer commit. It checks control before lease, immediately after acquisition, after bounded on-disk `active.asva` reread/identity validation, and immediately before pointer publish. The active native identity must still equal the captured source identity before bytes are read, and the reread manifest identity must also match before staging the pointer; same-byte delete/recreate ABA is `SourceChanged` with zero publication calls. If both captured and on-disk identity are `NoActive`, commit must use `CreateNewFromStage`; `AlreadyExists` means a cooperative writer won and returns the explicit same-name status without touching that pointer, while I/O `Failed` remains a controlled failure. Only an existing on-disk active identity that exactly matches the captured existing identity may use `AtomicReplace`. `AtomicReplace` is never create-or-replace. A caller-cached current pointer is not accepted as disk evidence. Cancellation is checked around each batch, chunk, and <=1 MiB write block.

Both create-new and replace publication treat a completed filesystem mutation as terminal even if the injected FileOps throws before returning. Unknown outcomes are reconciled only from physical evidence: the prepared stage must be absent, the target native identity must equal the captured stage identity, and the target bytes must be the exact staged canonical payload with matching digest/manifest identity. Same bytes under a different native file identity never prove this operation published them. Recovery is likewise provenance-bound: an exact prepared stage must retain its captured native identity and staged bytes; a distinct replace backup must be a normalized same-directory protected artifact and decode to the exact old Layer revision or source-active ASVA. Ordinary FileOps/resolver exceptions are contained as typed failures and never publish partial active snapshots. Post-publication reconciliation and fallback-stage proof are themselves no-throw boundaries: if inspection/readback throws, the commit returns controlled `Failed`, leaves an unresolved `Publishing` pin protected, and never reports an unverified recovery path.

- [x] **Step 4: Run GREEN and byte-determinism regression**
...
RunTests.bat Release --test-case="Vegetation storage*"
RunArchGate.bat
git diff --check -- README.md docs/sdd/SDD-2026-07-16-vegetation-authoring-and-bake.md docs/specs/modules/asset.md docs/plans/2026-07-16-vegetation-authoring-and-bake.md project/src/engine/Function/Asset/VegetationBaker.h project/src/engine/Function/Asset/VegetationBaker.cpp project/src/engine/Function/Asset/VegetationChunkSet.h project/src/engine/Function/Asset/VegetationChunkSet.cpp project/src/engine/Function/Asset/VegetationFileOps.h project/src/engine/Function/Asset/VegetationFileOps.cpp project/src/engine/Function/Asset/VegetationLayer.h project/src/engine/Function/Asset/VegetationLayer.cpp project/src/engine/Function/Asset/VegetationChunk.h project/src/engine/Function/Asset/VegetationChunk.cpp project/src/engine/Function/Asset/VegetationStorage.h project/src/engine/Function/Asset/VegetationStorage.cpp project/src/tests/Vegetation/VegetationTestSupport.h project/src/tests/Vegetation/vegetation_baker_tests.cpp project/src/tests/Vegetation/vegetation_brush_tests.cpp project/src/tests/Vegetation/vegetation_storage_tests.cpp
powershell -NoProfile -Command "$untrackedTask8 = @('project/src/engine/Function/Asset/VegetationBaker.h', 'project/src/engine/Function/Asset/VegetationBaker.cpp', 'project/src/engine/Function/Asset/VegetationChunkSet.h', 'project/src/engine/Function/Asset/VegetationChunkSet.cpp', 'project/src/tests/Vegetation/vegetation_baker_tests.cpp'); foreach ($path in $untrackedTask8) { $checkOutput = & git diff --no-index --check -- NUL $path 2>&1; if ($checkOutput) { $checkOutput; exit 1 } }; exit 0"
```
...
```bat
git add -- README.md docs/sdd/SDD-2026-07-16-vegetation-authoring-and-bake.md docs/specs/modules/asset.md docs/plans/2026-07-16-vegetation-authoring-and-bake.md project/src/engine/Function/Asset/VegetationBaker.h project/src/engine/Function/Asset/VegetationBaker.cpp project/src/engine/Function/Asset/VegetationChunkSet.h project/src/engine/Function/Asset/VegetationChunkSet.cpp project/src/engine/Function/Asset/VegetationFileOps.h project/src/engine/Function/Asset/VegetationFileOps.cpp project/src/engine/Function/Asset/VegetationLayer.h project/src/engine/Function/Asset/VegetationLayer.cpp project/src/engine/Function/Asset/VegetationChunk.h project/src/engine/Function/Asset/VegetationChunk.cpp project/src/engine/Function/Asset/VegetationStorage.h project/src/engine/Function/Asset/VegetationStorage.cpp project/src/tests/Vegetation/VegetationTestSupport.h project/src/tests/Vegetation/vegetation_baker_tests.cpp project/src/tests/Vegetation/vegetation_brush_tests.cpp project/src/tests/Vegetation/vegetation_storage_tests.cpp
git commit -m "feat(vegetation): bake deterministic chunk sets"
```

Checkpoint: deterministic ASVI/ASVC/ASVM/ASVA baking, checked dirty-union expansion, native-identity ABA rejection, provenance-pinned recovery, and no-throw publication reconciliation are implemented. Fresh generation, Task 8 focused Debug/Release tests, storage/codec/brush/patch regressions, ArchGate, and AIDevDoctor validation all pass. Independent specification and code-quality reviews report P0=P1=P2=P3=0; the selective Task 8 commit excludes the two Tracy LFS worktree artifacts.
...
- [x] **Step 1: Write deterministic service RED cases**
...
TEST_CASE("Vegetation service without a provider keeps palette and persistence but disables mutation")
{
    ...
    CHECK_FALSE(service.RequestBake(AshEngine::VegetationSurfaceBinding{ 42 }, t0));
...
Add RED cases for zero bake binding rejection before provider capture with zero side effects; exact nonzero bake binding delivery; exact binding value reuse across every `Pending` retry even when the caller's source value or unrelated stroke/selection-like test state changes; `Ready` after retry; permanent `Pending`; `Failed`, throw, malformed capture and malformed sample; active-stroke/save/bake/reload conflict with zero side effects; `Dirty` ordinary Reload rejection; explicit confirmed `ReloadDiscard`; save source revision conflict; layer generation, surface identity/revisions, and species digest staleness; Create Layer rejecting existing/escaping paths and injected zero/colliding IDs; the target absent at Create Layer but created externally before the first Save commit, which must preserve external bytes, return `AlreadyExists`, keep the session Dirty with no observed revision, and remove/track only its owned stage; palette Add/Replace/Remove each producing one reversible command, updating the immutable view and generation once, undo/redo restoring exact canonical authoring payload bytes while generation remains monotonic, Remove confirmation clearing weights, and Save -> Reload retaining the edit without any provider; save completion advancing persisted generation only after checked main-thread commit, proven indirectly because the target remains unchanged after worker completion and changes only after checked `Tick` commit while later capabilities/save/reload behavior follows the committed session; bake completion switching `active.asva` only after checked main-thread commit; working-set before/after dirty evidence surviving Undo/Redo/Save and every failed/stale/cancelled bake, then clearing only after the matching active-pointer commit; session replacement conservatively choosing full dirty when active Layer generation mismatches and no complete same-session evidence remains, using a generation-2 Layer with a valid Species plus nonzero Density/SpeciesWeight and asserting provider sampling, the expected chunk coordinate/manifest Species reference, and active generation 2; cooperative cancellation during sampling and stage writes; shutdown acknowledgement, stage cleanup and no pointer switch; and executor destruction after service shutdown. Stale operation serial uses structural invariant proof rather than a test backdoor: the public API permits only one active operation, serial increments only on successful schedule, active conflicts reject all new schedules, and `ClearAsync` follows exact completion observation and `Join`, so a legal executor cannot deliver an old completion after a newer operation. A reject-once executor plus FileOps serial recorder proves failed Save/Bake task submission does not consume a serial; validate the defensive commit branch further through the active-conflict suite, exact-join/executor-lifetime tests, and code review of schedule/check/clear ordering. Do not add a private-state getter/friend or violate the executor contract to fabricate an old completion. The executor RED suite separately covers cancel-before-run, cancel-while-running, completion observation, exact join, and shutdown with a barrier-controlled production task. Tests assert exact state/reason strings and use explicit `Tick(now)` rather than wall-clock waits.
...
- [x] **Step 3: Implement the minimum orchestration API**
...
public:
    ...
    bool RequestReloadDiscard(bool confirmed,
                              std::chrono::steady_clock::time_point now);
    bool RequestBake(const AshEngine::VegetationSurfaceBinding& binding,
                     std::chrono::steady_clock::time_point now);
...
The executor owns every worker thread and outlives the service. `Submit` creates the shared atomic cancellation state and passes the resulting `VegetationOperationControl` plus the absolute deadline into the work item; cancellation is therefore observable inside provider sampling and every file-write block. Every production thread entry catches all exceptions, stores `std::exception_ptr`, marks complete, and remains joinable; the service observes completion and exception before `Join`, then `Join` waits and erases that exact Task state. Shutdown follows the same sequence: request cancellation, wait for completion acknowledgement, observe the exception, and only then exact-Join; a teardown-time throw becomes `Failed` rather than being silently consumed. A consumed ID is absent, repeated Join is a no-op, and `CancelAndJoinAll` gathers IDs then uses the same erasing Join path, so completed history cannot grow without bound. Production uses one controlled thread per task and does not add a dispatcher, FIFO, or worker queue. Its barrier test proves cancellation after the task thread enters but before any semantic provider/file/business side effect, plus running cancellation acknowledgement, exact join, `CancelAndJoinAll`, a throwing work item becoming complete/failed/idle without `std::terminate`, and 256 sequential completed tasks being absent after Join. `ManualVegetationEditorTaskExecutor` lives only in test support and deterministically exposes `RunNext`/`RunAll`; genuine cancel-before-run completes the record as cancelled without invoking its body, and the seam implements the same observe-before-Join, exact record/order consumption, repeated-Join no-op, and snapshot-then-consume `CancelAndJoinAll` semantics. Its 256-task case proves neither task records nor scheduling order accumulate, while `Tick(now)` service tests remain sleep-free. Manual and production throwing adapters separately lock the shutdown observe-before-Join behavior.

The operation serial is a defensive checked-commit guard behind a stronger structural invariant: the service has at most one active async operation, advances serial only after a schedule is accepted, rejects every competing schedule while active, and calls `ClearAsync` only after observing and joining the exact task. Consequently no legal public-API/executor trace can deliver an old serial completion after a newer operation. Task 9 verifies this by the conflict matrix, exact-join and executor-lifetime tests, plus review of the serial schedule/check/clear sites; it does not expose private serial state or fabricate an executor-contract violation. Likewise `persisted_generation` remains private and is verified through target bytes before worker completion consumption versus after checked main-thread commit, followed by public save/reload/capability behavior.

`RequestBake(binding, now)` rejects `binding.surface_entity_id == 0` as its first check, before service/provider/capability lookup, retry-state mutation, or worker scheduling; this remains a zero-side-effect pure rejection even while another operation is active or no provider is installed. Nonzero active-operation conflicts are also zero-side-effect rejections. A newly accepted request freezes by value its binding, immutable Layer/generation, dirty evidence, source revision plus canonical/resolved/native identity, resolver snapshot, and canonical Species set. `BakeRequestContext::layer` is the single frozen palette owner shared by worker capture and completion-time palette identity comparison; there is no duplicate service `captured_palette`. The initial call and every due retry in `Tick(now)` call `capture_vegetation_surface` synchronously on the Editor/logic thread with that exact captured binding; a `Pending` retry may replace only the surface snapshot and never recaptures working-set, palette, source, resolver, stroke, selection, Scene, or UI state. `Pending` records only the next retry time and schedules no executor work; `Failed`/malformed/throw terminates without a worker. A wrapper-validated `Ready+snapshot` must additionally report a nonzero captured `surface_id`; all-zero identity fails before active-read, stage creation, or `Submit`, clears the frozen request, and a later request captures only its new binding/context. `ClearBakeRequest()` is the single release path for the frozen Layer/Species/resolver/palette record and retry/deadline/binding fields; every no-worker terminal path (capture failure/throw/malformed result, serial or identity failure, rejected submission, retry exhaustion, and absolute deadline in `Tick`) invokes it, while a scheduled worker keeps the record until joined completion. The large-palette lifecycle case proves a terminal request releases the prior frozen record before the next bake request.

Workers capture immutable Layer/Species/surface/active-store/resolver snapshots, operation control, paths, serials, value results, and an independently lifetime-owned cleanup registry only; they never call provider `capture`, and never capture `this`, Scene, selection, UI, or mutable AssetDatabase catalog internals. A null `pFileOps` resolves once to `get_default_vegetation_file_ops()`; tests inject `ScriptedVegetationFileOps`, and every Layer/Chunk operation receives that same reference. All service-owned `InspectPath` consumers, including indirect Layer rereads, are behind no-throw wrappers; exceptions become controlled failure/`SourceChanged`, exact-owned cleanup still runs, and no exception crosses Create/Open/Request/`Tick`. The service creates one `std::shared_ptr<VegetationOwnedStageCleanupRegistry>` and workers capture that shared pointer by value rather than a service subobject; `Track*` is internally synchronized. Every joined completion path, including success, stale, controlled failure, exception, cancellation and timeout, invokes `RetryAll` before async state is cleared. If exact-owned cleanup remains retained, service status is `Failed`, every capability is false and every real intent is rejected until an Editor-thread idle retry proves the registry empty. Pending retry times are exactly `50,100,200,400,800,1000,1000,1000 ms`, at most eight retries and never after the absolute 30-second operation deadline. Timeout tests pass an already-expired explicit request time and never sleep or add a production clock seam. Shutdown first requests cancellation, then waits for acknowledgement/join, retries exact cleanup, and discards completion without switching a pointer. A trusted provider that violates its 50 ms cooperative contract is logged, but C++17 teardown still waits and does not detach or forcibly kill it.

At `RequestBake(binding, now)` the frozen request record supplies the immutable bake inputs. Failed, cancelled, timed-out, stale, or publication-fault paths retain its dirty evidence. After worker completion, the logic thread rereads and strict-decodes the source Layer and compares request-entry revision, canonical path/identity, resolved absolute path and native file identity; in-place modification, same-byte delete/recreate and deletion all become `SourceChanged` with no active switch. It then independently reconstructs `VegetationChunkSetExpectedIdentity` from the current working Layer/generation, current wrapper-validated surface, current palette canonical Species digests and this operation's target/store identity; it never passes `prepared.expected_identity()` back as current state. Only after that checked pointer commit succeeds may the service call `acknowledge_bake_dirty_evidence(captured_generation)`; any later edits make acknowledgement fail without clearing newer evidence. Every virtual surface `identity()` read is contained by a no-throw helper. Open/reload/session replacement discards the old working set, and the active-manifest versus opened-Layer generation rule then supplies the restart full-dirty fallback.

Reload captures the working generation when scheduled, checks the same cancel/deadline control immediately before and after its synchronous file read, and requires the generation still match at `Tick` publication. Thus clean reload and confirmed discard both preserve a newer Undo/Redo/edit and return `SourceChanged`; read-phase cancellation and expiry remain exactly `Cancelled/TimedOut`. Bake active-read and sampling cancellation/expiry likewise keep those exact terminal states rather than collapsing to `Failed`. The first-Save race test inserts an external target after worker `Prepare` and before `Tick`, proving create-new preserves the external bytes. The Copy As test then edits and saves the original source and proves the destination remains at its copied bytes, locking the no-rebind contract.

Before every public intent/getter, the service synchronizes the current working-set publication: a generation different from `persisted_generation` immediately changes Clean to Dirty without waiting for `Tick`. It first compares the O(1) working-set generation cursor and returns before `publish_snapshot()` or palette path scanning on a stable frame. Only a changed generation compares the ordered palette `{path,id,digest}` identity against the cached immutable view; authoring-only stroke/Undo/Redo generations reuse the view without typed load/canonical encode/hash, while a palette identity change rebuilds it. This closes the Undo/Redo-to-next-Tick overwrite window for Reload/Open/Create and keeps a palette Undo current even when the first observer is a getter. A repeated-getter fixture asserts stable view identity and zero new file/task/command work. For palette and stroke mutations, `RecordExecutedCommand` is handled as a tri-state: `Recorded` is Dirty+success; `RolledBack` synchronizes the restored view/content, preserves the pre-mutation Clean/Dirty semantic while generation remains monotonic, and reports a controlled failure; `RollbackFailed` is conservative Dirty+failure and cannot be overwritten by Reload/Open.

`BeginStroke` requires `stroke_without_path.path.empty()`, captures through `capture_vegetation_surface`, accepts only `Ready+snapshot`, and stores that immutable snapshot and full identity for the stroke. The service appends only canonical surface requests supplied through `AppendStrokePoint`. `EndStroke` creates exactly one `shared_ptr<const StrokeContext>` immutable worker snapshot; the closure copies only that pointer, while completion reads the service-owned optional context by const reference rather than copying the complete path again. One controlled worker operation validates the complete path through `sample_vegetation_surface_batch` in stable batches of at most 4096; a 4097-request test compares every request field after reconstructing the two exact batches. It accepts only per-sample `Ready` and treats `Outside` as whole-stroke failure because Editor hit requests are required to be on the explicitly bound surface. It publishes no partial samples. On completion the Editor thread re-captures through the wrapper and requires the exact stored surface identity/revisions before it calls the pure Task 5 brush API; only a successful non-no-op patch is applied and passed once to `RecordExecutedCommand`. `Pending` is not retried for strokes. All other sampling/capture/cancel/deadline/stale paths discard the pending stroke with generation and history unchanged. `CreateLayer` validates an asset-root-relative path, confirms absence through the injected FileOps, and opens an unsaved Dirty session with `expected_revision=nullopt`; it performs no target write. The first `RequestSave` stages on a worker and commits with atomic create-new, so a racing creator is preserved and the session remains dirty with `AlreadyExists`. The Layer obtains a nonzero 128-bit identity from the injected generator in tests or exactly two values from an OS-backed `std::independent_bits_engine<std::random_device,64,uint64_t>` per candidate in production, expands them into the 16-byte ID in explicit little-endian word order, retries zero/current-ID collisions, and never overwrites an existing file. This entropy creates asset identity only and is never read by the deterministic baker. Palette Add/Replace/Remove first resolve the Species through the typed AssetDatabase path, then call the Task 5 palette patch API and record exactly one already-executed document command. Successful edits update the immutable palette view, advance generation once, mark the session dirty, and remain usable without a surface provider; exact canonical authoring bytes round-trip through Add/Replace Undo/Redo while content generation remains monotonic. Remove requires the explicit clear-weights confirmation when referenced. Save/reload round-trip palette order, IDs, digests, paths, and cleared planes.

`DefaultLoadBudget()` is the explicit resident-policy default, not a wire/world cap: `max_file_bytes=256 MiB`, `max_payload_bytes=256 MiB`, `max_decoded_bytes=1 GiB`, `max_palette_records=65534`, `max_tile_records=262144`, and `max_instance_records=8388608`. `DefaultChunkSetLoadBudget()` uses that per-file budget plus `max_manifest_entries=262144`, `max_total_inspected_bytes=2 GiB`, and `max_summary_bytes=64 MiB`. Task 9 tests every exact field, mutates every field of one returned value, and proves both a separately obtained value and a fresh call retain the exact defaults; actual `EditorApplicationImpl` bootstrap construction/injection remains Task 11 because Phase 2 product bootstrap still has no provider or service caller. Callers/tests may inject smaller or larger resident policies without changing wire/world limits.

- [x] **Step 4: Run GREEN and lifecycle regression**
...
build_editor.bat Debug
RunArchGate.bat
git diff --check -- docs/sdd/SDD-2026-07-16-vegetation-authoring-and-bake.md docs/specs/modules/asset.md docs/specs/modules/editor.md docs/plans/2026-07-16-vegetation-authoring-and-bake.md project/src/editor/Services/VegetationEditorTaskExecutor.h project/src/editor/Services/VegetationEditorTaskExecutor.cpp project/src/editor/Services/VegetationEditorService.h project/src/editor/Services/VegetationEditorService.cpp project/src/tests/Vegetation/VegetationTestSupport.h project/src/tests/Editor/vegetation_editor_service_tests.cpp project/src/tests/premake5.lua
```
...
```bat
git add -- docs/sdd/SDD-2026-07-16-vegetation-authoring-and-bake.md docs/specs/modules/asset.md docs/specs/modules/editor.md docs/plans/2026-07-16-vegetation-authoring-and-bake.md project/src/editor/Services/VegetationEditorTaskExecutor.h project/src/editor/Services/VegetationEditorTaskExecutor.cpp project/src/editor/Services/VegetationEditorService.h project/src/editor/Services/VegetationEditorService.cpp project/src/tests/Vegetation/VegetationTestSupport.h project/src/tests/Editor/vegetation_editor_service_tests.cpp project/src/tests/premake5.lua
git commit -m "feat(editor): orchestrate vegetation authoring"
```

The behavior/interface tests, SDD, long-lived Asset/Editor specs, and this Task 9 plan update are one commit. The two pre-existing Tracy LFS worktree files are explicitly excluded. Independent specification and code-quality reviews report P0=P1=P2=P3=0, and the final reviewed SDD lock is recorded above.
...
TEST_CASE("Vegetation Phase 2 authoring and deterministic bake exit contract")
{
    ...
    CHECK_FALSE(VegetationTest::CompleteBake(
        service, executor, provider, binding, false).published);
...
Add `EditorPanelIds::Vegetation = "vegetation"` and `EditorWindowTitles::Vegetation = "Vegetation"`. Add a service pointer to `PanelBootstrapContext`, construct one panel through `PanelManager`, and pass only `VegetationPanelDeps`. The panel uses `UIContext` through `EditorFrameContext`; it never includes Graphics, backend, Scene internals, or Terrain internals. It displays the immutable palette view and exposes New Layer, Load, Add/Replace/Remove Species, Save and Reload intents even when no provider exists. Removing a referenced Species requires an explicit clear-weights confirmation. Paint/Erase/Bake controls are disabled with the exact capability reason when no provider is injected. A future enabled Bake intent must carry the nonzero `VegetationSurfaceBinding` resolved for that exact user action and call `RequestBake(binding, now)` directly; the panel/service must not cache it as a session default or fall back to the last stroke/selection. Phase 2's null-provider bootstrap never calls `RequestBake` and never fabricates a placeholder binding.
