...
namespace AshEngine
{
	...
	auto Application::start() -> bool
	{
		...
		while (!_should_exit())
		{
			...
			if (_should_render_frame())
			{
				...
				else
				{
					...
					if (frameRendered && renderer && perfGateController.is_enabled() && perfGateSamplingStarted)
					{
			...
			if (perfGateController.is_enabled() && !perfGateSamplingStarted)
			{
				...
				if (perfReadinessDecision.succeeded)
				{
					perfGateController.begin();
					perfGateSamplingStarted = true;
		...
		if (perfGateController.is_enabled())
		{
			const bool perfGateCompleted =
				perfGateSamplingStarted && perfGateController.should_request_exit();
			perfGateAbnormalExit = !perfGateCompleted ||
				runtimeFailureDetected.load(std::memory_order_acquire) ||
				logicThreadFailed.load(std::memory_order_acquire);
			if (!perfGateCompleted)
			{
				if (!runtimeFailureDetected.load(std::memory_order_acquire))
				{
					HLogError("PerfGate terminated before readiness, sampling, and GPU timing drain completed.");
				}
				runtimeFailureDetected.store(true, std::memory_order_release);
			}
		}
