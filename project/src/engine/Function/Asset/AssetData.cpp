...
namespace AshEngine
{
	namespace
	{
		...
		static auto optimize_mesh_for_gpu(Mesh& mesh) -> void;
		...
		static auto optimize_mesh_section_for_gpu(Mesh& mesh, MeshSection& section) -> void
		{
			if (section.topology != MeshPrimitiveTopology::Triangles ||
				section.vertex_count == 0 ||
				section.index_count < 3 ||
				section.index_offset + section.index_count > mesh.indices.size() ||
				section.vertex_offset + section.vertex_count > mesh.vertices.size())
			{
				return;
			}

			std::vector<uint32_t> local_indices(section.index_count);
			for (uint32_t index = 0; index < section.index_count; ++index)
			{
				const uint32_t absolute_index = mesh.indices[section.index_offset + index];
				if (absolute_index < section.vertex_offset ||
					absolute_index >= section.vertex_offset + section.vertex_count)
				{
					return;
				}
				local_indices[index] = absolute_index - section.vertex_offset;
			}

			std::vector<uint32_t> optimized_indices(local_indices.size());
			...
			std::vector<uint32_t> overdraw_indices(local_indices.size());
			const MeshVertex* local_vertices = mesh.vertices.data() + section.vertex_offset;
			meshopt_optimizeOverdraw(
				overdraw_indices.data(),
				optimized_indices.data(),
				optimized_indices.size(),
				reinterpret_cast<const float*>(&local_vertices[0].position),
				section.vertex_count,
				sizeof(MeshVertex),
				1.05f);

			std::vector<MeshVertex> optimized_vertices(section.vertex_count);
			meshopt_optimizeVertexFetch(
				optimized_vertices.data(),
				overdraw_indices.data(),
				overdraw_indices.size(),
				local_vertices,
				section.vertex_count,
				sizeof(MeshVertex));

			std::copy(
				optimized_vertices.begin(),
				optimized_vertices.end(),
				mesh.vertices.begin() + section.vertex_offset);
			for (uint32_t index = 0; index < section.index_count; ++index)
			{
				mesh.indices[section.index_offset + index] = section.vertex_offset + overdraw_indices[index];
			}
		}
		...
		static auto optimize_mesh_for_gpu(Mesh& mesh) -> void
		{
			...
			{
				optimize_mesh_section_for_gpu(mesh, section);
			}
		}
