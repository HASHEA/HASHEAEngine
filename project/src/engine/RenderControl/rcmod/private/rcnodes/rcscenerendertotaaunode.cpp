...
#include "rcscenerendertotaaunode.h"

RC_NAMESPACE_BEGIN

RCEffectNode*    RCSceneRenderToTAAUNode::Create(RCEffect* i_pEffect)
{
    return DOME_New(RCSceneRenderToTAAUNode)(i_pEffect);
}

DResult          RCSceneRenderToTAAUNode::Destroy(RCEffectNode* i_pEffectNode)
{
    DOME_Del(i_pEffectNode);
    return R_SUCCESS;
}

RCSceneRenderToTAAUNode::RCSceneRenderToTAAUNode(RCEffect* i_pEffect)
...
RCSceneRenderToTAAUNode::~RCSceneRenderToTAAUNode()
{

}

DResult         RCSceneRenderToTAAUNode::buildMDEffect(MDEffect* o_pStack, Int i_OutputSelector)
{
    const static DStringHash k_MDOperator_MDSceneRender("MDSceneRenderToTAAU");
	...
	executePushInput(o_pStack, 3); // screen stretch rate
...
void            RCSceneRenderToTAAUNode::finishLoad()
{
