...
#include "rcscenerendertonode.h"

RC_NAMESPACE_BEGIN

RCEffectNode*    RCSceneRenderToNode::Create(RCEffect* i_pEffect)
{
    return DOME_New(RCSceneRenderToNode)(i_pEffect);
}

DResult          RCSceneRenderToNode::Destroy(RCEffectNode* i_pEffectNode)
{
    DOME_Del(i_pEffectNode);
    return R_SUCCESS;
}

RCSceneRenderToNode::RCSceneRenderToNode(RCEffect* i_pEffect)
...
RCSceneRenderToNode::~RCSceneRenderToNode()
{

}

DResult         RCSceneRenderToNode::buildMDEffect(MDEffect* o_pStack, Int i_OutputSelector)
{
    const static DStringHash k_MDOperator_MDSceneRender("MDSceneRenderTo");
...
void            RCSceneRenderToNode::finishLoad()
{
