...
class RCGBuffer4Node : public RCEffectNode
{
public:
    static RCEffectNode*    Create(RCEffect* i_pEffect);
    static DResult          Destroy(RCEffectNode* i_pEffectNode);

    RCGBuffer4Node(RCEffect* i_pEffect);
    virtual ~RCGBuffer4Node();
...
private:
    MDOperandValuePtr       m_GBuffer4Texture;
};
