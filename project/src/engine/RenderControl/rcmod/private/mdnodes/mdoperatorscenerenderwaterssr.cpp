#include "pch.h"
/*
filename:       mdoperatorscenerenderwaterssr.cpp
author:         Bin Yang
date:           2017-Oct-24
description:
*/

#include "mdoperatorscenerenderwaterssr.h"
...
MDOperatorSceneRenderWaterSSR::MDOperatorSceneRenderWaterSSR()
	: m_OperatorName("MDSceneRenderWaterSSR")
{
...
****************************/
const DString&      MDOperatorSceneRenderWaterSSR::getOperatorName() const
{
	return m_OperatorName;
}

Bool                MDOperatorSceneRenderWaterSSR::isGpuOperator() const
{
	return DM_FALSE;
}

Int                 MDOperatorSceneRenderWaterSSR::getInputCount() const
{
	return nParamCount;
}

DSimpleTypeID       MDOperatorSceneRenderWaterSSR::getInputTypeID(Int i_Index) const
{
	return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

Int                 MDOperatorSceneRenderWaterSSR::getOutputCount() const
{
	return 1;
}

DSimpleTypeID       MDOperatorSceneRenderWaterSSR::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
	return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorSceneRenderWaterSSR::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
...
DResult             MDOperatorSceneRenderWaterSSR::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorSceneRenderWaterSSR::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
...
DResult             MDOperatorSceneRenderWaterSSR::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
	return R_SUCCESS;
}
