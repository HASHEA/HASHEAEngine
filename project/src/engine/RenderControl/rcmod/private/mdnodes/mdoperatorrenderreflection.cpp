...
#include "mdoperatorrenderreflection.h"
...
MDOperatorRenderReflection::MDOperatorRenderReflection()
    : m_OperatorName("MDRenderReflection")
{
...
****************************/
const DString&      MDOperatorRenderReflection::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRenderReflection::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRenderReflection::getInputCount() const
{
    return s_nInputCount;
}

DSimpleTypeID       MDOperatorRenderReflection::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorRenderReflection::getOutputCount() const
{
    return 2;
}

DSimpleTypeID       MDOperatorRenderReflection::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_nInputCount);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorRenderReflection::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_nInputCount);
...
DResult             MDOperatorRenderReflection::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
	DOME_ASSERT(i_pParamList[1] && i_pParamList[1]->isTexture());
    ...
    if (i_Index == 0)
    {
        ...
        l_Result = i_pParamList[6]->getTextureSize(o_Size);
        if(DM_SUCC(l_Result))
            return l_Result;
    ...
    {
        return i_pParamList[6]->getTextureSize(o_Size);
    }
    else
    {
        DOME_ASSERT(0);
...
****************************/
MDOperandPtr        MDOperatorRenderReflection::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	FRAMETIMER_BEGIN(FTT_RC_CAL_RENDERREFLECTION, FTT_RC_CAL_EXECUTE_EFFECTPASS);
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
	DOME_ASSERT(i_pParamList[1] && i_pParamList[1]->isTexture());
	DOME_ASSERT(i_pParamList[2] && i_pParamList[2]->isTexture());
...
#define GetTexture(name, index) \
	...
	*((OSTexture2D*)l_##name.getPtr()) = l_p##name->getValue<OSTexture2D>();
	...
	l_pScenePlugin->RenderReflection(
	...
	FRAMETIMER_END(FTT_RC_CAL_RENDERREFLECTION);

    return l_pOperand;
}

DResult             MDOperatorRenderReflection::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
