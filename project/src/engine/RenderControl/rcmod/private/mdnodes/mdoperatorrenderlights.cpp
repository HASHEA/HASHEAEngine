...
#include "mdoperatorrenderlights.h"
...
MDOperatorRenderLights::MDOperatorRenderLights()
    : m_OperatorName("MDRenderLights")
{
...
****************************/
const DString&      MDOperatorRenderLights::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRenderLights::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRenderLights::getInputCount() const
{
    return s_nInputCount;
}

DSimpleTypeID       MDOperatorRenderLights::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorRenderLights::getOutputCount() const
{
    return 2;
}

DSimpleTypeID       MDOperatorRenderLights::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_nInputCount);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorRenderLights::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_nInputCount);
...
DResult             MDOperatorRenderLights::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
	DOME_ASSERT(i_pParamList[1] && i_pParamList[1]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorRenderLights::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
...
DResult             MDOperatorRenderLights::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
