#include "pch.h"
/*
    filename:       mdoperatorcopyrectto.cpp
    author:         Ming Dong
    date:           2016-JUN-28
    description:    
*/

#include "mdoperatorcopyrectto.h"
...
MDOperatorCopyRectTo::MDOperatorCopyRectTo()
    : m_OperatorName("MDCopyRectTo")
{
...
****************************/
const DString&      MDOperatorCopyRectTo::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorCopyRectTo::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorCopyRectTo::getInputCount() const
{
    return 6;
}

DSimpleTypeID       MDOperatorCopyRectTo::getInputTypeID(Int i_Index) const
{
    switch (i_Index)
    {
    case 0:
...
Int                 MDOperatorCopyRectTo::getOutputCount() const
{
    return 1;
}

DSimpleTypeID       MDOperatorCopyRectTo::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_Index == 0);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorCopyRectTo::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_Index == 0);
...
DResult             MDOperatorCopyRectTo::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_Index == 0);
    DOME_ASSERT(i_ParamCount == 6);
...
****************************/
MDOperandPtr        MDOperatorCopyRectTo::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
    PERF_COUNTER_EX(0);
	FRAMETIMER_BEGIN(FTT_RC_CAL_COPYRECTTO, FTT_RC_CAL_EXECUTE_EFFECTPASS);
    DResult l_Result;
    DOME_ASSERT(i_ParamCount == 6);
    DOME_ASSERT(i_pParamList[0]->isTexture());      // source texture
    DOME_ASSERT(i_pParamList[1]->isTexture());      // render target texture
    ...
    DOME_ASSERT(DM_SUCC(l_Result));
	FRAMETIMER_END(FTT_RC_CAL_COPYRECTTO);
    return i_pParamList[1];
}

DResult             MDOperatorCopyRectTo::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
    return R_SUCCESS;
}
