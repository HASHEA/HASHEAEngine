#include "pch.h"
/*
filename:       MDOperatorPreUWater.cpp
author:         Bin Yang
date:           2017-Oct-24
description:
*/

#include "MDOperatorPreUWater.h"
...
MDOperatorPreUWater::MDOperatorPreUWater()
	: m_OperatorName("MDPreUWater")
{
...
****************************/
const DString& MDOperatorPreUWater::getOperatorName() const
{
	return m_OperatorName;
}

Bool                MDOperatorPreUWater::isGpuOperator() const
{
	return DM_FALSE;
}

Int                 MDOperatorPreUWater::getInputCount() const
{
	return nParamCount;
}

DSimpleTypeID       MDOperatorPreUWater::getInputTypeID(Int i_Index) const
{
	return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

Int                 MDOperatorPreUWater::getOutputCount() const
{
	return 1;
}

DSimpleTypeID       MDOperatorPreUWater::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
	return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorPreUWater::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
...
DResult             MDOperatorPreUWater::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorPreUWater::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
...
DResult             MDOperatorPreUWater::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
