...
#include "mdoperatorrendercompositeresult.h"
...
// MDSceneRender($F4)
static const int s_InputCount = 11;
MDOperatorRenderCompositeResult ::MDOperatorRenderCompositeResult ()
    : m_OperatorName("MDRenderCompositeResult")
{
...
****************************/
const DString&      MDOperatorRenderCompositeResult ::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRenderCompositeResult ::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRenderCompositeResult ::getInputCount() const
{
    return s_InputCount;
}

DSimpleTypeID       MDOperatorRenderCompositeResult ::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorRenderCompositeResult ::getOutputCount() const
{
    return 1;
}

DSimpleTypeID       MDOperatorRenderCompositeResult ::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorRenderCompositeResult ::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	return RGDF_RGBA16F;
}

DResult             MDOperatorRenderCompositeResult ::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
...
****************************/
MDOperandPtr        MDOperatorRenderCompositeResult ::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	FRAMETIMER_BEGIN(FTT_RC_CAL_RENDERCOMPOSITE, FTT_RC_CAL_EXECUTE_EFFECTPASS);
    DOME_ASSERT(i_ParamCount == s_InputCount);
	...
	DOME_ASSERT(i_pParamList[9] && i_pParamList[9]->isTexture());
...
#define GetTexture(name, index) \
	...
	*((OSTexture2D*)l_##name.getPtr()) = l_p##name->getValue<OSTexture2D>();
	...
	GetTexture(AO,				4);
	GetTexture(SSS,				5);
	GetTexture(LightSpecular,	6);
	GetTexture(IBLSpecular,		7);
	GetTexture(SSR,				8);
	GetTexture(ResolveDepth,	9);
	GetTexture(Depth,			10);
	...
	l_pScenePlugin->RenderCompositeResult(
...
DResult             MDOperatorRenderCompositeResult ::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
