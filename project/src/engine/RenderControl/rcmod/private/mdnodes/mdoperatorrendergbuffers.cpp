...
#include "mdoperatorrendergbuffers.h"
...
MDOperatorRenderGBuffers::MDOperatorRenderGBuffers()
    : m_OperatorName("MDRenderGBuffers")
{
...
****************************/
const DString&      MDOperatorRenderGBuffers::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRenderGBuffers::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRenderGBuffers::getInputCount() const
{
    return 2;
}

DSimpleTypeID       MDOperatorRenderGBuffers::getInputTypeID(Int i_Index) const
{
    DOME_ASSERT(i_Index == 0 || i_Index == 1);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

Int                 MDOperatorRenderGBuffers::getOutputCount() const
{
    return 7;
}

DSimpleTypeID       MDOperatorRenderGBuffers::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
...
RCGPUDATAFORMAT     MDOperatorRenderGBuffers::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == 2);
    DOME_ASSERT(i_Index >= 0 && i_Index < 7);
...
DResult             MDOperatorRenderGBuffers::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == 2);
...
****************************/
MDOperandPtr        MDOperatorRenderGBuffers::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
...
DResult             MDOperatorRenderGBuffers::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
