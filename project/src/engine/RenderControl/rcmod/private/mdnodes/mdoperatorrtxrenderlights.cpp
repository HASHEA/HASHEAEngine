#include "pch.h"
/*
    filename:       mdoperatorrtxrenderlights.cpp
    author:         Ming Dong
    date:           2016-JUN-28
    description:    
*/

#include "mdoperatorrtxrenderlights.h"
...
// MDSceneRender($F4)
static const int s_nInputCount = 11;

MDOperatorRTXRenderLights::MDOperatorRTXRenderLights()
    : m_OperatorName("MDRTXRenderLights")
{
...
****************************/
const DString&      MDOperatorRTXRenderLights::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRTXRenderLights::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRTXRenderLights::getInputCount() const
{
    return s_nInputCount;
}

DSimpleTypeID       MDOperatorRTXRenderLights::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorRTXRenderLights::getOutputCount() const
{
    return 2;
}

DSimpleTypeID       MDOperatorRTXRenderLights::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_nInputCount);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorRTXRenderLights::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_nInputCount);
    ...
    if (i_Index == 0)
    {
        ...
        l_Result = i_pParamList[7]->getFloat(l_Params);
...
DResult             MDOperatorRTXRenderLights::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
	DOME_ASSERT(i_pParamList[1] && i_pParamList[1]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorRTXRenderLights::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
...
#define GetTexture(name, index) \
	...
	*((OSTexture2D*)l_##name.getPtr()) = l_p##name->getValue<OSTexture2D>();
    ...
    DSimpleTypedValue* l_pADCValue = i_pParamList[8]->getDataPtr();
    DVector4f l_ADC = l_pADCValue->getDVector4f();
    DSimpleTypedValue* l_pASCValue = i_pParamList[9]->getDataPtr();
    DVector4f l_ASC = l_pASCValue->getDVector4f();
    int l_bShadow = i_pParamList[10]->getDataPtr()->getF32() < 0.5f ? 0 : 1;

	// render
	const static DStringHash k_KEY_ScenePlugin("RCPI_Scene");
	RCPI_Scene* l_pScenePlugin = (RCPI_Scene*)l_pRCEffectMgr->getPlugin(k_KEY_ScenePlugin);
    
	l_pScenePlugin->RTXRenderLights(
...
DResult             MDOperatorRTXRenderLights::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
