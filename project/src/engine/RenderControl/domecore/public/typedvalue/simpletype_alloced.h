...
class TSimpleType_Alloced : public ISimpleType
{
public:
    TSimpleType_Alloced(const DSimpleTypeName& i_TypeName, const DSimpleTypeName& i_GroupName)
        : m_TypeName(i_TypeName)
        , m_GroupName(i_GroupName)
    {
    }

    virtual ~TSimpleType_Alloced(){}
    ...
    {
        o_Value.get() = (PtrInt)DOME_NewEx(TYPE, ALLOCATOR_T);
        return R_SUCCESS;
    }

    virtual DResult             create(DSimpleValuePtr& o_Value, const void* i_pData) const
    {
        o_Value.get() = (PtrInt)DOME_NewEx(TYPE, ALLOCATOR_T)(*((TYPE*)i_pData));
        return R_SUCCESS;
    }

    virtual DResult             create(DSimpleValuePtr& o_Value, const DSimpleValuePtr& i_Value) const
    {
        o_Value.get() = (PtrInt)DOME_NewEx(TYPE, ALLOCATOR_T)(*((const TYPE*)i_Value.get()));
        return R_SUCCESS;
    }
    ...
    {
        DOME_ASSERT(o_Value);
        DOME_DelEx((TYPE*)o_Value.get(), ALLOCATOR_T);
        return R_SUCCESS;
    }
