...
float4 PSMain(VSFullscreenOutput input) : SV_Target0
{
    ...
    const float depth = SceneDepth.SampleLevel(ScenePointClampSampler, input.uv, 0);
    ...
    if (debug_view == 4u)
    {
        const float4 gbuffer_e = SceneGBufferE.SampleLevel(ScenePointClampSampler, input.uv, 0);
