...
void CSMain(uint3 dispatch_id : SV_DispatchThreadID)
{
	...
	{
		float slice_start = AshVolumetricSliceEdgeDepth01(slice);
		float slice_end = AshVolumetricSliceEdgeDepth01(slice + 1u);
		...
		float2 atlas_uv = AshVolumetricAtlasUVFromTileUV(uv, slice, slices_per_row, atlas_inv_size);
