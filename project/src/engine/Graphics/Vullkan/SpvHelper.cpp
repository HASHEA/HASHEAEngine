...
namespace RHI
{
	...
	void parse_binary(const uint32_t* data, size_t data_size, StringBuffer& name_buffer, ParseResult* parse_result)
	{
		H_ASSERT((data_size % 4) == 0);
		uint32_t spv_word_count = safe_cast<uint32_t>(data_size / 4);
		uint32_t spv_magic_number = data[0];
		H_ASSERT(spv_magic_number == 0x07230203);
		uint32_t id_bound = data[3];
		Array<Id> ids;
		ids.init(nullptr, id_bound, id_bound);
		...
		while (word_index < spv_word_count)
		{
			...
			case (SpvOpName):
			{
				...
				char* name_view = name_buffer.append_get(name);
			...
			case (SpvOpTypeStruct):
			{
				...
				if (word_count > 2) {
					const uint32_t members_count = word_count - 2;
					id.count = members_count;
					id.width = 0;

					for (uint16_t member_index = 0; member_index < members_count; ++member_index) {
					...
					const uint32_t size_difference = 16 - (id.width % 16);
		...
		for (uint32_t id_index = 0; id_index < ids.size(); id_index++)
		{
			...
			case (SpvOpSpecConstantComposite):
			{
				...
				StringView::copy_to(id_spec_binding.name, specialization_name.name, 32);
		...
		for (size_t i = 0; i < parse_result->set_count; i++) {
			...
			auto sorting_func = [](const void* a, const void* b) -> int32_t {
