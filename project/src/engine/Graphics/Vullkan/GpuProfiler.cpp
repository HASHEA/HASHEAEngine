...
namespace RHI
{
    ...
    void GpuVisualProfiler::update(VulkanContext& context) {

      /*  context.set_gpu_timestamps_enable(!paused);*/
        ...
        //// Collect timestamps
        //uint32_t active_timestamps = gpu.copy_gpu_timestamps(&timestamps[max_queries_per_frame * current_frame]);
        //per_frame_active[current_frame] = (uint16_t)active_timestamps;
        ...
        //// Get colors
        //for (uint32_t i = 0; i < active_timestamps; ++i) {
        //    GPUTimeQuery& timestamp = timestamps[max_queries_per_frame * current_frame + i];

        //    const double hashed_name = HashCaculate(timestamp.name);
        //    uint32_t color_index = name_to_color.Get(hashed_name);
        //    // No entry found, add new color
        //    if (color_index == UINT32_MAX) {

        //        color_index = (uint32_t)name_to_color.size;
        //        name_to_color.insert(hashed_name, color_index);
        //    }

        //    timestamp.color = Color::get_distinct_color(color_index);
        //}
    ...
    void GPUTimeQueriesManager::init(FramePool* thread_frame_pools_, Allocator* allocator_, uint16_t queries_per_thread_, uint16_t num_threads_, uint16_t max_frames) {
    ...
    uint32_t GPUTimeQueriesManager::resolve(uint32_t current_frame, GPUTimeQuery* timestamps_to_fill) {
        // For each pool
        uint32_t copied_timestamps = 0;
        for (uint32_t t = 0; t < num_threads; ++t) {
            const uint32_t pool_index = (num_threads * current_frame) + t;
    ...
    GPUTimeQuery* GpuTimeQueryTree::push(const char* name) {
