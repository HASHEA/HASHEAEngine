...
class TSimpleType_Embed : public ISimpleType
{
public:
    TSimpleType_Embed(const DSimpleTypeName& i_TypeName, const DSimpleTypeName& i_GroupName)
        : m_TypeName(i_TypeName)
        , m_GroupName(i_GroupName)
    {
    }

    virtual ~TSimpleType_Embed(){}
    ...
    {
        const TYPE& l_v0 = *((TYPE*)&i_V0);
        const TYPE& l_v1 = *((TYPE*)i_pData);
        if(l_v0 < l_v1) return -1;
        else if(l_v0 > l_v1) return 1;
        else return 0;
    }
...
template<>
DResult TSimpleType_Embed<Int>::convertCopy(DSimpleValuePtr& o_Dest, const ISimpleType* i_pSrcType, const void* i_pData) const
{
    DResult hr = R_FAILED;
    DSimpleTypeID l_TypeID = i_pSrcType->getTypeID();
    if (DSimpleTypeManager::Instance().getTypeID_Int() == l_TypeID)
    {    
        Int l_Value = (Int)(*((Int*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F32() == l_TypeID)
    {
        Int l_Value = (Int)(*((F32*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F64() == l_TypeID)
    {
        Int l_Value = (Int)(*((F64*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    return hr;
...
template<>
DResult TSimpleType_Embed<F32>::convertCopy(DSimpleValuePtr& o_Dest, const ISimpleType* i_pSrcType, const void* i_pData) const
{
    DResult hr = R_FAILED;
    DSimpleTypeID l_TypeID = i_pSrcType->getTypeID();
    if (DSimpleTypeManager::Instance().getTypeID_Int() == l_TypeID)
    {    
        F32 l_Value = (F32)(*((Int*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F32() == l_TypeID)
    {
        F32 l_Value = (F32)(*((F32*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F64() == l_TypeID)
    {
        F32 l_Value = (F32)(*((F64*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    return hr;
...
template<>
DResult TSimpleType_Embed<F64>::convertCopy(DSimpleValuePtr& o_Dest, const ISimpleType* i_pSrcType, const void* i_pData) const
{
    DResult hr = R_FAILED;
    DSimpleTypeID l_TypeID = i_pSrcType->getTypeID();
    if (DSimpleTypeManager::Instance().getTypeID_Int() == l_TypeID)
    {    
        F64 l_Value = (F64)(*((Int*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F32() == l_TypeID)
    {
        F64 l_Value = (F64)(*((F32*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F64() == l_TypeID)
    {
        F64 l_Value = (F64)(*((F64*)i_pData));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    return hr;
...
template<>
DResult TSimpleType_Embed<Int>::convertCopy(DSimpleValuePtr& o_Dest, const ISimpleType* i_pSrcType, const DSimpleValuePtr& i_SrcValue) const
{
    DResult hr = R_FAILED;
    DSimpleTypeID l_TypeID = i_pSrcType->getTypeID();
    if (DSimpleTypeManager::Instance().getTypeID_Int() == l_TypeID)
    {    
        Int l_Value = (Int)(*((Int*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F32() == l_TypeID)
    {
        Int l_Value = (Int)(*((F32*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F64() == l_TypeID)
    {
        Int l_Value = (Int)(*((F64*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    return hr;
...
template<>
DResult TSimpleType_Embed<F32>::convertCopy(DSimpleValuePtr& o_Dest, const ISimpleType* i_pSrcType, const DSimpleValuePtr& i_SrcValue) const
{
    DResult hr = R_FAILED;
    DSimpleTypeID l_TypeID = i_pSrcType->getTypeID();
    if (DSimpleTypeManager::Instance().getTypeID_Int() == l_TypeID)
    {    
        F32 l_Value = (F32)(*((Int*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F32() == l_TypeID)
    {
        F32 l_Value = (F32)(*((F32*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F64() == l_TypeID)
    {
        F32 l_Value = (F32)(*((F64*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    return hr;
...
template<>
DResult TSimpleType_Embed<F64>::convertCopy(DSimpleValuePtr& o_Dest, const ISimpleType* i_pSrcType, const DSimpleValuePtr& i_SrcValue) const
{
    DResult hr = R_FAILED;
    DSimpleTypeID l_TypeID = i_pSrcType->getTypeID();
    if (DSimpleTypeManager::Instance().getTypeID_Int() == l_TypeID)
    {    
        F64 l_Value = (F64)(*((Int*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F32() == l_TypeID)
    {
        F64 l_Value = (F64)(*((F32*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    else if (DSimpleTypeManager::Instance().getTypeID_F64() == l_TypeID)
    {
        F64 l_Value = (F64)(*((F64*)&i_SrcValue));
        copy(o_Dest, &l_Value);
        hr = R_SUCCESS;
    }
    return hr;
