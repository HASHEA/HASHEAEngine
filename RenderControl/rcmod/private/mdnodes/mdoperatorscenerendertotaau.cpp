#include "pch.h"
#include "mdoperatorscenerendertotaau.h"
...
MDOperatorSceneRenderToTAAU::MDOperatorSceneRenderToTAAU()
    : m_OperatorName("MDSceneRenderToTAAU")
{
...
****************************/
const DString&      MDOperatorSceneRenderToTAAU::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorSceneRenderToTAAU::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorSceneRenderToTAAU::getInputCount() const
{
    return nParamCount;
}

DSimpleTypeID       MDOperatorSceneRenderToTAAU::getInputTypeID(Int i_Index) const
{
    if(i_Index == 3) return RCGlobal::k_SimpleTypeID_DVector4f;
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

Int                 MDOperatorSceneRenderToTAAU::getOutputCount() const
{
    return 2;
}

DSimpleTypeID       MDOperatorSceneRenderToTAAU::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == nParamCount);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorSceneRenderToTAAU::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
...
DResult             MDOperatorSceneRenderToTAAU::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == nParamCount);
    DOME_ASSERT(i_pParamList[0]	&& i_pParamList[0]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorSceneRenderToTAAU::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	DResult dResult;
	...
	DSimpleTypedValue* l_pScreenStretchRateValue = i_pParamList[3]->getDataPtr();
	DSimpleTypedValue* l_pViewport	 = i_pParamList[4]->getDataPtr();

	F32 l_ScreenStretchRate = l_pScreenStretchRateValue->getF32();
	...
	{
		l_RtSize.x = (l_RtSize.x / l_ScreenStretchRate);
		l_RtSize.y = (l_RtSize.y / l_ScreenStretchRate);
		l_RtSize.x = 1920.0f;
		l_RtSize.y = 1017.0f;
	}
	...
	*((OSTexture2D*)l_ReturnTexture.getPtr()) = l_RtTex;
	...
	//l_pScenePlugin->renderMainCamera_TXAA((RCOSRendererData*)l_pRenderer->getOSRendererData(), &l_ColorTexture, &l_DepthTexture, &l_GBufferDTexture, &l_ReturnTexture, l_ViewPort, TRUE);
...
DResult             MDOperatorSceneRenderToTAAU::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
