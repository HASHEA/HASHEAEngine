...
MDOperatorSSGI::MDOperatorSSGI()
    : m_OperatorName("MDSSGI")
{
...
****************************/
const DString&      MDOperatorSSGI::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorSSGI::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorSSGI::getInputCount() const
{
    return s_InputCount;
}

DSimpleTypeID       MDOperatorSSGI::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorSSGI::getOutputCount() const
{
    return 1;
}

DSimpleTypeID       MDOperatorSSGI::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorSSGI::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	return RGDF_RGBA16F;
}

DResult             MDOperatorSSGI::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	...
	o_Size.x = Math::Min((int)o_Size.x, 1920);
	o_Size.y = Math::Min((int)o_Size.y, 1080);
...
****************************/
MDOperandPtr        MDOperatorSSGI::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	...
	*((OSTexture2D*)l_ColorTexture.getPtr()) = l_RtTex;
	...
	DSimpleTypedValue* l_pf4Param0 = i_pParamList[4]->getDataPtr();
	RCMOD_Float4 l_Param0;
	*(DVector4f*)l_Param0.getPtr() = (l_pf4Param0->getDVector4f());
...
DResult             MDOperatorSSGI::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
