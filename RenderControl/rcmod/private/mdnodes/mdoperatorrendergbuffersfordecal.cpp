#include "pch.h"
#ifndef __MDOPERATORRENDERGBUFFERSFORDECAL_H__
#define __MDOPERATORRENDERGBUFFERSFORDECAL_H__

#include "mdoperatorrendergbuffersfordecal.h"
...
MDOperatorRenderGBuffersForDecal::MDOperatorRenderGBuffersForDecal()
    : m_OperatorName("MDRenderGBuffersForDecal")
{
...
****************************/
const DString&      MDOperatorRenderGBuffersForDecal::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRenderGBuffersForDecal::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRenderGBuffersForDecal::getInputCount() const
{
    return 7;
}

DSimpleTypeID       MDOperatorRenderGBuffersForDecal::getInputTypeID(Int i_Index) const
{
    DOME_ASSERT(i_Index >= 0 && i_Index <= 6);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

Int                 MDOperatorRenderGBuffersForDecal::getOutputCount() const
{
    return 6;
}

DSimpleTypeID       MDOperatorRenderGBuffersForDecal::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
...
RCGPUDATAFORMAT     MDOperatorRenderGBuffersForDecal::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == 7);
    DOME_ASSERT(i_Index >= 0 && i_Index < 7);
...
DResult             MDOperatorRenderGBuffersForDecal::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == 7);
...
****************************/
MDOperandPtr        MDOperatorRenderGBuffersForDecal::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	...
	*((OSTexture2D*)l_DecalMask.getPtr())   = l_pDecalMask->getValue<OSTexture2D>();
	...
	l_pScenePlugin->RenderGBuffersForDecal(
	...
	FRAMETIMER_END(FTT_RC_CAL_RENDERGBUFFERFORDECAL);

    return l_pOperand;
}

DResult             MDOperatorRenderGBuffersForDecal::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
	DOME_Del(i_pResult);
    return R_SUCCESS;
}
