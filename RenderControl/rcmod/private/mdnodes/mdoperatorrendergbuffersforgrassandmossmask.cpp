/*
filename:       mdoperatorrendergbuffersforgrassandambientocclusion.cpp
author:         Wenze Dong
date:           2023 / 09 / 25
description:
*/
...
#include "mdoperatorrendergbuffersforgrassandmossmask.h"
#include <rc/public/iexecuter.h>
#include "../../../DevEnv/Include/PerfAnalyzer.h"

RC_NAMESPACE_BEGIN

static const int s_InputCount = 7; // 4 + 3

MDOperatorRenderGBuffersForGrassAndMossMask::MDOperatorRenderGBuffersForGrassAndMossMask()
    : m_OperatorName("MDRenderGBuffersForGrassAndMossMask")
{
...
****************************/
const DString& MDOperatorRenderGBuffersForGrassAndMossMask::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorRenderGBuffersForGrassAndMossMask::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorRenderGBuffersForGrassAndMossMask::getInputCount() const
{
    return s_InputCount;
}

DSimpleTypeID       MDOperatorRenderGBuffersForGrassAndMossMask::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorRenderGBuffersForGrassAndMossMask::getOutputCount() const
{
    return 8;//7 + 1
}

DSimpleTypeID       MDOperatorRenderGBuffersForGrassAndMossMask::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
...
RCGPUDATAFORMAT     MDOperatorRenderGBuffersForGrassAndMossMask::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_InputCount);
    DOME_ASSERT(i_Index >= 0 && i_Index < 8);
...
DResult             MDOperatorRenderGBuffersForGrassAndMossMask::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == s_InputCount);
...
****************************/
MDOperandPtr        MDOperatorRenderGBuffersForGrassAndMossMask::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
    PERF_COUNTER_EX(0);
    ...
    *((OSTexture2D*)l_NoiseTexture.getPtr()) = l_pNoiseTexture->getValue<OSTexture2D>();
    ...
    DMatrix4x4f paramMat0, paramMat1, paramMat2;
    ...
    enableFusionVT = paramMat0.M(0);
    enableFusionModel = paramMat1.M(1);
    enableMoss = paramMat2.M(6);

    RCMOD_Float4x4 fusionVTParam, fusionModelParam, ambientOcclusionParam;
    fusionVTParam.set(paramMat0.m);
    fusionModelParam.set(paramMat1.m);
    ambientOcclusionParam.set(paramMat2.m);
...
DResult             MDOperatorRenderGBuffersForGrassAndMossMask::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
