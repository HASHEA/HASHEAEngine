...
MDOperatorFSR2::MDOperatorFSR2()
	:m_OperatorName("MDFSR2")
{
}
...
const DString& MDOperatorFSR2::getOperatorName()const
{
	return m_OperatorName;
}

Bool MDOperatorFSR2::isGpuOperator()const
{
	return DM_FALSE;
}

Int MDOperatorFSR2::getInputCount()const
{
	return s_InputCount;
}

Int MDOperatorFSR2::getOutputCount()const
{
	return 1;
}

DSimpleTypeID MDOperatorFSR2::getInputTypeID(Int i_Index)const
{
...
DSimpleTypeID MDOperatorFSR2::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList)const
{
	return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT MDOperatorFSR2::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList)const
{
	return RGDF_RGBA8;
}

DResult MDOperatorFSR2::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList)const
{
...
MDOperandPtr MDOperatorFSR2::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint)const
{
	...
	l_Result = l_pRenderer->createTexture2D(l_OutputTex, l_RtSize.x, l_RtSize.y, 1, l_Format, RBU_DEFAULT, DM_TRUE, DM_NULL, 1);
	DOME_ASSERT(DM_SUCC(l_Result));
	...
	*((OSTexture2D*)l_MotionVectorsInputTexture.getPtr()) = l_pMotionVectorsTextureValue->getValue<OSTexture2D>();
	...
	l_pScenePlugin->renderMainCamera_FSR2UpScale((RCOSRendererData*)l_pRenderer->getOSRendererData(), &l_ColorInputTexture, &l_DepthInputTexture, &l_MotionVectorsInputTexture, nullptr, &l_OutputTexture);
...
DResult MDOperatorFSR2::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult)const
{
