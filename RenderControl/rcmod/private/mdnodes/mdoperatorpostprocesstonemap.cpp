...
#include "mdoperatorpostprocesstonemap.h"
...
// MDSceneRender($F4)
static const unsigned int nParamCount = 8;
MDOperatorPostProcessTonemap::MDOperatorPostProcessTonemap()
    : m_OperatorName("MDPostProcessTonemap")
{
...
****************************/
const DString&      MDOperatorPostProcessTonemap::getOperatorName() const
{
    return m_OperatorName;
}

Bool                MDOperatorPostProcessTonemap::isGpuOperator() const
{
    return DM_FALSE;
}

Int                 MDOperatorPostProcessTonemap::getInputCount() const
{
    return nParamCount;
}

DSimpleTypeID       MDOperatorPostProcessTonemap::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorPostProcessTonemap::getOutputCount() const
{
    return 1;
}

DSimpleTypeID       MDOperatorPostProcessTonemap::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == nParamCount);
    return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorPostProcessTonemap::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == nParamCount);
...
DResult             MDOperatorPostProcessTonemap::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
    DOME_ASSERT(i_ParamCount == nParamCount);
    DOME_ASSERT(i_pParamList[0]	&& i_pParamList[0]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorPostProcessTonemap::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
...
DResult             MDOperatorPostProcessTonemap::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
