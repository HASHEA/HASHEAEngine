/*
    filename:       mdoperatorscenerender.h
    author:         Ming Dong
    date:           2016-Aug-24
    description:    
*/
...
enum MDWRITETOTEXTURE_STATE
{
	MDWT_WAITING,
	MDWT_WRITING
};

class MDOperatorRenderToTexture : public MDOperatorCpu 
{

public:
    MDOperatorRenderToTexture();
    virtual ~MDOperatorRenderToTexture();
...
protected:
	void _WriteToTexture(RCEffectManager* i_EffectManager, RCMOD_Texture* i_ColorSrcTexture, RCMOD_String i_FilePath) const;

private:
    mutable DString					m_OutputTypeName;
    DString							m_OperatorName;

	mutable MDWRITETOTEXTURE_STATE	m_emState;
	mutable float					m_fTimer;
	mutable int						m_nFileIndex;
};
