#include "pch.h"
/*
	filename:       mdoperatorrtxrenderreflection.cpp
	author:         Yang Bin
	date:           2020-Apr-15
	description:
*/

#include "mdoperatorrtxrenderreflection.h"
...
// MDSceneRender($F4)
static const int s_nInputCount = 8;

MDOperatorRTXRenderReflection::MDOperatorRTXRenderReflection()
	: m_OperatorName("MDRTXRenderReflection")
{
...
****************************/
const DString& MDOperatorRTXRenderReflection::getOperatorName() const
{
	return m_OperatorName;
}

Bool                MDOperatorRTXRenderReflection::isGpuOperator() const
{
	return DM_FALSE;
}

Int                 MDOperatorRTXRenderReflection::getInputCount() const
{
	return s_nInputCount;
}

DSimpleTypeID       MDOperatorRTXRenderReflection::getInputTypeID(Int i_Index) const
{
...
Int                 MDOperatorRTXRenderReflection::getOutputCount() const
{
	return 2;
}

DSimpleTypeID       MDOperatorRTXRenderReflection::getOutputTypeID(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	return RCGlobal::k_SimpleTypeID_OSTexture2D;
}

RCGPUDATAFORMAT     MDOperatorRTXRenderReflection::getOutputTexFmt(Int i_Index, MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
...
DResult             MDOperatorRTXRenderReflection::calcOutputTexSize(Int i_Index, MDEffect* i_pMDEffect, DVector2i& o_Size, Int i_ParamCount, const MDOperandCPtr* i_pParamList) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
	DOME_ASSERT(i_pParamList[1] && i_pParamList[1]->isTexture());
...
****************************/
MDOperandPtr        MDOperatorRTXRenderReflection::execute(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, IRP* o_pInputReleasePoint) const
{
	DOME_ASSERT(i_ParamCount == s_nInputCount);
	DOME_ASSERT(i_pParamList[0] && i_pParamList[0]->isTexture());
	DOME_ASSERT(i_pParamList[1] && i_pParamList[1]->isTexture());
...
#define GetTexture(name, index) \
	...
	*((OSTexture2D*)l_##name.getPtr()) = l_p##name->getValue<OSTexture2D>();
	...
	l_pScenePlugin->RTXRenderReflection(
...
DResult             MDOperatorRTXRenderReflection::destroyResult(MDEffect* i_pMDEffect, Int i_ParamCount, const MDOperandPtr* i_pParamList, MDOperandPtr i_pResult) const
{
