...
class RCGBuffer3Node : public RCEffectNode
{
public:
    static RCEffectNode*    Create(RCEffect* i_pEffect);
    static DResult          Destroy(RCEffectNode* i_pEffectNode);

    RCGBuffer3Node(RCEffect* i_pEffect);
    virtual ~RCGBuffer3Node();
...
private:
    MDOperandValuePtr       m_GBuffer3Texture;
};
