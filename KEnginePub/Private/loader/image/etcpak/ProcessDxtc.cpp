...
static etcpak_force_inline void ProcessRGB_AVX(const uint8_t* src, char*& dst)
{
    ...
    for (int j = 4; j < 8; j++)
        dst[j] = (char)DxtcIndexTable[(uint8_t)dst[j]];
    for (int j = 12; j < 16; j++)
        dst[j] = (char)DxtcIndexTable[(uint8_t)dst[j]];
...
void ETC_PAK::CompressDxt1(const uint32_t* src, uint64_t* dst, uint32_t blocks, size_t width)
{
#ifdef __AVX2__
    if (width % 8 == 0)
    {
...
void ETC_PAK::CompressDxt1Dither(const uint32_t* src, uint64_t* dst, uint32_t blocks, size_t width)
{
    uint32_t buf[4 * 4];
    int      i = 0;
...
void ETC_PAK::CompressDxt5(const uint32_t* src, uint64_t* dst, uint32_t blocks, size_t width)
{
    int  i   = 0;
    auto ptr = dst;
    do
