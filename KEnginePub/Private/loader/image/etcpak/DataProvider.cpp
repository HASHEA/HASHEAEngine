...
namespace ETC_PAK
{
    ...
    DataProvider::DataProvider(uint32_t* pData, size_t uCount, uint32_t width, uint32_t height, bool bAlpha, bool mipmap, bool linearize)
        : m_offset(0)
        , m_lines(32)
        , m_mipmap(mipmap)
        , m_done(false)
        , m_linearize(linearize)
    {
        m_bmp.emplace_back(new Bitmap(pData, uCount, width, height, bAlpha, m_lines));
        m_current = m_bmp[0].get();
    }
