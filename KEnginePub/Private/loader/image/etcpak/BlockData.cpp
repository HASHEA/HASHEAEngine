...
#define INTERNAL_RGBA_PUNCHTHROUGH_ETC2 0x9276
...
typedef struct
{
    ...
    uint32_t bytesOfKeyValueData; // 预留=32
    // 扩展出来预留32位keyvalue data 区域
    struct _resveKeyValueData
    {
        uint32_t keyAndValueByteSize;
        uint32_t tgaConvertMask;
        uint32_t ddsConvertMask;
        char     channel;
        char     resvdata[32 - sizeof(uint32_t) * 3 - sizeof(char)];
    } resveKeyValueData;
} _KTX_header;
...
// static uint8_t* OpenForWriting(const char* fn, size_t len, const v2i& size, FILE** f, int levels, Type type)
//{
//	*f = fopen(fn, "wb+");
//	assert(*f);
...
static int AdjustSizeForMipmaps(const v2i& size, int levels)
{
    ...
    for (int i = 1; i < levels; i++)
    {
        ...
        uint32_t bytes  = std::max(4, current.x) * std::max(4, current.y) / 2;
    ...
    , m_type(type)
{
    m_levels = 1;
    assert(m_size.x % 4 == 0 && m_size.y % 4 == 0);
    InitDataAndHeader(mipmap);
}

BOOL BlockData::InitDataAndHeader(BOOL mipmap)
{
    BOOL bRet     = false;
    BOOL bRetCode = false;
    m_levels      = 1;
    ...
    if (mipmap)
    {
        m_levels  = NumberOfMipLevels(m_size);
        m_maplen += AdjustSizeForMipmaps(m_size, m_levels);
    }

    if (m_type == Etc2_RGBA || m_type == Dxt5)
    {
        m_maplen *= 2;
    }

    if (m_type == Dxt1 || m_type == Dxt5)
    {
        m_dataOffset = sizeof(_DDS_header);
    }
    else if (m_type == Etc2_RGB || m_type == Etc2_RGBA)
    {
        m_dataOffset  = sizeof(_KTX_header);
        // ktx每个mipmaplevel前面还要加个长度，真他妈的啰嗦，后面多线程写完，再用finish来调整位置
        m_maplen     += m_levels * sizeof(uint32_t);
    }
    ...
    memset(m_data, 0, m_maplen);

    if (m_type == Dxt1 || m_type == Dxt5)
    {
        _DDS_header* header          = (_DDS_header*)m_data;
        header->dwMagic              = ('D' << 0) | ('D' << 8) | ('S' << 16) | (' ' << 24);
        header->dwSize               = 124;
        header->dwFlags              = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT | DDSD_LINEARSIZE | DDSD_PIXELFORMAT | DDSD_BACKBUFFERCOUNT | DDSD_MIPMAPCOUNT;
        header->dwWidth              = width;
        header->dwHeight             = height;
        header->sPixelFormat.dwSize  = 32;
        header->sPixelFormat.dwFlags = DDPF_FOURCC;
        header->dwMipMapCount        = m_levels;
        if (m_type == Dxt1)
        {
            //	get the RAM for the compressed image
            //(8 bytes per 4x4 pixel block)
            header->dwPitchOrLinearSize   = ((width + 3) >> 2) * ((height + 3) >> 2) * 8;
        ...
        else
        {
            ...
            header->dwPitchOrLinearSize   = ((width + 3) >> 2) * ((height + 3) >> 2) * 16;
    ...
    {
        _KTX_header* header                   = (_KTX_header*)m_data;
        uint8_t      identifier_reference[12] = KTX_IDENTIFIER_REF;
        memcpy(header->identifier, identifier_reference, 12);
        ...
        if (m_type == Etc2_RGB)
        {
            ...
            header->resveKeyValueData.channel = 3;
        }
        else if (m_type == Etc2_RGBA)
        {
            header->glInternalFormat          = GL_COMPRESSED_RGBA8_ETC2_EAC;
            header->glBaseInternalFormat      = GL_RGBA;
            header->resveKeyValueData.channel = 4;
        }
        else
        {
            assert(0);
        ...
        header->numberOfMipmapLevels  = m_levels;
        header->bytesOfKeyValueData   = 32;
    }
...
void BlockData::FixData()
{
    ...
    if (m_levels > 2)
    {
        // 最后两个level是黑的，可能是原来的算法没算最后两个level，把倒数第三个level的block复制到最后两个level上去
        size_t expandSize = 0;
        if (m_type == Etc2_RGB || m_type == Etc2_RGBA)
        {
            expandSize = m_levels * sizeof(uint32_t);
        }
        size_t pos = m_maplen - expandSize - blockbytes * 3;
        memcpy(m_data + pos + blockbytes, m_data + pos, blockbytes);
        memcpy(m_data + pos + blockbytes * 2, m_data + pos, blockbytes);
    }
    ...
    if (m_type == Etc2_RGB || m_type == Etc2_RGBA)
    {
        ...
        for (uint8_t i = 0; i < m_levels; ++i)
        {
            // uint32_t levelbytes = width / 4 * height / 4
            uint32_t blockw = std::max<uint32_t>(1, width / 4);
            uint32_t blockh = std::max<uint32_t>(1, height / 4);

            uint32_t levelbytes = blockw * blockh * blockbytes;

            size_t moveBytes = m_maplen - (offset + sizeof(uint32_t));
            assert(moveBytes > 0);
            memmove(m_data + offset + sizeof(uint32_t), m_data + offset, moveBytes);
            memcpy(m_data + offset, &levelbytes, sizeof(uint32_t));
            offset += levelbytes + sizeof(uint32_t);
...
namespace
{
    ...
    static etcpak_force_inline void DecodeT(uint64_t block, uint32_t* dst, uint32_t w)
    {
        ...
        const auto c2r = clampu8((int32_t)(cr1 + table59T58H[codeword]));
        const auto c2g = clampu8((int32_t)(cg1 + table59T58H[codeword]));
        const auto c2b = clampu8((int32_t)(cb1 + table59T58H[codeword]));

        const auto c3r = clampu8((int32_t)(cr1 - table59T58H[codeword]));
        const auto c3g = clampu8((int32_t)(cg1 - table59T58H[codeword]));
        const auto c3b = clampu8((int32_t)(cb1 - table59T58H[codeword]));
    ...
    static etcpak_force_inline void DecodeH(uint64_t block, uint32_t* dst, uint32_t w)
    {
        ...
        const uint32_t col_tab[] = {
            uint32_t(clampu8((int32_t)(r0 + table59T58H[codeword])) | (clampu8((int32_t)(g0 + table59T58H[codeword])) << 8) | (clampu8((int32_t)(b0 + table59T58H[codeword])) << 16)),
            uint32_t(clampu8((int32_t)(r0 - table59T58H[codeword])) | (clampu8((int32_t)(g0 - table59T58H[codeword])) << 8) | (clampu8((int32_t)(b0 - table59T58H[codeword])) << 16)),
            uint32_t(clampu8((int32_t)(r1 + table59T58H[codeword])) | (clampu8((int32_t)(g1 + table59T58H[codeword])) << 8) | (clampu8((int32_t)(b1 + table59T58H[codeword])) << 16)),
            uint32_t(clampu8((int32_t)(r1 - table59T58H[codeword])) | (clampu8((int32_t)(g1 - table59T58H[codeword])) << 8) | (clampu8((int32_t)(b1 - table59T58H[codeword])) << 16))
        };
...
uint8_t* BlockData::GetBuffer()
{
    return m_data;
}

size_t BlockData::GetBufferLen()
{
    return m_maplen;
}
