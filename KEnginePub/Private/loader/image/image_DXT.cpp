...
)
{
    ...
    /*	write it out	*/
    fout                 = KGFOpen(filename, "wb");
    if (fout)
    {
        {
            KGFWrite(&header, sizeof(DDS_header), fout);
            KGFWrite(DDS_data, DDS_size, fout);
        }
        KGFClose(fout);
    }
    /*	done	*/
        ...
        for (i = 0; i < width; i += 4)
        {
            ...
            for (y = 0; y < my; ++y)
            {
                for (x = 0; x < mx; ++x)
                {
                    ...
                    ublock[idx++] = uncompressed[(j + y) * width * channels + (i + x) * channels + chan_step + chan_step];
                    ublock[idx++] =
                        has_alpha * uncompressed[(j + y) * width * channels + (i + x) * channels + channels - 1] + (1 - has_alpha) * 255;
                }
                for (x = mx; x < 4; ++x)
                {
                    ublock[idx++] = ublock[0];
