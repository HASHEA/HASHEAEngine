...
#include "KEnginePub/Private/profile/microprofile/stb/stb_sprintf.h"
...
struct MicroProfile
{
	...
	MicroProfileGpuTimerStateVulkan* pVulkanGPU;
...
bool MicroProfileWebServerUpdate_Vulkan();
void MicroProfileDumpToFile_Vulkan();
...
#define MicroProfileWebServerUpdate_Vulkan() false;
void MicroProfileDumpToFile_Vulkan() do {} while (0)
#endif 
...
uint64_t MicroProfileGpuEnterInternal_Vulkan(MicroProfileThreadLogGpu* pGpuLog, MicroProfileToken nToken_)
{
	if (MicroProfileGroupTokenActive(nToken_))
	{
		if (!MicroProfileGetThreadLog())
		...
		MP_ASSERT(pGpuLog->pContext != (void*)-1); // must be called between GpuBegin/GpuEnd
		const KMicroProfile::KMicroProfileVulkanFunctions* pFuntions = &(KMicroProfile::GPUProfilerVulkan::s_VulkanFunctions);
		MP_ASSERT(pFuntions);
		...
		uint64_t nTimer = MicroProfileGpuInsertTimeStamp_Vulkan(pGpuLog->pContext, pFuntions);
		MicroProfileLogPutGpu(nToken_, nTimer, MP_LOG_ENTER, pGpuLog);
		MicroProfileThreadLog* pLog = MicroProfileGetThreadLog();
		MicroProfileLogPutGpu(ETOKEN_GPU_CPU_TIMESTAMP, MP_TICK(), MP_LOG_EXTRA_DATA, pGpuLog);
		MicroProfileLogPutGpu(ETOKEN_GPU_CPU_SOURCE_THREAD, pLog->nLogIndex, MP_LOG_EXTRA_DATA, pGpuLog);
...
void MicroProfileGpuLeaveInternal_Vulkan(MicroProfileThreadLogGpu* pGpuLog, MicroProfileToken nToken_, uint64_t nTickStart)
{
	if (nTickStart)
	{
		if (!MicroProfileGetThreadLog())
...
void MicroProfileFlip_Vulkan(void* pContext)
{
	MicroProfileFlip_CB_Vulkan(pContext, nullptr);
}
...
void MicroProfileFlip_CB_Vulkan(void* pContext, MicroProfileOnFreeze FreezeCB)
{
	MICROPROFILE_COUNTER_LOCAL_UPDATE_SET_ATOMIC(g_MicroProfileBytesPerFlip);
	MICROPROFILE_SCOPE(g_MicroProfileFlip);
	std::lock_guard<std::recursive_mutex> Lock(MicroProfileMutex());
	...
	do 
	{
		if (MicroProfileWebServerUpdate_Vulkan())
		{
			S.nAutoClearFrames = MICROPROFILE_GPU_FRAME_DELAY + 3; //hide spike from dumping webpage
		}
	...
	if (nRunning)
	{
		...
		MicroProfileGpuBegin(pContext, S.pGpuGlobal);
		uint32_t nGpuTimeStamp = MicroProfileGpuFlip_Vulkan(pContext);
		...
		pFramePut->nFrameStartGpu = nGpuTimeStamp;
		{
			...
			{
				uint64_t nTickCurrent = pFrameCurrent->nFrameStartGpu;
				uint64_t nTickNext = pFrameNext->nFrameStartGpu = MicroProfileGpuGetTimeStamp_Vulkan((uint32_t)pFrameNext->nFrameStartGpu);
				nTickCurrent = MicroProfileLogTickMin(nTickCurrent, nTickNext);
				float fTime = 1000.f * (nTickNext - nTickCurrent) / MicroProfileTicksPerSecondGpu_Vulkan();
				fTime = fTimeGpu;
				if (S.fDumpGpuSpike > 0.f && fTime > S.fDumpGpuSpike && fTime < fDumpTimeThreshold)
				{
					nDumpNextFrame = 1;
				...
				if (pLog->nGpu)
				{
					...
					for (uint32_t j = 0; j < 2; ++j)
					{
						...
						for (uint32_t k = nStart; k < nEnd; ++k)
						{
							...
							if (MicroProfileLogGetType(L) < MP_LOG_EXTRA_DATA)
							{
								pLog->Log[k] = MicroProfileLogSetTick(L, MicroProfileGpuGetTimeStamp_Vulkan((uint32_t)MicroProfileLogGetTick(L)));
							}
						}
					}
				}
...

		{
...

			{
				...
				for (uint32_t idx_thread = 0; idx_thread < MICROPROFILE_MAX_THREADS; ++idx_thread)
				{
					...
					float fToMs = bGpu ? MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu_Vulkan()) : MicroProfileTickToMsMultiplierCpu();
					float fFrameTime = fToMs * (nTickEndLog - nTickStartLog);
					if (fFrameTime > 1000)
					{
						uprintf("very long frame\n");
...
void MicroProfileCalcAllTimers_Vulkan(float* pTimers, float* pAverage, float* pMax, float* pMin, float* pCallAverage, float* pExclusive, float* pAverageExclusive, float* pMaxExclusive, float* pTotal, uint32_t nSize)
{
	for (uint32_t i = 0; i < S.nTotalTimers && nSize; ++i)
	{
		const uint32_t nGroupId = S.TimerInfo[i].nGroupIndex;
		const float fToMs = MicroProfileTickToMsMultiplier(S.GroupInfo[nGroupId].Type == MicroProfileTokenTypeGpu ? MicroProfileTicksPerSecondGpu_Vulkan() : MicroProfileTicksPerSecondCpu());
		uint32_t nTimer = i;
		uint32_t nIdx = i * 2;
		uint32_t nAggregateFrames = S.nAggregateFrames ? S.nAggregateFrames : 1;
		uint32_t nAggregateCount = S.Aggregate[nTimer].nCount ? S.Aggregate[nTimer].nCount : 1;
...
void MicroProfileDumpCsv_Vulkan(MicroProfileWriteCallback CB, void* Handle)
{
	uint32_t nAggregateFrames = S.nAggregateFrames ? S.nAggregateFrames : 1;
	float fToMsCPU = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
	float fToMsGPU = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu_Vulkan());
	...
	MicroProfileCalcAllTimers_Vulkan(pTimers, pAverage, pMax, pMin, pCallAverage, pTimersExclusive, pAverageExclusive, pMaxExclusive, pTotal, nNumTimers);
...
void MicroProfileDumpHtml(MicroProfileWriteCallback CB, void* Handle, uint64_t nMaxFrames, const char* pHost, uint64_t nStartFrameId = (uint64_t)-1)
{
	...
	for(uint32_t i = 0; i < S.nNumLogs; ++i)
	{
		if(S.Pool[i])
		{
			...
			// Type correctness fixes #87 in MicroProfile
				...
				case MP_LOG_PAYLOAD:
					{
						...
						char* pDstEnd = &String[MICROPROFILE_MAX_STRING-7];
						{
							while(k != nLogEnd)
							{
								...
								if(pDst < pDstEnd)
								{
									MicroProfileGetLogPayload(v2, pDst);
									pDst += 7;
								}
								k = (k+1) % MICROPROFILE_BUFFER_SIZE;
							}
...
void MicroProfileDumpHtml_Vulkan(MicroProfileWriteCallback CB, void* Handle, uint64_t nMaxFrames, const char* pHost, uint64_t nStartFrameId = (uint64_t)-1)
{
	//stall pushing of timers
	uint64_t nActiveGroup[MICROPROFILE_MAX_GROUP_INTS];
	memcpy(nActiveGroup, S.nActiveGroups, sizeof(S.nActiveGroups));
	...
	int64_t nTickReferenceCpu, nTickReferenceGpu;
	int64_t nTicksPerSecondCpu = MicroProfileTicksPerSecondCpu();
	int64_t nTicksPerSecondGpu = MicroProfileTicksPerSecondGpu_Vulkan();
	int nTickReference = 0;
	if (MicroProfileGetGpuTickReference_Vulkan(&nTickReferenceCpu, &nTickReferenceGpu))
	{
		nTickStartGpu = (nTickStart - nTickReferenceCpu) * nTicksPerSecondGpu / nTicksPerSecondCpu + nTickReferenceGpu;
		nTickReference = 1;
	}
...
#define pp(...) MicroProfilePrintf(CB, Handle, __VA_ARGS__)
	...
	MicroProfilePrintf(CB, Handle, "];\n");
	MicroProfilePrintf(CB, Handle, "S.CSwitchTime = [");
	fToMsCpu = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
	for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd; j = (j + 1) % MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE)
	{
		MicroProfileContextSwitch CS = S.ContextSwitch[j];
		float fTime = MicroProfileLogTickDifference(nTickStart, CS.nTicks) * fToMsCpu;
...
void MicroProfileDumpToFile_Vulkan()
{
	...
	if (S.nDumpFileNextFrame & 1)
	{
		...
		if (F)
		{
			MicroProfileDumpHtml_Vulkan(MicroProfileWriteFile, F, MICROPROFILE_WEBSERVER_MAXFRAMES, S.HtmlDumpPath);
			fclose(F);
		}
	}
	if (S.nDumpFileNextFrame & 2)
	{
		...
		if (F)
		{
			MicroProfileDumpCsv_Vulkan(MicroProfileWriteFile, F);
			fclose(F);
		}
	}
}
...
void MicroProfileWebSocketSendFrame_Vulkan(MpSocket Connection)
{
	if (S.nFrameCurrent != S.WebSocketFrameLast[0] || S.nFrozen)
	{
		MicroProfileWebSocketSendState(Connection);
		...
		MicroProfileWSPrintStart(Connection);
		float fTickToMsCpu = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
		float fTickToMsGpu = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu_Vulkan());
		MicroProfileFrameState* pFrameCurrent = &S.Frames[S.nFrameCurrent];
		MicroProfileFrameState* pFrameNext = &S.Frames[S.nFrameNext];
...
void MicroProfileWebSocketFrame_Vulkan()
{
	if (!S.nNumWebSockets)
	{
		return;
	...
	for (uint32_t i = 0; i < S.nNumWebSockets;)
	{
		...
		if (FD_ISSET(s, &Write))
		{
			...
			MicroProfileWebSocketSendFrame_Vulkan(s);
		}
		if (S.nSocketFail)
		{
			bConnected = false;
...
bool MicroProfileWebServerUpdate_Vulkan()
{
	...
	MpSocket Connection = accept(S.ListenerSocket, 0, 0);
	bool bServed = false;
	MicroProfileWebSocketFrame_Vulkan();
	if (!MP_INVALID_SOCKET(Connection))
	{
		std::lock_guard<std::recursive_mutex> Lock(MicroProfileMutex());
		char Req[8192];
					...
					{
						{
							...
							S.WebServerPut = 0;
#if 0 == MICROPROFILE_MINIZ
							MicroProfileDumpHtml_Vulkan(MicroProfileWriteSocket, &Connection, R.nFrames, pHost, R.nFrameStart);
							uint64_t nDataEnd = S.nWebServerDataSent;
							uint64_t nTickEnd = MP_TICK();
							uint64_t nDiff = (nTickEnd - nTickStart);
							float fMs = MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu()) * nDiff;
							...
							MicroProfileCompressedSocketState CompressState;
							MicroProfileCompressedSocketStart(&CompressState, Connection);
							MicroProfileDumpHtml_Vulkan(MicroProfileCompressedWriteSocket, &CompressState, R.nFrames, pHost, R.nFrameStart);
							S.nWebServerDataSent += CompressState.nSize;
							uint64_t nDataEnd = S.nWebServerDataSent;
							uint64_t nTickEnd = MP_TICK();
							uint64_t nDiff = (nTickEnd - nTickStart);
...
void MicroProfileContextSwitchShutdownTrace()
{
	...
	char loggerNameBuffer[sizeof(KERNEL_LOGGER_NAME)] = KERNEL_LOGGER_NAME;
	EVENT_TRACE_LOGFILE log;
	ZeroMemory(&log, sizeof(log));
    log.LoggerName = loggerNameBuffer;
...
#endif //#if MICROPROFILE_GPU_TIMERS_D3D12

#if MICROPROFILE_GPU_TIMERS_VULKAN

// Test old version
#define TEST_OLD_VERSION 1
#if TEST_OLD_VERSION
uint32_t MicroProfileGpuInsertTimeStamp(void* pContext)
{
	const KMicroProfile::KMicroProfileVulkanFunctions* functions = &(KMicroProfile::GPUProfilerVulkan::s_VulkanFunctions);
	...
	functions->vkCmdWriteTimestamp(CB, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, S.pGPU->QueryPool[nNode], nQueryIndex);
	MP_ASSERT(nQueryIndex <= 0xffff);
	//uprintf("insert timestamp %d :: %d ... ctx %p\n", nQueryIndex, nFrame, pContext);
	return ((nFrame << 16) & 0xffff0000) | (nQueryIndex);
}
...
void MicroProfileGpuFetchRange(VkCommandBuffer CommandBuffer, uint32_t nNode, uint32_t nBegin, int32_t nCount, uint64_t nFrame, int64_t nTimestampOffset)
{
	...
	functions->vkGetQueryPoolResults(S.pGPU->Devices[nNode], S.pGPU->QueryPool[nNode], nBegin, nCount, 8 * nCount, &S.pGPU->nResults[nBegin], 8, VK_QUERY_RESULT_64_BIT);
	...
	functions->vkCmdResetQueryPool(CommandBuffer, S.pGPU->QueryPool[nNode], nBegin, nCount);
	for (int i = 0; i < nCount; ++i)
	{
		S.pGPU->nQueryFrames[i + nBegin] = nFrame;
	}
...
void MicroProfileGpuWaitFence(uint32_t nNode, uint64_t nFrame)
{
	...
	{
		MICROPROFILE_SCOPEI("Microprofile", "gpu-wait", MP_GREEN4);
		r = functions->vkWaitForFences(S.pGPU->Devices[nNode], 1, &S.pGPU->Frames[nFrame].Fences[nNode], 1, 1000 * 30);
#if 0
		if (c++ > 1000 && (c % 100) == 0)
		{
...
uint32_t MicroProfileGpuFlip(void* pContext)
{
	...
	functions->vkWaitForFences(Device, 1, &Fence, 1, (uint64_t)-1);
	uint32_t nFrameTimeStamp = MicroProfileGpuInsertTimeStamp(pContext);
	functions->vkResetCommandBuffer(F.CommandBuffer[nNode], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
	...
	functions->vkBeginCommandBuffer(F.CommandBuffer[nNode], &CBI);
	...
	functions->vkEndCommandBuffer(F.CommandBuffer[nNode]);
	...
	functions->vkQueueSubmit(Queue, 1, &SubmitInfo, Fence);
	return nFrameTimeStamp;
}
...
int MicroProfileGetGpuTickReference(int64_t* pOutCPU, int64_t* pOutGpu)
{
	...
	functions->vkWaitForFences(Device, 1, &Fence, 1, (uint64_t)-1);
	functions->vkResetFences(Device, 1, &Fence);
	functions->vkResetCommandBuffer(CB, VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
	functions->vkBeginCommandBuffer(CB, &CBI);
	functions->vkCmdResetQueryPool(CB, S.pGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES, 1);
	functions->vkCmdWriteTimestamp(CB, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, S.pGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES);
	functions->vkEndCommandBuffer(CB);
	VkSubmitInfo SubmitInfo = {};
	SubmitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
	SubmitInfo.pNext = nullptr;
	SubmitInfo.waitSemaphoreCount = 0;
	...
	SubmitInfo.signalSemaphoreCount = 0;
	SubmitInfo.pSignalSemaphores = nullptr;
	functions->vkQueueSubmit(S.pGPU->Queues[nGpu], 1, &SubmitInfo, Fence);
	functions->vkWaitForFences(Device, 1, &Fence, 1, (uint64_t)-1);
	*pOutGpu = 0;
	functions->vkGetQueryPoolResults(Device, S.pGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES, 1, 8, pOutGpu, 8, VK_QUERY_RESULT_64_BIT);
	*pOutCPU = MP_TICK();
	return 1;
}
...
uint32_t MicroProfileGpuInsertTimeStamp_Vulkan(void* pContext, const KMicroProfile::KMicroProfileVulkanFunctions* functions)
{
	MP_ASSERT(functions);
	VkCommandBuffer CB = (VkCommandBuffer)pContext;
	uint32_t nNode = S.pVulkanGPU->nCurrentNode;
	uint32_t nFrame = S.pVulkanGPU->nFrame;
	uint32_t nQueryIndex = (S.pVulkanGPU->nFrameCount.fetch_add(1) + S.pVulkanGPU->nFrameStart) % MICROPROFILE_VULKAN_MAX_QUERIES;
	functions->vkCmdWriteTimestamp(CB, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, S.pVulkanGPU->QueryPool[nNode], nQueryIndex);
	MP_ASSERT(nQueryIndex <= 0xffff);
	//uprintf("insert timestamp %d :: %d ... ctx %p\n", nQueryIndex, nFrame, pContext);
	return ((nFrame << 16) & 0xffff0000) | (nQueryIndex);
}

void MicroProfileGpuFetchRange_Vulkan(VkCommandBuffer CommandBuffer, uint32_t nNode, uint32_t nBegin, int32_t nCount, uint64_t nFrame, int64_t nTimestampOffset, const KMicroProfile::KMicroProfileVulkanFunctions* functions)
{
	MP_ASSERT(functions);
	if (nCount <= 0)
		return;

	//functions->vkGetQueryPoolResults(S.pVulkanGPU->Devices[nNode], S.pVulkanGPU->QueryPool[nNode], nBegin, nCount, 8 * nCount, &S.pVulkanGPU->nResults[nBegin], 8, VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_PARTIAL_BIT);
	functions->vkGetQueryPoolResults(S.pVulkanGPU->Devices[nNode], S.pVulkanGPU->QueryPool[nNode], nBegin, nCount, 8 * nCount, &S.pVulkanGPU->nResults[nBegin], 8, VK_QUERY_RESULT_64_BIT);
	functions->vkCmdResetQueryPool(CommandBuffer, S.pVulkanGPU->QueryPool[nNode], nBegin, nCount);
	for (int i = 0; i < nCount; ++i)
	{
		S.pVulkanGPU->nQueryFrames[i + nBegin] = nFrame;
	}
}
void MicroProfileGpuWaitFence_Vulkan(uint32_t nNode, uint64_t nFrame, const KMicroProfile::KMicroProfileVulkanFunctions* functions)
{
	MP_ASSERT(functions);
	int r;
	int c = 0;
	...
	{
		MICROPROFILE_SCOPEI("Microprofile", "gpu-wait", MP_GREEN4);
		r = functions->vkWaitForFences(S.pVulkanGPU->Devices[nNode], 1, &S.pVulkanGPU->Frames[nFrame].Fences[nNode], 1, 1000 * 30);
#if 0
		if (c++ > 1000 && (c % 100) == 0)
		{
			uprintf("waiting really long time for fence\n");
...
void MicroProfileGpuFetchResults_Vulkan(VkCommandBuffer Buffer, uint64_t nFrame, const KMicroProfile::KMicroProfileVulkanFunctions* functions)
{
	uint64_t nPending = S.pVulkanGPU->nPendingFrame;
	//while(nPending <= nFrame)
	//while(0 <= nFrame - nPending)
	while (0 <= (int64_t)(nFrame - nPending))
	{
		uint32_t nInternal = nPending % MICROPROFILE_VULKAN_INTERNAL_DELAY;
		uint32_t nNode = S.pVulkanGPU->Frames[nInternal].nNode;
		MicroProfileGpuWaitFence_Vulkan(nNode, nInternal, functions);
		int64_t nTimestampOffset = 0;
		...
		uint32_t nBegin = S.pVulkanGPU->Frames[nInternal].nBegin;
		uint32_t nCount = S.pVulkanGPU->Frames[nInternal].nCount;
		MicroProfileGpuFetchRange_Vulkan(Buffer, nNode, nBegin, (nBegin + nCount) > MICROPROFILE_VULKAN_MAX_QUERIES ? MICROPROFILE_VULKAN_MAX_QUERIES - nBegin : nCount, nPending, nTimestampOffset, functions);
		MicroProfileGpuFetchRange_Vulkan(Buffer, nNode, 0, (nBegin + nCount) - MICROPROFILE_VULKAN_MAX_QUERIES, nPending, nTimestampOffset, functions);

		nPending = ++S.pVulkanGPU->nPendingFrame;
		MP_ASSERT(S.pVulkanGPU->nFrame > nPending);
	}

}

uint64_t MicroProfileGpuGetTimeStamp_Vulkan(uint32_t nIndex)
{
	if (nIndex == (uint32_t)-1)
	{
		return 0;
	...
	uint32_t nFrame = nIndex >> 16;
	uint32_t nQueryIndex = nIndex & 0xffff;
	uint32_t lala = S.pVulkanGPU->nQueryFrames[nQueryIndex];
	MP_ASSERT((0xffff & lala) == nFrame);
	//uprintf("read TS [%d <- %lld]\n", nQueryIndex, S.pVulkanGPU->nResults[nQueryIndex]);
	return S.pVulkanGPU->nResults[nQueryIndex];
	return 0;
}

uint64_t MicroProfileTicksPerSecondGpu_Vulkan()
{

	return S.pVulkanGPU->nFrequency;
	return 1;
}

uint32_t MicroProfileGpuFlip_Vulkan(void* pContext)
{
	const KMicroProfile::KMicroProfileVulkanFunctions* functions = &(KMicroProfile::GPUProfilerVulkan::s_VulkanFunctions);
	uint32_t nNode = S.pVulkanGPU->nCurrentNode;
	uint32_t nFrameIndex = S.pVulkanGPU->nFrame % MICROPROFILE_VULKAN_INTERNAL_DELAY;
	uint32_t nCount = 0, nStart = 0;

	VkCommandBuffer CommandBuffer = S.pVulkanGPU->Frames[nFrameIndex].CommandBuffer[nNode];
	auto& F = S.pVulkanGPU->Frames[nFrameIndex];
	VkFence Fence = F.Fences[nNode];
	VkDevice Device = S.pVulkanGPU->Devices[nNode];
	VkQueue Queue = S.pVulkanGPU->Queues[nNode];

	functions->vkWaitForFences(Device, 1, &Fence, 1, (uint64_t)-1);
	uint32_t nFrameTimeStamp = MicroProfileGpuInsertTimeStamp_Vulkan(pContext, functions);
	functions->vkResetCommandBuffer(F.CommandBuffer[nNode], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
	...
	nCount = S.pVulkanGPU->nFrameCount.exchange(0);
	nStart = S.pVulkanGPU->nFrameStart;
	S.pVulkanGPU->nFrameStart = (S.pVulkanGPU->nFrameStart + nCount) % MICROPROFILE_VULKAN_MAX_QUERIES;
	uint32_t nEnd = MicroProfileMin(nStart + nCount, (uint32_t)MICROPROFILE_VULKAN_MAX_QUERIES);
	MP_ASSERT(nStart != nEnd);
	uint32_t nSize = nEnd - nStart;
	
	S.pVulkanGPU->Frames[nFrameIndex].nBegin = nStart;
	S.pVulkanGPU->Frames[nFrameIndex].nCount = nCount;
	S.pVulkanGPU->Frames[nFrameIndex].nNode = nNode;
	S.pVulkanGPU->nFrame++;
	//fetch from earlier frames
	MicroProfileGpuFetchResults_Vulkan(CommandBuffer, S.pVulkanGPU->nFrame - MICROPROFILE_GPU_FRAME_DELAY, functions);
...
void MicroProfileGpuInitVulkan(VkDevice* pDevices, VkPhysicalDevice* pPhysicalDevices, VkQueue* pQueues, uint32_t* QueueFamily, uint32_t nNodeCount, const KMicroProfile::KMicroProfileVulkanFunctions* functions)
{
#if TEST_OLD_VERSION
	MP_ASSERT(functions);
	S.pGPU = MP_ALLOC_OBJECT(MicroProfileGpuTimerState);
	...
	for (uint32_t i = 0; i < nNodeCount; ++i)
	{
		...
		r = (VkResult)functions->vkCreateQueryPool(S.pGPU->Devices[i], &Q, 0, &S.pGPU->QueryPool[i]);
		MP_ASSERT(r == VK_SUCCESS);
		(void)r;

		CreateInfo.queueFamilyIndex = QueueFamily[i];
		r = (VkResult)functions->vkCreateCommandPool(S.pGPU->Devices[i], &CreateInfo, 0, &S.pGPU->CommandPool[i]);
		MP_ASSERT(r == VK_SUCCESS);
		(void)r;
		...
		for (uint32_t j = 0; j < MICROPROFILE_VULKAN_INTERNAL_DELAY; ++j)
		{
			...
			r = (VkResult)functions->vkAllocateCommandBuffers(S.pGPU->Devices[i], &AllocInfo, &F.CommandBuffer[i]);
			MP_ASSERT(r == VK_SUCCESS);
			(void)r;
			...
			r = (VkResult)functions->vkCreateFence(S.pGPU->Devices[i], &FCI, 0, &F.Fences[i]);
			MP_ASSERT(r == VK_SUCCESS);
			(void)r;
			...
			if (j == 0)
			{
				...
				CBI.pInheritanceInfo = 0;
				CBI.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
				functions->vkBeginCommandBuffer(F.CommandBuffer[i], &CBI);
				functions->vkCmdResetQueryPool(F.CommandBuffer[i], S.pGPU->QueryPool[i], 0, MICROPROFILE_VULKAN_MAX_QUERIES + 1);

				functions->vkEndCommandBuffer(F.CommandBuffer[i]);
				VkSubmitInfo SubmitInfo = {};
				SubmitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
				SubmitInfo.pNext = nullptr;
				SubmitInfo.waitSemaphoreCount = 0;
				...
				SubmitInfo.signalSemaphoreCount = 0;
				SubmitInfo.pSignalSemaphores = nullptr;
				functions->vkQueueSubmit(pQueues[i], 1, &SubmitInfo, F.Fences[i]);
				functions->vkWaitForFences(S.pGPU->Devices[i], 1, &F.Fences[i], 1, (uint64_t)-1);
				functions->vkResetCommandBuffer(F.CommandBuffer[i], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
			}
		}
	}

	VkPhysicalDeviceProperties Properties;
	functions->vkGetPhysicalDeviceProperties(pPhysicalDevices[0], &Properties);
	S.pGPU->nFrequency = 1000000000ll / Properties.limits.timestampPeriod;
#else
	MP_ASSERT(functions);
	S.pVulkanGPU = MP_ALLOC_OBJECT(MicroProfileGpuTimerStateVulkan);
	memset(S.pVulkanGPU, 0, sizeof(*S.pVulkanGPU));
	S.pVulkanGPU->nNodeCount = nNodeCount;
	VkQueryPoolCreateInfo Q;
	Q.sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO;
	Q.pNext = 0;
	Q.flags = 0;
	...
	for (uint32_t i = 0; i < nNodeCount; ++i)
	{
		S.pVulkanGPU->Devices[i] = pDevices[i];
		S.pVulkanGPU->PhysicalDevices[i] = pPhysicalDevices[i];
		S.pVulkanGPU->Queues[i] = pQueues[i];
		r = (VkResult)functions->vkCreateQueryPool(S.pVulkanGPU->Devices[i], &Q, 0, &S.pVulkanGPU->QueryPool[i]);
		MP_ASSERT(r == VK_SUCCESS);
		(void)r;

		CreateInfo.queueFamilyIndex = QueueFamily[i];
		r = (VkResult)functions->vkCreateCommandPool(S.pVulkanGPU->Devices[i], &CreateInfo, 0, &S.pVulkanGPU->CommandPool[i]);
		MP_ASSERT(r == VK_SUCCESS);
		(void)r;

		for (uint32_t j = 0; j < MICROPROFILE_VULKAN_INTERNAL_DELAY; ++j)
		{
			auto& F = S.pVulkanGPU->Frames[j];
			VkCommandBufferAllocateInfo AllocInfo;
			AllocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
			AllocInfo.pNext = 0;
			AllocInfo.commandBufferCount = 1;
			AllocInfo.commandPool = S.pVulkanGPU->CommandPool[i];
			AllocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
			r = (VkResult)functions->vkAllocateCommandBuffers(S.pVulkanGPU->Devices[i], &AllocInfo, &F.CommandBuffer[i]);
			MP_ASSERT(r == VK_SUCCESS);
			(void)r;
			...
			FCI.pNext = 0;
			FCI.flags = j == 0 ? 0 : VK_FENCE_CREATE_SIGNALED_BIT;
			r = (VkResult)functions->vkCreateFence(S.pVulkanGPU->Devices[i], &FCI, 0, &F.Fences[i]);
			MP_ASSERT(r == VK_SUCCESS);
			(void)r;
			...
			if (j == 0)
			{
				...
				CBI.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
				functions->vkBeginCommandBuffer(F.CommandBuffer[i], &CBI);
				functions->vkCmdResetQueryPool(F.CommandBuffer[i], S.pVulkanGPU->QueryPool[i], 0, MICROPROFILE_VULKAN_MAX_QUERIES + 1);
				...
				SubmitInfo.pSignalSemaphores = nullptr;
				functions->vkQueueSubmit(pQueues[i], 1, &SubmitInfo, F.Fences[i]);
				functions->vkWaitForFences(S.pVulkanGPU->Devices[i], 1, &F.Fences[i], 1, (uint64_t)-1);
				functions->vkResetCommandBuffer(F.CommandBuffer[i], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
			}
		}
	}
	...
	VkPhysicalDeviceProperties Properties;
	functions->vkGetPhysicalDeviceProperties(pPhysicalDevices[0], &Properties);
	S.pVulkanGPU->nFrequency = 1000000000ll / Properties.limits.timestampPeriod;
#endif
}

void MicroProfileGpuShutdown_Vulkan()
{
#if TEST_OLD_VERSION
	const KMicroProfile::KMicroProfileVulkanFunctions* functions = &(KMicroProfile::GPUProfilerVulkan::s_VulkanFunctions);
	MP_ASSERT(functions);
	...
	for (uint32_t i = 0; i < S.pGPU->nNodeCount; i++)
	{
		for (uint32_t j = 0; j < MICROPROFILE_VULKAN_INTERNAL_DELAY; ++j)
		{
			...
			if (F.Fences[i])
			{
				functions->vkDestroyFence(S.pGPU->Devices[i], F.Fences[i], 0);
			}
			if (F.CommandBuffer[i])
			{
				functions->vkFreeCommandBuffers(S.pGPU->Devices[i], S.pGPU->CommandPool[i], 1, &F.CommandBuffer[i]);
			}
		}

		functions->vkDestroyQueryPool(S.pGPU->Devices[i], S.pGPU->QueryPool[i], 0);
		functions->vkDestroyCommandPool(S.pGPU->Devices[i], S.pGPU->CommandPool[i], 0);
	}
	...
	for (uint32_t i = 0; i < S.pVulkanGPU->nNodeCount; i++)
	{
		for (uint32_t j = 0; j < MICROPROFILE_VULKAN_INTERNAL_DELAY; ++j)
		{
			auto& F = S.pVulkanGPU->Frames[j];
			if (F.Fences[i])
			{
				functions->vkDestroyFence(S.pVulkanGPU->Devices[i], F.Fences[i], 0);
			}
			if (F.CommandBuffer[i])
			{
				functions->vkFreeCommandBuffers(S.pVulkanGPU->Devices[i], S.pVulkanGPU->CommandPool[i], 1, &F.CommandBuffer[i]);
			}
		}

		functions->vkDestroyQueryPool(S.pVulkanGPU->Devices[i], S.pVulkanGPU->QueryPool[i], 0);
		functions->vkDestroyCommandPool(S.pVulkanGPU->Devices[i], S.pVulkanGPU->CommandPool[i], 0);
	}

	MP_FREE(S.pVulkanGPU);
	S.pVulkanGPU = 0;
#endif
}
void MicroProfileSetCurrentNodeVulkan(uint32_t nNode)
{
	...
	S.pGPU->nCurrentNode = nNode;
#else
	S.pVulkanGPU->nCurrentNode = nNode;
#endif
}

int MicroProfileGetGpuTickReference_Vulkan(int64_t* pOutCPU, int64_t* pOutGpu)
{
	const KMicroProfile::KMicroProfileVulkanFunctions* functions = &(KMicroProfile::GPUProfilerVulkan::s_VulkanFunctions);
	MP_ASSERT(functions);

	auto& F = S.pVulkanGPU->Frames[S.pVulkanGPU->nFrame % MICROPROFILE_VULKAN_INTERNAL_DELAY];
	uint32_t nGpu = S.pVulkanGPU->nCurrentNode;
	...
	CBI.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
	VkCommandBuffer CB = F.CommandBuffer[nGpu];
	VkDevice Device = S.pVulkanGPU->Devices[nGpu];
	VkFence Fence = F.Fences[nGpu];
	...
	functions->vkResetCommandBuffer(CB, VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
	functions->vkBeginCommandBuffer(CB, &CBI);
	functions->vkCmdResetQueryPool(CB, S.pVulkanGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES, 1);
	functions->vkCmdWriteTimestamp(CB, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, S.pVulkanGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES);
	functions->vkEndCommandBuffer(CB);
	VkSubmitInfo SubmitInfo = {};
	SubmitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
	SubmitInfo.pNext = nullptr;
	...
	SubmitInfo.signalSemaphoreCount = 0;
	SubmitInfo.pSignalSemaphores = nullptr;
	functions->vkQueueSubmit(S.pVulkanGPU->Queues[nGpu], 1, &SubmitInfo, Fence);
	functions->vkWaitForFences(Device, 1, &Fence, 1, (uint64_t)-1);
	*pOutGpu = 0;
	functions->vkGetQueryPoolResults(Device, S.pVulkanGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES, 1, 8, pOutGpu, 8, VK_QUERY_RESULT_64_BIT);
	*pOutCPU = MP_TICK();
	return 1;
}
#endif // #if MICROPROFILE_GPU_TIMERS_VULKAN
...
#include "KEnginePub/Private/profile/microprofile/microprofile_html.h"
